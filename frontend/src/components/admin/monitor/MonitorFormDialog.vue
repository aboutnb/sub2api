  ...
  >
    <form id="channel-monitor-form" @submit.prevent="handleSubmit" class="space-y-5">
      ...
      <div v-if="usesQuotaMode">
        ...
        <p class="mt-1 text-xs text-ink-muted">{{ t('admin.channelMonitor.form.linkedAccountHint') }}</p>
      ...
      <div v-if="usesProbePart">
        ...
        <p v-if="editing && editing.api_key_masked" class="mt-1 text-xs text-ink-muted">{{ editing.api_key_masked }}</p>
      </div>
      ...
      <div>
        ...
        <p class="mt-1 text-xs text-ink-muted">{{ t('admin.channelMonitor.form.intervalSecondsHint') }}</p>
      </div>
      ...
      <div>
        ...
        <p class="mt-1 text-xs text-ink-muted">{{ t('admin.channelMonitor.form.jitterSecondsHint') }}</p>
      </div>
      ...
      <div class="flex items-center justify-between">
        ...
        <Toggle
          ...
          :aria-label="t('admin.channelMonitor.form.enabled')"
        />
      </div>

      <!-- 高级设置区：请求模板 + 自定义 headers/body（仅探活模式有意义） -->
      <details v-if="usesProbePart" class="rounded-lg border border-line bg-surface-muted/50 p-3 dark:border-line dark:bg-canvas/30">
        <summary class="cursor-pointer text-sm font-medium text-ink dark:text-ink-muted">
          {{ t('admin.channelMonitor.advanced.section') }}
        </summary>
        <p class="mt-1 text-xs text-ink-muted">{{ t('admin.channelMonitor.advanced.sectionHint') }}</p>
        ...
        <div class="mt-4 space-y-4">
          <div>
            ...
            />
            <p class="mt-1 text-xs text-ink-muted">{{ t('admin.channelMonitor.templateField.applyHint') }}</p>
          </div>
...
function apiModeButtonClass(mode: APIMode): string {
  ...
  return 'border-blue-100 bg-white/70 text-ink hover:border-primary-300 dark:border-line dark:bg-surface dark:text-ink-muted'
}
