<template>
  <BaseDialog
    ...
    @close="$emit('close')"
  >
    <p class="mb-3 text-sm text-ink dark:text-ink-muted">
    ...
    <div v-else-if="monitors.length === 0" class="py-6 text-center text-sm text-ink-muted">
    ...
    <div v-else>
      ...
      <div class="mb-2 flex items-center gap-3 text-xs">
        ...
        <button
          type="button"
          class="text-ink-muted hover:underline dark:text-ink-muted"
          @click="selectNone"
        >
          {{ t('admin.channelMonitor.template.selectNone') }}
        </button>
        <span class="ml-auto text-ink-muted dark:text-ink-muted">
      ...
      <ul class="max-h-80 divide-y divide-line overflow-y-auto rounded-lg border border-line dark:divide-line dark:border-line">
        <li
          v-for="m in monitors"
          :key="m.id"
          class="flex cursor-pointer items-center gap-3 px-3 py-2 hover:bg-surface-muted dark:hover:bg-dark-800"
          @click="toggle(m.id)"
        >
          <input
            type="checkbox"
          ...
          <span class="text-xs text-ink-muted">{{ m.provider }}</span>
          <span v-if="m.provider === 'openai'" class="text-xs text-ink-muted">{{ m.api_mode }}</span>
          <span
            v-if="!m.enabled"
            class="ml-auto rounded bg-surface-muted px-1.5 py-0.5 text-xs text-ink-muted dark:bg-surface-muted dark:text-ink-muted"
          >
