  ...
  >
    <form id="import-proxy-data-form" class="space-y-4" @submit.prevent="handleImport">
      ...
      <div>
        ...
        <div
          class="flex items-center justify-between gap-3 rounded-lg border border-dashed border-line-strong bg-surface-muted px-4 py-3 dark:border-line-strong dark:bg-surface"
        >
          <div class="min-w-0">
            <div class="truncate text-sm text-ink dark:text-ink-strong">
              {{ fileName || t('admin.proxies.dataImportSelectFile') }}
            </div>
            <div class="text-xs text-ink-muted dark:text-ink-muted">JSON (.json)</div>
          </div>
