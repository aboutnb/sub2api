<template>
  <BaseDialog :show="show" :title="t('admin.users.groupConfig')" width="wide" @close="$emit('close')">
    <div v-if="user" class="space-y-6">
      <!-- 用户信息头部 -->
      <div class="flex items-center gap-4 rounded-2xl border-2 border-primary-300 bg-primary-50 p-5 shadow-pixel-sm dark:border-primary-700 dark:bg-primary-900/20">
        <div class="flex h-14 w-14 items-center justify-center rounded-full bg-white shadow-sm dark:bg-surface-muted">
          <span class="text-2xl font-semibold text-primary-600 dark:text-primary-400">{{ user.email.charAt(0).toUpperCase() }}</span>
        </div>
        <div class="flex-1">
          <p class="text-lg font-semibold text-ink-strong dark:text-white">{{ user.email }}</p>
          <p class="mt-1 text-sm text-ink dark:text-ink-muted">{{ t('admin.users.groupConfigHint', { email: user.email }) }}</p>
        </div>
      </div>
      ...
      <div v-else class="space-y-6">
        ...
        <div v-if="exclusiveGroups.length > 0">
          <div class="mb-3 flex items-center gap-2">
            ...
            <h4 class="text-sm font-semibold text-ink dark:text-ink-muted">{{ t('admin.users.exclusiveGroups') }}</h4>
            <span class="text-xs text-ink-muted">({{ exclusiveGroupConfigs.filter(c => c.isSelected).length }}/{{ exclusiveGroupConfigs.length }})</span>
          </div>
              ...
              <div class="flex items-center gap-4">
                ...
                <div class="flex-shrink-0">
                  <label class="relative flex h-6 w-6 cursor-pointer items-center justify-center">
                    <input
                      ...
                      class="peer sr-only"
                    />
                    <div class="h-5 w-5 rounded-md border-2 border-line-strong transition-all peer-checked:border-primary-500 peer-checked:bg-primary-500 dark:border-line-strong peer-checked:dark:border-primary-500">
                ...
                <div class="min-w-0 flex-1">
                  <div class="flex items-center gap-2">
                    <span class="text-base font-semibold text-ink-strong dark:text-white">{{ config.groupName }}</span>
                  ...
                  <div class="mt-1.5 flex items-center gap-3 text-sm">
                    <span class="inline-flex items-center gap-1 text-ink-muted dark:text-ink-muted">
                    ...
                    <span class="text-ink-muted dark:text-ink-muted">
                      {{ t('admin.users.defaultRate') }}: <span class="font-medium text-ink dark:text-ink-muted">{{ config.defaultRate }}x</span>
                    </span>
                  </div>
                </div>
                ...
                <div class="flex flex-shrink-0 items-center gap-3">
                  <label class="text-sm font-medium text-ink dark:text-ink-muted">{{ t('admin.users.customRate') }}</label>
                  <input
                    type="number"
                    step="0.001"
                    min="0.001"
                    ...
                    class="hide-spinner w-24 rounded-lg border border-line-strong bg-white px-3 py-2 text-sm font-medium transition-colors focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20 dark:border-line-strong dark:bg-surface-muted dark:focus:border-primary-500"
                  />
                </div>
              </div>
            </div>
        ...
        <div v-if="publicGroups.length > 0">
          <div class="mb-3 flex flex-wrap items-center gap-2">
            ...
            <h4 class="text-sm font-semibold text-ink dark:text-ink-muted">
              {{ restrictPublicGroups ? t('admin.users.publicGroupsRestricted') : t('admin.users.publicGroups') }}
            </h4>
            <span class="text-xs text-ink-muted">({{ publicGroupConfigs.length }})</span>
            <label class="ml-auto flex cursor-pointer items-center gap-2 text-sm text-ink dark:text-ink-muted">
              <input
                type="checkbox"
                :checked="restrictPublicGroups"
                @change="toggleRestrictPublicGroups"
                class="h-4 w-4 cursor-pointer rounded border-line-strong text-primary-600 focus:ring-primary-500 dark:border-line-strong"
              />
              {{ t('admin.users.restrictPublicGroups') }}
            </label>
          </div>
          <p class="mb-3 text-xs text-ink-muted dark:text-ink-muted">{{ t('admin.users.restrictPublicGroupsHint') }}</p>
          <div class="grid gap-3">
            <div
        ...
        <div v-if="groups.length === 0" class="flex flex-col items-center justify-center py-12 text-center">
          <div class="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-surface-muted dark:bg-surface-muted">
            <svg class="h-8 w-8 text-ink-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
            </svg>
          </div>
          <p class="text-ink-muted dark:text-ink-muted">{{ t('common.noGroupsAvailable') }}</p>
        </div>
      </div>
    </div>
