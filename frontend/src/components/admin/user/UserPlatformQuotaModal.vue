<template>
  ...
  >
    <div v-if="user" class="space-y-4">
      ...
      <div v-if="loading" class="py-10 text-center text-ink-muted">{{ t('common.loading') }}</div>
      <div v-else class="overflow-x-auto">
        <table class="min-w-full text-sm">
          <thead>
            <tr class="border-b border-line text-ink dark:border-line dark:text-ink-muted">
          ...
          <tbody>
            <tr v-for="row in quotas" :key="row.platform" class="border-b border-line dark:border-line">
              <td class="px-3 py-2 font-mono text-ink-strong dark:text-white">{{ row.platform }}</td>
              <td class="px-3 py-2">
                <div class="flex items-center gap-1">
                  ...
                  <button
                    type="button"
                    class="text-xs text-ink-muted hover:text-amber-500 disabled:opacity-50"
              ...
              <td class="px-3 py-2 text-xs text-ink-muted dark:text-ink-muted">
                {{ formatUsage(row.daily_usage_usd) }} / {{ formatUsage(row.weekly_usage_usd) }} / {{ formatUsage(row.monthly_usage_usd) }}
              </td>
            </tr>
          </tbody>
        </table>
        <p class="mt-3 text-xs text-ink-muted">{{ t('admin.users.platformQuota.hint') }}</p>
        <div class="mt-3">
