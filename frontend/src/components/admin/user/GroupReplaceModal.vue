    ...
    <div v-if="oldGroup" class="space-y-4">
      ...
      <div class="rounded-lg border border-line bg-surface-muted p-3 dark:border-line-strong dark:bg-surface">
        <div class="flex items-center gap-2">
          ...
          <span class="font-medium text-ink-strong dark:text-white">{{ oldGroup.name }}</span>
          <Icon name="arrowRight" size="sm" class="ml-auto text-ink-muted" />
      ...
      <div v-if="availableGroups.length > 0" class="max-h-64 space-y-2 overflow-y-auto">
        <label
          ...
          :class="selectedGroupId === group.id
            ...
            : 'border-line hover:border-line-strong dark:border-line-strong dark:hover:border-line-strong'"
        >
          <input
            type="radio"
            :value="group.id"
          ...
          <div class="flex-1">
            ...
            <span class="ml-2 text-xs text-ink-muted">{{ group.platform }}</span>
          </div>
        </label>
      </div>

      <!-- 无可选分组 -->
      <div v-else class="py-6 text-center text-sm text-ink-muted">
