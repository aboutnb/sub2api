<template>
  <div :class="flat ? '' : 'card overflow-hidden'">
    <div
      ...
      class="flex items-center justify-end gap-2 border-b border-line px-4 py-2 dark:border-line"
    >
      <span v-if="pendingIpCount > 0" class="text-xs text-ink-muted dark:text-ink-muted">
        {{ t('usage.ipGeo.pending', { count: pendingIpCount }) }}
      </span>
      <button
        type="button"
        class="inline-flex min-h-9 items-center gap-1 rounded-lg px-2 text-xs font-medium text-primary-600 transition-colors hover:bg-primary-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500/40 disabled:cursor-not-allowed disabled:opacity-50 dark:text-primary-400 dark:hover:bg-primary-900/30"
      ...
      >
        <template #cell-user="{ row }">
          <div class="text-sm">
            <button
              v-if="row.user?.email"
              class="inline-flex min-h-9 items-center rounded-lg px-2 font-medium text-primary-600 underline decoration-dashed underline-offset-2 transition-colors hover:bg-primary-50 hover:text-primary-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500/40 dark:text-primary-400 dark:hover:bg-primary-900/30 dark:hover:text-primary-300"
              @click="$emit('userClick', row.user_id, row.user?.email)"
              :title="t('admin.usage.clickToViewBalance')"
            >
              {{ row.user.email }}
            </button>
            <span v-else class="font-medium text-ink-strong dark:text-white">-</span>
            ...
            <span class="ml-1 text-ink-muted dark:text-ink-muted">#{{ row.user_id }}</span>
          </div>
        </template>

        <template #cell-api_key="{ row }">
          <span class="text-sm text-ink-strong dark:text-white">{{ row.api_key?.name || '-' }}</span>
        </template>

        <template #cell-account="{ row }">
          <span class="text-sm text-ink-strong dark:text-white">{{ row.account?.name || '-' }}</span>
        </template>
          ...
          <div class="space-y-0.5 text-xs">
            <div v-if="row.model_mapping_chain && row.model_mapping_chain.includes('→')" class="space-y-0.5">
              <div v-for="(step, i) in row.model_mapping_chain.split('→')" :key="i"
                   class="break-all"
                   :class="i === 0 ? 'font-medium text-ink-strong dark:text-white' : 'text-ink-muted dark:text-ink-muted'"
            ...
            <div v-else-if="row.upstream_model && row.upstream_model !== row.model" class="space-y-0.5">
              ...
              <div class="break-all text-ink-muted dark:text-ink-muted">
                <span class="mr-0.5">↳</span>{{ row.upstream_model }}
              </div>
            </div>
            <span v-else class="font-medium text-ink-strong dark:text-white">{{ row.model }}</span>
            <div
        ...
        <template #cell-reasoning_effort="{ row }">
          ...
          <span v-else data-testid="reasoning-effort-cell" class="text-sm text-ink-strong dark:text-white">
            {{ formatReasoningEffort(row.reasoning_effort) }}
          </span>
        </template>
        ...
        <template #cell-endpoint="{ row }">
          <div class="max-w-[320px] space-y-1 text-xs">
            <div class="break-all text-ink dark:text-ink-muted">
              <span class="font-medium text-ink-muted dark:text-ink-muted">{{ t('usage.inbound') }}:</span>
              <span class="ml-1">{{ row.inbound_endpoint?.trim() || '-' }}</span>
            </div>
            <div v-if="showUpstreamEndpoint" class="break-all text-ink dark:text-ink-muted">
              <span class="font-medium text-ink-muted dark:text-ink-muted">{{ t('usage.upstream') }}:</span>
        ...
        <template #cell-group="{ row }">
          ...
          <span v-else class="text-sm text-ink-muted dark:text-ink-muted">-</span>
        </template>
        ...
        <template #cell-tokens="{ row }">
          ...
          <div v-if="isImageUsage(row)" class="flex items-center gap-1.5">
            ...
            <span class="font-medium text-ink-strong dark:text-white">{{ row.image_count }}{{ t('usage.imageUnit') }}</span>
            <span class="text-ink-muted">({{ formatImageBillingSize(row, t) }})</span>
          </div>
          <!-- Token 请求 -->
          <div v-else class="flex items-center gap-1.5">
            <div class="space-y-1 text-sm">
              <div class="flex items-center gap-2">
                <div class="inline-flex items-center gap-1">
                  ...
                  <span class="font-medium text-ink-strong dark:text-white">{{ row.input_tokens?.toLocaleString() || 0 }}</span>
                </div>
                <div class="inline-flex items-center gap-1">
                  <Icon name="arrowUp" size="sm" class="h-3.5 w-3.5 text-violet-500" />
                  <span class="font-medium text-ink-strong dark:text-white">{{ row.output_tokens?.toLocaleString() || 0 }}</span>
                </div>
              </div>
            ...
            >
              <div class="flex h-4 w-4 cursor-help items-center justify-center rounded-full bg-surface-muted transition-colors group-hover:bg-blue-100 dark:bg-surface-muted dark:group-hover:bg-blue-900/50">
                <Icon name="infoCircle" size="xs" class="text-ink-muted group-hover:text-blue-500 dark:text-ink-muted dark:group-hover:text-blue-400" />
              </div>
            </div>
          </div>
        </template>
        ...
        <template #cell-latency="{ row }">
          <div class="flex items-stretch gap-2">
            ...
            <div class="grid grid-cols-[max-content_max-content] items-baseline gap-x-2 gap-y-0.5 text-xs">
              <span class="text-ink-muted dark:text-ink-muted">{{ t('usage.latencyFirstToken') }}</span>
              ...
              <span class="text-ink-muted dark:text-ink-muted">{{ t('usage.latencyDuration') }}</span>
        ...
        <template #cell-created_at="{ value }">
          <span class="text-sm text-ink dark:text-ink-muted">{{ formatDateTime(value) }}</span>
        </template>

        <template #cell-request_id="{ row }">
          <div v-if="row.request_id" class="flex max-w-[160px] items-center gap-1.5">
            <span class="truncate font-mono text-xs text-ink-muted dark:text-ink-muted" :title="row.request_id">
              {{ row.request_id }}
            </span>
            <button
              type="button"
              class="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-ink-muted transition-colors hover:bg-surface-muted hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500/40 dark:hover:bg-dark-700 dark:hover:text-ink-muted"
              ...
              :aria-label="copiedRequestId === row.request_id ? t('keys.copied') : t('keys.copyToClipboard')"
        ...
        <template #cell-user_agent="{ row }">
          <span v-if="row.user_agent" class="text-sm text-ink dark:text-ink-muted block max-w-[320px] truncate" :title="row.user_agent">{{ formatUserAgent(row.user_agent) }}</span>
          <span v-else class="text-sm text-ink-muted dark:text-ink-muted">-</span>
        </template>

        <template #cell-ip_address="{ row }">
          <div v-if="row.ip_address">
            <span class="text-sm font-mono text-ink dark:text-ink-muted">{{ row.ip_address }}</span>
  ...
  <Teleport to="body">
    ...
    >
      <div class="whitespace-nowrap rounded-lg border border-ink-strong bg-gray-900 px-3 py-2.5 text-xs text-white shadow-xl dark:border-gray-600 dark:bg-surface">
        <div class="space-y-1.5">
          <div>
            <div class="text-xs font-semibold text-ink-muted mb-1">{{ t('usage.tokenDetails') }}</div>
            <div v-if="tokenTooltipData && tokenTooltipData.input_tokens > 0 && !hasImageInputTokens(tokenTooltipData)" class="flex items-center justify-between gap-4">
              <span class="text-ink-muted">{{ t('admin.usage.inputTokens') }}</span>
            ...
            <div v-if="tokenTooltipData && hasImageInputTokens(tokenTooltipData)" class="flex items-center justify-between gap-4">
              <span class="text-ink-muted">{{ t('usage.imageInputTokens') }}</span>
            ...
            <div v-if="tokenTooltipData && tokenTooltipData.output_tokens > 0 && !hasImageOutputTokens(tokenTooltipData)" class="flex items-center justify-between gap-4">
              <span class="text-ink-muted">{{ t('admin.usage.outputTokens') }}</span>
            ...
            <div v-if="tokenTooltipData && hasImageOutputTokens(tokenTooltipData)" class="flex items-center justify-between gap-4">
              <span class="text-ink-muted">{{ t('usage.imageOutputTokens') }}</span>
            ...
            <div v-if="tokenTooltipData && tokenTooltipData.cache_creation_tokens > 0">
              ...
              <div v-else class="flex items-center justify-between gap-4">
                <span class="text-ink-muted">{{ t('admin.usage.cacheCreationTokens') }}</span>
            ...
            <div v-if="tokenTooltipData && tokenTooltipData.cache_read_tokens > 0" class="flex items-center justify-between gap-4">
              <span class="text-ink-muted">{{ t('admin.usage.cacheReadTokens') }}</span>
              <span class="font-medium text-white">{{ tokenTooltipData.cache_read_tokens.toLocaleString() }}</span>
            </div>
          </div>
          <div class="flex items-center justify-between gap-6 border-t border-ink-strong pt-1.5">
            <span class="text-ink-muted">{{ t('usage.totalTokens') }}</span>
          ...
          <div class="mb-2 border-b border-ink-strong pb-1.5">
            ...
            <template v-if="tooltipData && !isImageUsage(tooltipData) && (!tooltipData.billing_mode || tooltipData.billing_mode === BILLING_MODE_TOKEN)">
              <div v-if="tooltipData && textInputTokens(tooltipData) > 0" class="flex items-center justify-between gap-4">
                <span class="text-ink-muted">{{ t('usage.inputTokenPrice') }}</span>
              ...
              <div v-if="tooltipData && hasImageInputTokens(tooltipData)" class="flex items-center justify-between gap-4">
                <span class="text-ink-muted">{{ t('usage.imageInputTokenPrice') }}</span>
              ...
              <div v-if="tooltipData && tooltipData.output_cost > 0 && textOutputTokens(tooltipData) > 0" class="flex items-center justify-between gap-4">
                <span class="text-ink-muted">{{ t('usage.outputTokenPrice') }}</span>
              ...
              <div v-if="tooltipData && hasImageOutputTokens(tooltipData)" class="flex items-center justify-between gap-4">
                <span class="text-ink-muted">{{ t('usage.imageOutputTokenPrice') }}</span>
            ...
            <template v-else-if="tooltipData && isImageUsage(tooltipData)">
              ...
              <div v-if="formatImageSizeBreakdown(tooltipData)" class="flex items-center justify-between gap-4">
                <span class="text-ink-muted">{{ t('usage.imageSizeBreakdown') }}</span>
                <span class="font-medium text-white">{{ formatImageSizeBreakdown(tooltipData) }}</span>
              </div>
              <div class="flex items-center justify-between gap-4">
                <span class="text-ink-muted">{{ t('usage.imageUnitPrice') }}</span>
                <span class="font-medium text-sky-300">${{ imageUnitPrice(tooltipData).toFixed(6) }}</span>
              </div>
              <div class="flex items-center justify-between gap-4">
                <span class="text-ink-muted">{{ t('usage.imageTotalPrice') }}</span>
                <span class="font-medium text-white">${{ tooltipData.total_cost?.toFixed(6) || '0.000000' }}</span>
              </div>
            </template>
            <div v-else class="flex items-center justify-between gap-4">
              <span class="text-ink-muted">{{ t('usage.unitPrice') }}</span>
              <span class="font-medium text-sky-300">${{ tooltipData?.total_cost?.toFixed(6) || '0.000000' }}</span>
            </div>
            <div v-if="tooltipData && tooltipData.cache_creation_cost > 0" class="flex items-center justify-between gap-4">
              <span class="text-ink-muted">{{ t('admin.usage.cacheCreationCost') }}</span>
              <span class="font-medium text-white">${{ tooltipData.cache_creation_cost.toFixed(6) }}</span>
            </div>
            <div v-if="tooltipData && tooltipData.cache_read_cost > 0" class="flex items-center justify-between gap-4">
              <span class="text-ink-muted">{{ t('admin.usage.cacheReadCost') }}</span>
              <span class="font-medium text-white">${{ tooltipData.cache_read_cost.toFixed(6) }}</span>
            </div>
          </div>
          <!-- Rate and Summary -->
          <div class="flex items-center justify-between gap-6">
            <span class="text-ink-muted">{{ t('usage.serviceTier') }}</span>
            <span class="font-semibold text-cyan-300">{{ getUsageServiceTierLabel(tooltipData?.service_tier, t) }}</span>
          </div>
          <div class="flex items-center justify-between gap-6">
            <span class="text-ink-muted">{{ t('usage.rate') }}</span>
            <span class="font-semibold text-blue-400">{{ formatMultiplier(tooltipData?.rate_multiplier || 1) }}x</span>
          </div>
          <div class="flex items-center justify-between gap-6">
            <span class="text-ink-muted">{{ t('usage.original') }}</span>
            <span class="font-medium text-white">${{ tooltipData?.total_cost?.toFixed(6) || '0.000000' }}</span>
          </div>
          <div class="flex items-center justify-between gap-6">
            <span class="text-ink-muted">{{ t('usage.userBilled') }}</span>
            <span class="font-semibold text-green-400">${{ tooltipData?.actual_cost?.toFixed(6) || '0.000000' }}</span>
          </div>
          <!-- Account billing (separated from user billing) -->
          <template v-if="showAccountBilling">
            <div class="flex items-center justify-between gap-6 border-t border-ink-strong pt-1.5">
              <span class="text-ink-muted">{{ t('usage.accountMultiplier') }}</span>
              <span class="font-semibold text-blue-400">{{ formatMultiplier(tooltipData?.account_rate_multiplier ?? 1) }}x</span>
            </div>
            <div class="flex items-center justify-between gap-6">
              <span class="text-ink-muted">{{ t('usage.accountBilled') }}</span>
...
const getRequestTypeBadgeClass = (row: AdminUsageLog): string => {
  ...
  if (requestType === 'sync') return 'bg-surface-muted text-ink-strong dark:bg-surface-muted dark:text-gray-200'
