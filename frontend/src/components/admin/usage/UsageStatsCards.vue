<template>
  <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
    <div class="card p-4 flex items-center gap-3">
      ...
      <div>
        <p class="text-xs font-medium text-ink-muted">{{ t('usage.totalRequests') }}</p>
        <p class="text-xl font-bold">{{ stats?.total_requests?.toLocaleString() || '0' }}</p>
        <p class="text-xs text-ink-muted">{{ t('usage.inSelectedRange') }}</p>
      </div>
    </div>
    <div class="card p-4 flex items-center gap-3">
      ...
      <div>
        <p class="text-xs font-medium text-ink-muted">{{ t('usage.totalTokens') }}</p>
        <p class="text-xl font-bold">{{ formatTokens(stats?.total_tokens || 0) }}</p>
        <p class="flex flex-wrap items-center gap-x-1 text-xs text-ink-muted">
          ...
          <span class="group relative inline-flex cursor-help items-center gap-0.5" tabindex="0">
            ...
            <span
              class="pointer-events-none absolute left-1/2 top-full z-30 mt-2 hidden w-56 -translate-x-1/2 rounded-lg border border-line bg-white p-3 text-left text-xs text-ink shadow-lg group-hover:block group-focus:block dark:border-line-strong dark:bg-surface dark:text-ink-strong"
            >
              <span class="mb-2 block font-medium text-ink-strong dark:text-white">
                {{ cacheDetailLabel() }}
              </span>
    ...
    <div class="card p-4 flex items-center gap-3">
      ...
      <div class="min-w-0 flex-1">
        <p class="text-xs font-medium text-ink-muted">{{ t('usage.totalCost') }}</p>
    ...
    <div class="card p-4 flex items-center gap-3">
      ...
      <div><p class="text-xs font-medium text-ink-muted">{{ t('usage.avgDuration') }}</p><p class="text-xl font-bold">{{ formatDuration(stats?.average_duration_ms || 0) }}</p></div>
    </div>
  </div>
</template>
