    ...
    <div class="space-y-4">
      ...
      <div class="rounded-xl border border-line p-4 dark:border-line">
        <div class="flex items-center justify-between">
          <h4 class="text-sm font-semibold text-ink dark:text-gray-200">
        ...
        <div class="mt-3 space-y-2">
          <div v-if="tasksLoading" class="text-sm text-ink-muted dark:text-ink-muted">
            {{ t('admin.usage.cleanup.loadingTasks') }}
          </div>
          <div v-else-if="tasks.length === 0" class="text-sm text-ink-muted dark:text-ink-muted">
          ...
          <div v-else class="space-y-2">
            <div
              v-for="task in tasks"
              :key="task.id"
              class="flex flex-col gap-2 rounded-lg border border-line px-3 py-2 text-sm text-ink dark:border-line dark:text-ink-muted"
            >
              <div class="flex flex-wrap items-center justify-between gap-2">
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-xs text-ink-muted">#{{ task.id }}</span>
                  <button
                    v-if="canCancel(task)"
                    type="button"
              ...
              <div class="flex flex-wrap items-center gap-4 text-xs text-ink-muted dark:text-ink-muted">
...
const statusClass = (status: string) => {
  const map: Record<string, string> = {
    ...
    canceled: 'bg-line text-ink dark:bg-line-strong dark:text-ink-muted'
  }
  return map[status] || 'bg-surface-muted text-ink'
}
