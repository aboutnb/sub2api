<template>
  <div class="rounded-2xl border border-line bg-surface-muted p-4 dark:border-line dark:bg-surface/50">
    ...
    <div v-if="mode === 'custom'" class="mt-4 space-y-4">
      <div class="flex items-center justify-between">
        <div class="text-sm font-medium text-ink-strong dark:text-white">
          OR
          <span class="ml-1 text-xs font-normal text-ink-muted dark:text-ink-muted">
            ({{ anyOf.length }}/50)
          </span>
        </div>
        <button
      ...
      <div v-if="anyOf.length === 0" class="rounded-xl border border-dashed border-line-strong p-4 text-sm text-ink-muted dark:border-line-strong dark:text-ink-muted">
      ...
      <div
        ...
        :key="groupIndex"
        class="rounded-2xl border border-line bg-white p-4 shadow-sm dark:border-line dark:bg-surface"
      >
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0">
            <div class="text-sm font-medium text-ink-strong dark:text-white">
              {{ t('admin.announcements.form.targetingCustom') }} #{{ groupIndex + 1 }}
              <span class="ml-2 text-xs font-normal text-ink-muted dark:text-ink-muted">AND ({{ (group.all_of?.length || 0) }}/50)</span>
            </div>
            <div class="mt-1 text-xs text-ink-muted dark:text-ink-muted">
        ...
        <div class="mt-4 space-y-3">
          <div
            ...
            :key="condIndex"
            class="rounded-xl border border-line bg-surface-muted p-3 dark:border-line dark:bg-canvas/30"
          >
