      ...
      <div v-for="method in methods" :key="method.type" class="space-y-1">
        <div class="flex items-center justify-between">
          ...
          <div class="space-y-1 text-right">
            <span v-for="[currency, amount] in sortedAmounts(method.amount)" :key="currency" class="block text-sm font-medium text-ink-strong dark:text-white">
              {{ formatMoney(currency, amount) }}
            </span>
            <span class="ml-2 text-xs text-ink-muted dark:text-ink-muted">
              ({{ method.count }})
            </span>
          </div>
        </div>
        <div v-for="[currency, amount] in sortedAmounts(method.amount)" :key="currency" class="flex items-center gap-2">
          <span class="w-10 text-xs text-ink-muted dark:text-ink-muted">{{ currency }}</span>
          <div class="h-2 flex-1 overflow-hidden rounded-full bg-surface-muted dark:bg-surface-muted">
            <div
