<template>
  <section class="mt-3 border-t border-line pt-3 dark:border-line-strong">
    <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
      <div class="min-w-0 flex-1 sm:max-w-2xl">
        <label class="block text-xs font-medium text-ink-muted dark:text-ink-muted">
    ...
    <div v-if="modelValue.periods.length > 0" class="mt-3 space-y-3">
      <div
        ...
        :key="index"
        class="grid grid-cols-1 gap-2 border-t border-line pt-3 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_2rem] sm:items-end dark:border-line-strong"
      >
        <div class="min-w-0">
          <label :for="`${inputIdPrefix}-start-${index}`" class="block text-xs text-ink-muted">
        ...
        <div class="min-w-0">
          <label :for="`${inputIdPrefix}-end-${index}`" class="block text-xs text-ink-muted">
        ...
        <div class="min-w-0">
          <label :for="`${inputIdPrefix}-multiplier-${index}`" class="block text-xs text-ink-muted">
        ...
        <button
          type="button"
          class="flex h-8 w-8 items-center justify-center rounded text-ink-muted hover:text-red-500"
