<template>
  <div class="flex items-start gap-2 rounded border p-2"
       :class="isEmpty ? 'border-red-400 bg-red-50 dark:border-red-500 dark:bg-red-950/20' : 'border-line bg-white dark:border-line-strong dark:bg-surface-muted'">
    <!-- Token mode: context range + prices ($/MTok) -->
    <template v-if="mode === 'token'">
      <div class="pricing-interval-grid grid min-w-0 flex-1 gap-2">
        <div>
          <label class="text-xs text-ink-muted">{{ t('admin.channels.form.minTokens') }}</label>
        ...
        <div>
          <label class="text-xs text-ink-muted">{{ t('admin.channels.form.maxTokens') }} <span class="text-ink-muted">{{ t('admin.channels.form.inclusive') }}</span></label>
        ...
        <div>
          <label class="text-xs text-ink-muted">{{ t('admin.channels.form.inputPrice') }} <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-ink-muted">$/M</span></label>
        ...
        <div>
          <label class="text-xs text-ink-muted">{{ t('admin.channels.form.outputPrice') }} <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-ink-muted">$/M</span></label>
          <input :value="interval.output_price" @input="emitField('output_price', ($event.target as HTMLInputElement).value)"
            type="number" step="any" min="0" class="input mt-0.5 text-xs" />
        </div>
        <div>
          <label class="text-xs text-ink-muted">{{ t('admin.channels.form.cacheWrite5mPriceShort') }} <span class="text-ink-muted">$/M</span></label>
          <input :value="interval.cache_write_price" @input="emitField('cache_write_price', ($event.target as HTMLInputElement).value)"
            type="number" step="any" min="0" class="input mt-0.5 text-xs" />
        </div>
        <div>
          <label class="text-xs text-ink-muted">{{ t('admin.channels.form.cacheWrite1hPriceShort') }} <span class="text-ink-muted">$/M</span></label>
          <input :value="interval.cache_write_1h_price" @input="emitField('cache_write_1h_price', ($event.target as HTMLInputElement).value)"
            type="number" step="any" min="0" class="input mt-0.5 text-xs" />
        </div>
        <div>
          <label class="text-xs text-ink-muted">{{ t('admin.channels.form.cacheReadPriceShort') }} <span class="text-ink-muted">$/M</span></label>
        ...
        <template v-if="enableMultipliers">
          <div>
            <label class="text-xs text-ink-muted">{{ t('admin.channels.form.inputMultiplier') }}</label>
          ...
          <div>
            <label class="text-xs text-ink-muted">{{ t('admin.channels.form.outputMultiplier') }}</label>
            <input :value="interval.output_multiplier" @input="emitField('output_multiplier', ($event.target as HTMLInputElement).value)"
              type="number" step="any" min="0.000001" class="input mt-0.5 text-xs" />
          </div>
          <div>
            <label class="text-xs text-ink-muted">{{ t('admin.channels.form.cacheWriteMultiplier') }}</label>
          ...
          <div>
            <label class="text-xs text-ink-muted">{{ t('admin.channels.form.cacheReadMultiplier') }}</label>
    ...
    <template v-else>
      ...
      <div class="flex-1">
        <label class="text-xs text-ink-muted">{{ t('admin.channels.form.perRequestPrice') }} <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-ink-muted">$</span></label>
        <input :value="interval.per_request_price" @input="emitField('per_request_price', ($event.target as HTMLInputElement).value)"
          type="number" step="any" min="0" class="input mt-0.5 text-xs" />
      </div>
    </template>

    <button type="button" @click="emit('remove')" class="mt-4 rounded p-0.5 text-ink-muted hover:text-red-500">
      <Icon name="x" size="sm" />
    </button>
  </div>
</template>
