<template>
  <div class="rounded-lg border border-line bg-surface-muted p-3 dark:border-line-strong dark:bg-surface">
    ...
    >
      ...
      <div v-if="collapsed" class="flex min-w-0 flex-1 items-center gap-2 overflow-hidden">
        ...
        <div class="flex min-w-0 flex-1 flex-wrap items-center gap-1">
          ...
          <span
            ...
            class="whitespace-nowrap text-xs text-ink-muted"
          >
      ...
      <div v-else class="flex-1 text-xs font-medium text-ink-muted dark:text-ink-muted">
      ...
      <button
        ...
        class="flex-shrink-0 rounded p-1 text-ink-muted hover:text-red-500"
      >
    ...
    >
      <div class="collapsible-inner">
        ...
        <div class="mt-3 flex items-start gap-2">
          <div class="flex-1">
            <label class="text-xs font-medium text-ink-muted dark:text-ink-muted">
        ...
        <div v-if="entry.billing_mode === 'token'">
          ...
          <label class="mt-3 block text-xs font-medium text-ink-muted dark:text-ink-muted">
            {{ t('admin.channels.form.defaultPrices') }}
            <span class="ml-1 font-normal text-ink-muted">$/MTok</span>
          </label>
          <div class="pricing-default-grid mt-1 grid gap-2">
            <div>
              <label class="text-xs text-ink-muted">{{ t('admin.channels.form.inputPrice') }}</label>
          ...
          <div v-if="enableTierMultipliers" class="mt-3 grid max-w-2xl grid-cols-1 gap-2 sm:grid-cols-3">
            <div>
              <label class="text-xs text-ink-muted">{{ t('admin.channels.form.fastMultiplier') }}</label>
            ...
            <div>
              <label class="text-xs text-ink-muted">{{ t('admin.channels.form.flexMultiplier') }}</label>
          ...
          <div v-if="!hideTokenIntervals" class="mt-3">
            <div class="flex items-center justify-between">
              <label class="text-xs font-medium text-ink-muted dark:text-ink-muted">
                ...
                <span class="ml-1 font-normal text-ink-muted">(min, max]</span>
              </label>
        ...
        <div v-else-if="entry.billing_mode === 'per_request'">
          ...
          <label class="mt-3 block text-xs font-medium text-ink-muted dark:text-ink-muted">
            ...
            <span class="ml-1 font-normal text-ink-muted">$</span>
          </label>
          <div class="mt-1 w-48">
          ...
          <div v-else class="mt-2 rounded border border-dashed border-line-strong p-3 text-center text-xs text-ink-muted dark:border-line-strong">
