<template>
  <div class="card p-4">
        ...
        <table class="w-full text-xs">
          ...
          <tbody>
            <template v-for="item in displayEndpointStats" :key="item.endpoint">
              ...
              >
                <td class="max-w-[180px] truncate py-1.5 font-medium" :class="enableBreakdown ? 'text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300' : 'text-ink-strong dark:text-white'" :title="item.endpoint">
    ...
    <div v-else class="flex h-48 items-center justify-center text-sm text-ink-muted dark:text-ink-muted">
    ...
    datasets: [
      {
        ...
        backgroundColor: displayEndpointStats.value.map((_, index) =>
