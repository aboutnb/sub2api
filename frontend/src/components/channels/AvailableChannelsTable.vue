<template>
  ...
  <div class="table-wrapper">
    ...
    >
      <thead>
        <tr class="border-b border-line bg-surface-muted/50 text-xs font-medium uppercase tracking-wide text-ink-muted dark:border-line dark:bg-surface/50 dark:text-ink-muted">
      ...
      <tbody v-if="loading">
        <tr>
          <td colspan="5" class="py-10 text-center">
            <Icon name="refresh" size="lg" class="inline-block animate-spin text-ink-muted" />
          </td>
        </tr>
      </tbody>
      <tbody v-else-if="rows.length === 0">
        <tr>
          <td colspan="5" class="py-12 text-center">
            <Icon name="inbox" size="xl" class="mx-auto mb-3 h-12 w-12 text-ink-muted" />
            <p class="text-sm text-ink-muted dark:text-ink-muted">{{ emptyLabel }}</p>
          </td>
        </tr>
      </tbody>
      ...
      <tbody
        ...
        class="border-b-2 border-line last:border-b-0 dark:border-line-strong"
      >
        <tr
          v-for="(section, secIdx) in channel.platforms"
          :key="`${channel.name}-${section.platform}`"
          class="transition-colors hover:bg-surface-muted/40 dark:hover:bg-dark-800/40"
          :class="{ 'border-t border-line/70 dark:border-line/50': secIdx > 0 }"
        >
          <!-- 渠道名：只在第一行渲染并用 rowspan 纵向合并 -->
          <td
            v-if="secIdx === 0"
            :rowspan="channel.platforms.length"
            class="px-4 py-3 text-center align-middle font-medium text-ink-strong dark:text-white"
          >
            {{ channel.name }}
          </td>
          ...
          <td
            v-if="secIdx === 0"
            :rowspan="channel.platforms.length"
            class="px-4 py-3 align-middle text-xs text-ink-muted dark:text-ink-muted"
          >
          ...
          <td class="align-top px-4 py-3">
            <div class="flex flex-col gap-1.5">
              ...
              >
                <span
                  class="inline-flex items-center gap-0.5 text-[10px] font-medium uppercase text-ink-muted dark:text-ink-muted"
              ...
              <span v-if="section.groups.length === 0" class="text-xs text-ink-muted">-</span>
            </div>
          </td>
          ...
          <td class="align-top px-4 py-3">
            <div class="flex flex-wrap gap-1">
              ...
              />
              <span v-if="section.supported_models.length === 0" class="text-xs text-ink-muted">
                {{ noModelsLabel }}
              </span>
            </div>
          </td>
    ...
    <div data-testid="mobile-channels" class="w-full min-w-0 overflow-x-hidden lg:hidden">
      ...
      <section
        ...
        class="border-b-2 border-line px-4 py-4 last:border-b-0 dark:border-line-strong"
      >
        <header class="mb-3 min-w-0">
          <h3 class="break-words text-sm font-semibold text-ink-strong dark:text-white">
            {{ channel.name }}
          </h3>
          <p class="mt-1 break-words text-xs leading-5 text-ink-muted dark:text-ink-muted">
            {{ channel.description || '-' }}
          </p>
        </header>

        <div class="divide-y divide-line dark:divide-line/60">
          <div
          ...
          >
            ...
            <dl class="mt-3 space-y-3">
              <div class="min-w-0">
                <dt class="mb-1.5 text-[11px] font-medium text-ink-muted dark:text-ink-muted">
                  {{ columns.groups }}
                </dt>
