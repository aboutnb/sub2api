<template>
  <div
    :class="[
      ...
      enabled ? 'border-line dark:border-line-strong' : 'border-line bg-surface-muted opacity-50 dark:border-line dark:bg-surface/50',
    ]"
  ...
  >
    <div :class="[
      ...
      <div class="flex items-center gap-3">
        <div :class="[
          'rounded-md p-1.5',
          provider.enabled && enabled ? 'bg-green-100 dark:bg-green-900/30' : 'bg-surface-muted dark:bg-surface-muted',
        ]">
          <Icon
            name="server"
            size="sm"
            :class="provider.enabled && enabled ? 'text-green-600 dark:text-green-400' : 'text-ink-muted'"
          />
        </div>
        <span class="text-sm font-medium text-ink-strong dark:text-white">{{ provider.name }}</span>
        <span class="text-xs text-ink-muted dark:text-ink-muted">{{ keyLabel }}</span>
        <span v-if="provider.payment_mode" class="text-xs text-ink-muted dark:text-ink-muted">· {{ modeLabel }}</span>
        <span v-if="enabled && availableTypes.length" class="text-xs text-ink-muted dark:text-ink">|</span>
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="flex items-center gap-2 border-l border-line pl-3 dark:border-line-strong">
          <button type="button" @click="emit('edit')" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-ink-muted transition-colors hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-blue-900/20 dark:hover:text-blue-400">
            <Icon name="edit" size="sm" />
            <span class="text-xs">{{ t('common.edit') }}</span>
          </button>
          <button type="button" @click="emit('delete')" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-ink-muted transition-colors hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20 dark:hover:text-red-400">
