...
const statusMap: Record<OrderStatus, { key: string; class: string }> = {
  ...
  EXPIRED: { key: 'payment.status.expired', class: 'bg-surface-muted text-ink-strong dark:bg-canvas/30 dark:text-ink-muted' },
  CANCELLED: { key: 'payment.status.cancelled', class: 'bg-surface-muted text-ink-strong dark:bg-canvas/30 dark:text-ink-muted' },
...
const statusClass = computed(() => {
  const entry = statusMap[props.status]
  return entry?.class ?? 'bg-surface-muted text-ink-strong dark:bg-canvas/30 dark:text-ink-muted'
})
</script>
