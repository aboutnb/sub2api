<template>
  <BaseDialog :show="show" :title="dialogTitle" width="narrow" @close="handleClose">
    ...
    <div v-if="!success" class="flex flex-col items-center space-y-4">
      <!-- QR Code mode -->
      <template v-if="qrUrl">
        <div class="rounded-2xl bg-white p-4 shadow-sm dark:bg-surface">
          <canvas ref="qrCanvas" class="mx-auto"></canvas>
        </div>
        <p v-if="scanHint" class="text-center text-sm text-ink-muted dark:text-ink-muted">
          {{ scanHint }}
        </p>
      </template>
      ...
      <template v-else>
        <div class="flex flex-col items-center py-4">
          ...
          <p class="mt-4 text-sm text-ink-muted dark:text-ink-muted">{{ t('payment.qr.payInNewWindowHint') }}</p>
      ...
      <div v-else class="text-center">
        <p class="text-sm text-ink-muted dark:text-ink-muted">{{ qrUrl ? t('payment.qr.expiresIn') : '' }}</p>
      ...
      <div v-if="paidOrder" class="w-full rounded-xl bg-surface-muted p-4 dark:bg-surface">
        <div class="space-y-2 text-sm">
          ...
          <div class="flex justify-between">
            ...
            <span class="font-medium text-ink-strong dark:text-white">{{ paymentAmountSymbol(paidOrder) }}{{ paidOrder.pay_amount.toFixed(2) }}</span>
          </div>
        </div>
      </div>
    </div>
...
function reopenPopup() {
  if (props.payUrl) {
    window.open(props.payUrl, 'paymentPopup', getPaymentPopupFeaturesForMethod(props.paymentType))
  }
}
