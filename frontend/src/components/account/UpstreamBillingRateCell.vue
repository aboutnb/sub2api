<template>
  <div v-if="eligible" class="flex h-6 min-w-[7rem] items-center gap-1">
    <HelpTooltip class="-ml-1" width-class="w-max max-w-[calc(100vw-2rem)]" data-testid="upstream-billing-details">
      <template #trigger>
        <span
          class="cursor-help border-b border-dotted border-line-strong text-sm font-medium dark:border-line-strong"
          :class="hasEffectiveRate ? 'font-mono text-ink-strong dark:text-gray-200' : statusClass || 'text-ink-muted dark:text-ink-muted'"
...
const statusClass = computed(() => {
  if (!snapshot.value) return 'text-ink-muted dark:text-ink-muted'
  if (snapshot.value.status === 'unsupported') return 'text-ink-muted dark:text-ink-muted'
