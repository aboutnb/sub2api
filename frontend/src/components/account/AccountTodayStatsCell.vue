<template>
  <div>
    ...
    <div v-if="props.loading && !props.stats" class="space-y-0.5">
      ...
      <div class="h-3 w-16 animate-pulse rounded bg-line dark:bg-surface-muted"></div>
    ...
    <div v-else-if="props.stats" class="space-y-0.5 text-xs">
      ...
      <!-- Cost (Account) -->
      <div class="flex items-center gap-1">
        <span class="text-ink-muted dark:text-ink-muted">{{ t('usage.accountBilled') }}:</span>
      ...
      <div v-if="props.stats.user_cost != null" class="flex items-center gap-1">
        <span class="text-ink-muted dark:text-ink-muted">{{ t('usage.userBilled') }}:</span>
        <span class="font-medium text-ink dark:text-ink-muted">{{
