<template>
  <section v-if="state?.eligible" class="space-y-4 border-t border-line pt-4 dark:border-line-strong" data-testid="ollama-cloud-usage-settings">
    ...
    <div v-if="loading" class="flex h-20 items-center justify-center text-ink-muted">
    ...
    <template v-else>
      ...
      <div
        v-if="snapshot"
        class="border-y border-line py-3 dark:border-line"
        data-testid="ollama-cloud-usage-details"
      >
        <div class="grid grid-cols-[minmax(4rem,auto)_minmax(0,1fr)] gap-x-3 gap-y-1.5 text-xs">
          <span class="text-ink-muted dark:text-ink-muted">{{ t('admin.accounts.ollamaCloud.plan') }}</span>
          <span class="break-words text-ink-strong dark:text-white">{{ snapshot.data?.plan || '-' }}</span>
          <span class="text-ink-muted dark:text-ink-muted">{{ t('admin.accounts.ollamaCloud.fiveHour') }}</span>
          <span class="break-words text-ink-strong dark:text-white">{{ windowSummary(snapshot.data?.five_hour) }}</span>
          <span class="text-ink-muted dark:text-ink-muted">{{ t('admin.accounts.ollamaCloud.sevenDay') }}</span>
          <span class="break-words text-ink-strong dark:text-white">{{ windowSummary(snapshot.data?.seven_day) }}</span>
          <span class="text-ink-muted dark:text-ink-muted">{{ t('admin.accounts.ollamaCloud.balance') }}</span>
          <span class="break-words text-ink-strong dark:text-white">{{ snapshot.data?.balance || '-' }}</span>
          <span class="text-ink-muted dark:text-ink-muted">{{ t('admin.accounts.ollamaCloud.models') }}</span>
          <span class="break-words text-ink-strong dark:text-white">{{ modelSummary }}</span>
          <span class="text-ink-muted dark:text-ink-muted">{{ t('admin.accounts.ollamaCloud.status') }}</span>
          <span class="break-words font-medium text-ink-strong dark:text-white">{{ statusLabel }}</span>
          <span class="text-ink-muted dark:text-ink-muted">{{ t('admin.accounts.ollamaCloud.updatedAt') }}</span>
          <span class="break-words text-ink-strong dark:text-white">{{ formatDate(snapshot.fetched_at || snapshot.last_attempt_at) }}</span>
        </div>
        <p v-if="snapshot.last_error" class="mt-2 break-words border-t border-line pt-2 text-xs text-amber-700 dark:border-line dark:text-amber-300">
      ...
      <div v-if="state.configured" class="flex items-center justify-between gap-4 border-t border-line pt-4 dark:border-line">
        <div>
          <label class="text-sm font-medium text-ink-strong dark:text-white">
        ...
        <Toggle
          ...
          :disabled="saving"
          :aria-label="t('admin.accounts.ollamaCloud.autoRefresh')"
