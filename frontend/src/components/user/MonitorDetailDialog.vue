<template>
  <BaseDialog
    ...
    @close="$emit('close')"
  >
    <div v-if="loading" class="py-8 text-center text-sm text-ink-muted">
      {{ t('common.loading') }}
    </div>
    <div v-else-if="!detail" class="py-8 text-center text-sm text-ink-muted">
      {{ t('channelStatus.detailLoadError') }}
    </div>
    <div v-else class="overflow-x-auto">
      <table class="w-full text-left text-sm">
        <thead class="border-b border-line dark:border-line">
          <tr class="text-xs uppercase tracking-wider text-ink-muted dark:text-ink-muted">
        ...
        <tbody>
          <tr
            ...
            :key="m.model"
            class="border-b border-line dark:border-line"
          >
            <td class="py-2 pr-3 font-medium text-ink-strong dark:text-gray-100">{{ formatMonitorModel(m.model) }}</td>
            <td class="py-2 pr-3">
              <span
            ...
            <td class="py-2 pr-3 text-ink dark:text-ink-muted">{{ formatLatency(m.latest_latency_ms) }}</td>
            <td class="py-2 pr-3 text-ink dark:text-ink-muted">{{ formatPercent(m.availability_7d) }}</td>
            <td class="py-2 pr-3 text-ink dark:text-ink-muted">{{ formatPercent(m.availability_15d) }}</td>
            <td class="py-2 pr-3 text-ink dark:text-ink-muted">{{ formatPercent(m.availability_30d) }}</td>
            <td class="py-2 pr-3 text-ink dark:text-ink-muted">{{ formatLatency(m.avg_latency_7d_ms) }}</td>
          </tr>
        </tbody>
      </table>
    </div>
