  ...
  <div class="flex min-h-0 flex-1 flex-col">
    <div class="card flex min-h-0 flex-1 flex-col overflow-hidden">
      ...
      <DataTable
        ...
        :aria-label="t('usage.tabs.errors')"
        :caption="t('usage.tabs.errors')"
        clickable-rows
        server-side-sort
      ...
      >
        <template #cell-model="{ row }">
          <span v-if="row.model" class="text-sm font-medium text-ink-strong dark:text-white">{{ row.model }}</span>
          <span v-else class="text-sm text-ink-muted dark:text-ink-muted">-</span>
        </template>

        <template #cell-key_name="{ row }">
          <div class="text-sm">
            <span class="text-ink-strong dark:text-white">{{ row.key_name || '-' }}</span>
            <span
        ...
        <template #cell-category="{ row }">
          <span class="text-sm text-ink-strong dark:text-white">{{ t('usage.errors.categories.' + row.category) }}</span>
        </template>
