  ...
  <BaseDialog :show="show" :title="t('admin.users.attributes.title')" width="wide" @close="emit('close')">
    <div class="space-y-4">
      ...
      <div v-else-if="attributes.length === 0" class="py-12 text-center">
        <svg class="mx-auto h-12 w-12 text-ink-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1">
      ...
      <div v-else class="max-h-96 space-y-2 overflow-y-auto">
        <div
          ...
          :key="attr.id"
          class="flex items-center gap-3 rounded-lg border border-line bg-white p-3 dark:border-line-strong dark:bg-surface"
        >
          <!-- Drag Handle -->
          <div class="cursor-move text-ink-muted hover:text-ink dark:hover:text-ink-muted" :title="t('admin.users.attributes.dragToReorder')">
          ...
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              <span class="font-medium text-ink-strong dark:text-white">{{ attr.name }}</span>
              <span class="rounded bg-surface-muted px-1.5 py-0.5 font-mono text-xs text-ink-muted dark:bg-surface-muted dark:text-ink-muted">
                {{ attr.key }}
              </span>
            ...
            <div class="mt-0.5 flex items-center gap-2 text-xs text-ink-muted dark:text-ink-muted">
          ...
          <div class="flex items-center gap-1">
            <button
              ...
              class="rounded-lg p-1.5 text-ink-muted hover:bg-surface-muted hover:text-primary-600 dark:hover:bg-dark-700 dark:hover:text-primary-400"
              :title="t('common.edit')"
            >
            ...
            <button
              ...
              class="rounded-lg p-1.5 text-ink-muted hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20 dark:hover:text-red-400"
    ...
    <form id="attribute-form" @submit.prevent="handleSave" class="space-y-4">
      ...
      <div v-if="form.type === 'select' || form.type === 'multi_select'" class="space-y-2">
        ...
        <div v-for="(option, index) in form.options" :key="getOptionKey(option)" class="flex items-center gap-2">
          ...
          <button
            ...
            class="rounded-lg p-1.5 text-ink-muted hover:bg-red-50 hover:text-red-600"
          >
      ...
      <div class="flex items-center gap-6">
        <label class="flex items-center gap-2">
          <input v-model="form.required" type="checkbox" class="h-4 w-4 rounded border-line-strong text-primary-600" />
          <span class="text-sm text-ink dark:text-ink-muted">{{ t('admin.users.attributes.required') }}</span>
        </label>
        <label class="flex items-center gap-2">
          <input v-model="form.enabled" type="checkbox" class="h-4 w-4 rounded border-line-strong text-primary-600" />
          <span class="text-sm text-ink dark:text-ink-muted">{{ t('admin.users.attributes.enabled') }}</span>
        </label>
      </div>
    </form>
