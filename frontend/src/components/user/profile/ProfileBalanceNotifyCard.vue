<template>
  <div class="card">
    <div class="border-b border-line px-6 py-4 dark:border-line">
      <h2 class="text-lg font-medium text-ink-strong dark:text-white">
    ...
    <div class="px-6 py-6 space-y-6">
      ...
      <div class="flex items-center justify-between">
        ...
        <label class="relative inline-flex items-center cursor-pointer">
          ...
          <div class="w-11 h-6 bg-line peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-primary-300 dark:peer-focus:ring-primary-800 rounded-full peer dark:bg-surface-muted peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-line-strong after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:after:border-gray-600 peer-checked:bg-primary-600"></div>
        </label>
      </div>
      ...
      <template v-if="notifyEnabled">
        ...
        <div>
          <label class="input-label">
            ...
            <span class="text-xs text-ink-muted ml-2">{{ t('profile.balanceNotify.thresholdHint') }}</span>
          </label>
        ...
        <div>
          ...
          <div v-if="emailEntries.length > 0" class="space-y-2 mb-3">
            <div v-for="(entry, idx) in emailEntries" :key="idx"
              class="flex items-center justify-between px-3 py-2 bg-surface-muted dark:bg-surface-muted rounded-lg">
              <div class="flex items-center gap-2 min-w-0 flex-1">
                <label class="relative inline-flex items-center cursor-pointer shrink-0">
                  <input type="checkbox" :checked="!entry.disabled" @change="handleEmailToggle(entry)" class="sr-only peer" />
                  <div class="w-9 h-5 bg-line peer-focus:outline-none rounded-full peer dark:bg-line-strong peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-line-strong after:border after:rounded-full after:h-4 after:w-4 after:transition-all dark:after:border-line-strong peer-checked:bg-primary-600"></div>
                </label>
                <span class="text-sm text-ink dark:text-ink-muted truncate">{{ entry.email }}</span>
              </div>
                ...
                <template v-if="!entry.verified">
                  ...
                  <template v-if="verifyingEmail === entry.email">
                    <input
                      ...
                      type="text"
                      maxlength="6"
                      class="w-20 rounded border border-line-strong px-2 py-1 text-xs dark:border-line-strong dark:bg-surface-muted"
                    ...
                    <span v-if="verifyCountdown > 0" class="text-xs text-ink-muted">{{ verifyCountdown }}s</span>
                    <button v-else @click="sendCodeForSaved(entry.email)" :disabled="sendingSavedCode" class="text-xs text-ink-muted hover:text-ink">
                      {{ t('profile.balanceNotify.resend') }}
                    </button>
                    <button @click="verifyingEmail = ''" class="text-xs text-ink-muted hover:text-ink">
                      {{ t('common.cancel') }}
                    </button>
                  </template>
                  <template v-else>
          ...
          <div v-if="pendingEmails.length > 0" class="space-y-2 mb-3">
            <div v-for="(pe, idx) in pendingEmails" :key="pe.email"
              ...
              <span class="flex-1 text-sm text-ink dark:text-ink-muted">{{ pe.email }}</span>
              ...
              <div v-else class="flex items-center gap-1">
                ...
                <span v-if="pe.countdown > 0" class="text-xs text-ink-muted">{{ pe.countdown }}s</span>
                <button v-else @click="sendCodeFor(idx)" :disabled="pe.sending" class="text-xs text-ink-muted hover:text-ink">
                  {{ t('profile.balanceNotify.resend') }}
                </button>
              </div>
            </div>
