<template>
  <button
    type="button"
    class="group text-left p-5 rounded-2xl min-h-[280px] w-full bg-white/70 backdrop-blur-xl border border-line/80 shadow-card dark:bg-surface/60 dark:border-line/70 hover:-translate-y-1 hover:shadow-card-hover dark:hover:border-primary-500/30 hover:border-line-strong transition-all duration-300 ease-out flex flex-col"
    @click="emit('click')"
  >
    ...
    <div class="flex items-start gap-3">
      ...
      <div class="flex-1 min-w-0">
        <div class="text-base font-semibold truncate text-ink-strong dark:text-gray-100">
          {{ item.name }}
        </div>
        <div class="mt-0.5 flex items-center gap-1.5 min-w-0">
          ...
          <span class="font-mono text-xs truncate text-ink-muted dark:text-ink-muted">
            {{ formatMonitorModel(item.primary_model) }}
          </span>
          <span
            v-if="item.group_name"
            class="inline-flex items-center rounded-md px-1.5 py-0.5 text-[10px] font-medium bg-surface-muted text-ink dark:bg-surface-muted dark:text-ink-muted flex-shrink-0"
          >
            {{ item.group_name }}
          </span>
        </div>
    ...
    <!-- Divider -->
    <div class="mt-4 border-t border-line dark:border-line/60"></div>
...
const providerTintClass = computed(() =>
  PROVIDER_TINT[props.item.provider] ?? 'text-ink-muted dark:text-ink-muted'
)
