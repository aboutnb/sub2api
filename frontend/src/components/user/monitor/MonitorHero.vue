<template>
  <section class="py-3 md:py-4">
    <div class="flex items-center justify-end gap-3 flex-wrap">
      <div
        role="tablist"
        class="inline-flex p-0.5 rounded-xl bg-surface-muted dark:bg-surface border border-line/60 dark:border-line/60 text-xs"
      >
        <button
          ...
          :class="window === opt.value
            ? 'bg-white dark:bg-surface-muted shadow-sm text-ink-strong dark:text-white font-semibold'
      ...
      <button
        type="button"
        class="h-8 w-8 rounded-lg flex items-center justify-center text-ink-muted hover:text-ink hover:bg-surface-muted dark:text-ink-muted dark:hover:text-gray-200 dark:hover:bg-dark-700 transition-colors disabled:opacity-50"
        :disabled="loading"
