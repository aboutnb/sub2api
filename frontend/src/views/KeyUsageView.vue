<template>
  <div class="public-shell flex flex-col">
    <!-- Header (same pattern as HomeView) -->
    <header class="public-header relative z-20 px-4 py-3 sm:px-6">
      <nav class="mx-auto flex max-w-6xl items-center justify-between">
        <router-link to="/home" class="flex items-center gap-3">
          <div class="brand-mark-frame h-10 w-10">
            <img :src="siteLogo" :alt="`${siteName} logo`" class="h-full w-full object-contain" />
          </div>
          <span class="text-lg font-semibold tracking-tight text-ink-strong dark:text-white">{{ siteName }}</span>
        </router-link>
    ...
    <!-- Main Content -->
    <main class="mx-auto w-full max-w-5xl flex-1 px-4 py-10 sm:px-6 sm:py-12">
      <!-- Hero -->
      <div class="mb-10 text-center">
        <p class="pixel-kicker mb-4">API KEY / USAGE</p>
        <h1 class="mb-3 text-2xl font-extrabold tracking-tight text-ink-strong dark:text-white">
          {{ t('keyUsage.title') }}
        </h1>
        <p class="text-ink-muted dark:text-ink-muted text-base max-w-md mx-auto">
          {{ t('keyUsage.subtitle') }}
        </p>
      </div>

      <!-- Input Section -->
      <div class="public-panel mx-auto mb-14 max-w-xl p-4 sm:p-6">
        <div class="flex flex-col gap-3 sm:flex-row">
          <div class="flex-1 relative">
            <div class="absolute left-4 top-1/2 -translate-y-1/2 text-ink-muted dark:text-ink-muted">
            ...
            <button
              ...
              class="absolute right-1 top-1/2 flex min-h-11 min-w-11 -translate-y-1/2 items-center justify-center rounded-lg text-ink-muted transition-colors hover:bg-surface-muted hover:text-ink dark:text-ink-muted dark:hover:bg-dark-700 dark:hover:text-white"
        ...
        <p class="text-xs text-ink-muted dark:text-ink-muted mt-3 text-center">
        ...
        <div v-if="showDatePicker" class="mt-4">
          <div class="flex flex-wrap items-center gap-2 justify-center">
            <span class="text-xs text-ink-muted dark:text-ink-muted">{{ t('keyUsage.dateRange') }}</span>
            <button
          ...
          class="min-h-11 rounded-lg border-2 px-3 py-1.5 text-xs font-semibold transition-all"
              :class="currentRange === range.key
                ...
                : 'border-line-strong bg-white text-ink dark:border-line-strong dark:bg-canvas dark:text-ink-strong hover:border-primary-400'"
            >{{ range.label }}</button>
            <div v-if="currentRange === 'custom'" class="ml-1 flex w-full flex-wrap items-center justify-center gap-2 sm:w-auto sm:flex-nowrap">
              <input
                v-model="customStartDate"
                type="date"
                class="input min-h-10 w-[8.5rem] px-2 py-1.5 text-xs"
                :aria-label="t('keyUsage.startDate')"
              />
      ...
      <div v-if="showResults">
        ...
        <div v-else-if="resultData" class="space-y-6">
          ...
          <div v-if="statusInfo" class="fade-up flex items-center justify-center mb-2">
            <div class="inline-flex items-center gap-2 rounded-xl border-2 border-line-strong bg-white/90 px-5 py-2.5 shadow-pixel-sm backdrop-blur-sm dark:border-line-strong dark:bg-canvas/90">
              <span
                class="w-2.5 h-2.5 rounded-full pulse-dot"
                :class="statusInfo.isActive ? 'bg-emerald-500' : 'bg-rose-500'"
              ></span>
              <span class="text-sm font-medium text-ink-strong dark:text-white">{{ statusInfo.label }}</span>
              <span class="text-xs text-ink-muted dark:text-ink-muted">|</span>
              <span class="text-xs text-ink-muted dark:text-ink-muted">{{ statusInfo.statusText }}</span>
            </div>
          </div>
          ...
          <div v-if="ringItems.length > 0" :class="ringGridClass">
            <div
              ...
              :key="i"
              class="card card-hover fade-up p-8 backdrop-blur-sm"
              :class="`fade-up-delay-${Math.min(i + 1, 4)}`"
            >
              <div class="flex items-center justify-between mb-6">
                <h3 class="text-sm font-semibold uppercase tracking-wider text-ink-muted dark:text-ink-muted">
                  {{ ring.title }}
                </h3>
                <!-- Clock icon -->
                <svg v-if="ring.iconType === 'clock'" class="w-5 h-5 text-ink-muted dark:text-ink-muted" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
                </svg>
                <!-- Calendar icon -->
                <svg v-else-if="ring.iconType === 'calendar'" class="w-5 h-5 text-ink-muted dark:text-ink-muted" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/>
                </svg>
                <!-- Dollar icon -->
                <svg v-else class="w-5 h-5 text-ink-muted dark:text-ink-muted" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              ...
              <div class="flex justify-center">
                <div class="relative">
                  ...
                  <div class="absolute inset-0 flex flex-col items-center justify-center">
                    ...
                    <template v-else>
                      <span class="text-3xl font-bold tabular-nums text-ink-strong dark:text-white">
                        {{ displayPcts[i] ?? 0 }}%
                      </span>
                      <span class="text-xs text-ink-muted dark:text-ink-muted mt-0.5">{{ t('keyUsage.used') }}</span>
                      <span
                        class="text-sm font-semibold mt-1 tabular-nums"
                        :style="{ color: RING_GRADIENTS[i % 4].from }"
                      >{{ ring.amount }}</span>
                      <p v-if="ring.resetAt && formatResetTime(ring.resetAt)" class="text-xs text-ink-muted dark:text-ink-muted mt-0.5 tabular-nums">
          ...
          <div
            ...
            class="card fade-up fade-up-delay-3 overflow-hidden backdrop-blur-sm"
          >
            <div class="px-8 py-5 border-b border-line dark:border-line">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-ink-muted dark:text-ink-muted">{{ t('keyUsage.detailInfo') }}</h3>
            </div>
            <div class="divide-y divide-line dark:divide-line">
              ...
              >
                <div class="flex items-center gap-3">
                  ...
                  <span class="text-sm text-ink dark:text-ink-strong">{{ row.label }}</span>
                </div>
                <span class="text-sm font-semibold tabular-nums" :class="row.valueClass || 'text-ink-strong dark:text-white'">
                  {{ row.value }}
                </span>
              </div>
            </div>
          ...
          >
            <div class="px-8 py-5 border-b border-line dark:border-line">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-ink-muted dark:text-ink-muted">{{ t('keyUsage.tokenStats') }}</h3>
            </div>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-px bg-surface-muted dark:bg-surface">
              <div
                v-for="(cell, i) in usageStatCells"
                :key="i"
                class="bg-white px-6 py-4 dark:bg-canvas"
              >
                <div class="text-xs text-ink-muted dark:text-ink-muted mb-1">{{ cell.label }}</div>
                <div class="text-sm font-semibold tabular-nums text-ink-strong dark:text-white">{{ cell.value }}</div>
              </div>
            </div>
          </div>
          ...
          <div
            v-if="showDailyUsage"
            class="card fade-up fade-up-delay-4 overflow-hidden backdrop-blur-sm"
          >
            <div class="flex flex-col gap-3 px-8 py-5 border-b border-line dark:border-line sm:flex-row sm:items-center sm:justify-between">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-ink-muted dark:text-ink-muted">{{ t('keyUsage.dailyDetail') }}</h3>
              <div class="inline-flex rounded-xl border-2 border-line-strong bg-white p-1 dark:border-line-strong dark:bg-canvas">
                <button
                  v-for="option in dailyUsageOptions"
                  :key="option.value"
                  @click="setDailyUsageDays(option.value)"
                      class="min-h-11 min-w-12 rounded-lg border border-transparent px-3 py-1.5 text-xs font-bold transition-colors"
                  :class="dailyUsageDays === option.value
                    ? 'border-primary-700 bg-primary-100 text-primary-800 shadow-pixel-sm dark:border-primary-500 dark:bg-primary-900/30 dark:text-primary-200'
                    : 'text-ink hover:bg-surface-muted dark:text-ink dark:hover:bg-dark-800'"
                >
                  {{ option.label }}
                </button>
              </div>
            ...
            <div v-if="dailyUsageRows.length > 0" class="overflow-x-auto">
              <table class="w-full">
                <thead>
                  <tr class="border-b border-line bg-surface-muted dark:border-line dark:bg-canvas">
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-ink-muted dark:text-ink-muted">{{ t('keyUsage.date') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-ink-muted dark:text-ink-muted">{{ t('keyUsage.requests') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-ink-muted dark:text-ink-muted">{{ t('keyUsage.inputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-ink-muted dark:text-ink-muted">{{ t('keyUsage.outputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-ink-muted dark:text-ink-muted">{{ t('keyUsage.cacheReadTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-ink-muted dark:text-ink-muted">{{ t('keyUsage.cacheWriteTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-ink-muted dark:text-ink-muted">{{ t('keyUsage.cost') }}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr
                    ...
                    :key="row.date"
                    class="border-b border-line last:border-b-0 dark:border-line"
                  >
                    <td class="px-4 py-3 text-sm font-medium whitespace-nowrap text-ink-strong dark:text-white">{{ row.date }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-ink dark:text-ink-strong">{{ fmtNum(row.requests) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-ink dark:text-ink-strong">{{ fmtNum(row.input_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-ink dark:text-ink-strong">{{ fmtNum(row.output_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-ink dark:text-ink-strong">{{ fmtNum(row.cache_read_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-ink dark:text-ink-strong">{{ fmtNum(row.cache_write_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right font-medium text-ink-strong dark:text-white">{{ usd(row.actual_cost != null ? row.actual_cost : row.cost) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <div v-else class="px-8 py-8 text-center text-sm text-ink-muted dark:text-ink-muted">
          ...
          >
            <div class="px-8 py-5 border-b border-line dark:border-line">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-ink-muted dark:text-ink-muted">{{ t('keyUsage.modelStats') }}</h3>
            </div>
            <div class="overflow-x-auto">
              <table class="w-full">
                <thead>
                  <tr class="border-b border-line bg-surface-muted dark:border-line dark:bg-canvas">
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-ink-muted dark:text-ink-muted">{{ t('keyUsage.model') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-ink-muted dark:text-ink-muted">{{ t('keyUsage.requests') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-ink-muted dark:text-ink-muted">{{ t('keyUsage.inputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-ink-muted dark:text-ink-muted">{{ t('keyUsage.outputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-ink-muted dark:text-ink-muted">{{ t('keyUsage.cacheCreationTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-ink-muted dark:text-ink-muted">{{ t('keyUsage.cacheReadTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-ink-muted dark:text-ink-muted">{{ t('keyUsage.totalTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-ink-muted dark:text-ink-muted">{{ t('keyUsage.cost') }}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr
                    ...
                    class="border-b border-line last:border-b-0 dark:border-line"
                  >
                    <td class="px-4 py-3 text-sm font-medium whitespace-nowrap text-ink-strong dark:text-white">{{ m.model || '-' }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-ink dark:text-ink-strong">{{ fmtNum(m.requests) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-ink dark:text-ink-strong">{{ fmtNum(m.input_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-ink dark:text-ink-strong">{{ fmtNum(m.output_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-ink dark:text-ink-strong">{{ fmtNum(m.cache_creation_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-ink dark:text-ink-strong">{{ fmtNum(m.cache_read_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-ink dark:text-ink-strong">{{ fmtNum(m.total_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right font-medium text-ink-strong dark:text-white">{{ usd(m.actual_cost != null ? m.actual_cost : m.cost) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
    ...
    <footer class="relative z-10 border-t border-line/50 px-6 py-8 dark:border-line/50">
      <div class="mx-auto flex max-w-6xl flex-col items-center justify-center gap-4 text-center sm:flex-row sm:text-left">
        ...
        <div class="flex items-center gap-4">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            class="text-sm text-ink-muted transition-colors hover:text-ink dark:text-ink-muted dark:hover:text-white"
          >{{ t('home.docs') }}</a>
          <a
            :href="githubUrl"
            target="_blank"
...
const detailRows = computed<DetailRow[]>(() => {
  ...
  if (data.mode === 'quota_limited') {
    ...
    if (data.subscription) {
      ...
      if (subscriptionExpirationEnabled.value && sub.expires_at) {
        rows.push({
          iconBg: 'bg-amber-500/10', iconColor: 'text-amber-500', iconSvg: ICON_CALENDAR,
        ...
        rows.push({
          iconBg: 'bg-amber-500/10', iconColor: 'text-amber-500', iconSvg: ICON_CALENDAR,
          label: t('keyUsage.subscriptionExpires'), value: t('subscriptionProgress.noExpiration'), valueClass: '',
        })
      }
    }
