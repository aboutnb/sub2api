    ...
    <div class="space-y-4">
      ...
      <DataTable :columns="planColumns" :data="plans" :loading="plansLoading">
        ...
        <template #cell-price="{ value, row }">
          <div class="text-sm">
            <span class="font-medium text-ink-strong dark:text-white">{{ planCurrencySymbol(row.currency) }}{{ (value ?? 0).toFixed(2) }}</span>
            <span v-if="row.currency" class="ml-1 text-xs text-ink-muted">{{ row.currency }}</span>
            <span v-if="row.original_price" class="ml-1 text-xs text-ink-muted line-through">{{ planCurrencySymbol(row.currency) }}{{ row.original_price.toFixed(2) }}</span>
          </div>
        </template>
        ...
        <template #cell-for_sale="{ value, row }">
          <button
            ...
            :class="[
              ...
              value ? 'bg-primary-500' : 'bg-line-strong dark:bg-line-strong'
            ]"
        ...
        <template #cell-actions="{ row }">
          <div class="flex items-center gap-2">
            <button @click="openPlanEdit(row)" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-ink-muted transition-colors hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-blue-900/20 dark:hover:text-blue-400">
              <Icon name="edit" size="sm" />
              <span class="text-xs">{{ t('common.edit') }}</span>
            </button>
            <button @click="confirmDeletePlan(row)" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-ink-muted transition-colors hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20 dark:hover:text-red-400">
...
function getPlanNameClass(groupId: number): string {
  ...
  return group ? platformTextClass(group.platform) : 'text-ink-strong dark:text-white'
}
