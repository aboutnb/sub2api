<template>
  <AppLayout>
      ...
      <template #table>
        ...
        >
          <template #cell-created_at="{ value }">
            <span class="whitespace-nowrap text-ink dark:text-ink-muted">{{ formatTime(value) }}</span>
          </template>

          <template #cell-actor="{ row }">
            <div class="min-w-0 max-w-[220px]">
              <div class="truncate font-medium text-ink-strong dark:text-white" :title="row.actor_email">
                {{ row.actor_email || '—' }}
              </div>
          ...
          <template #cell-action="{ row }">
            <div class="min-w-0 max-w-xs">
              <div class="truncate font-mono text-sm text-ink-strong dark:text-gray-200" :title="row.action">
                {{ row.action }}
              </div>
              <div class="mt-0.5 truncate font-mono text-xs text-ink-muted" :title="`${row.method} ${row.path}`">
                {{ row.method }} {{ row.path }}
              </div>
            </div>
          </template>
          ...
          <template #cell-latency_ms="{ value }">
            <span class="whitespace-nowrap text-ink-muted dark:text-ink-muted">{{ value }} ms</span>
          </template>

          <template #cell-client_ip="{ value }">
            <span class="whitespace-nowrap font-mono text-ink dark:text-ink-muted">{{ value || '—' }}</span>
          </template>

          <template #cell-actions="{ row }">
            <button
              type="button"
              class="inline-flex min-h-9 items-center gap-1 rounded-lg px-2 font-medium text-primary-600 transition-colors hover:bg-primary-50 hover:text-primary-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500/40 dark:text-primary-400 dark:hover:bg-primary-900/30 dark:hover:text-primary-300"
          ...
          <template #empty>
            <div class="flex flex-col items-center py-8">
              <Icon name="shield" size="xl" class="mb-4 h-12 w-12 text-ink-muted dark:text-dark-600" />
              <p class="text-sm font-medium text-ink-muted dark:text-ink-muted">{{ t('admin.audit.empty') }}</p>
            </div>
          </template>
        </DataTable>
      </template>
    ...
    >
      <div v-if="detailLoading" class="flex items-center justify-center py-16">
        <div class="flex flex-col items-center gap-3">
          ...
          <div class="text-sm font-medium text-ink-muted dark:text-ink-muted">{{ t('common.loading') }}</div>
        </div>
      </div>

      <div v-else-if="detail" class="space-y-5 py-2">
        <!-- Hero: action + result at a glance -->
        <div class="rounded-2xl border border-line bg-surface-muted/60 p-5 dark:border-line dark:bg-canvas/60">
          <div class="flex flex-wrap items-center gap-3">
            ...
            <span class="break-all font-mono text-base font-semibold text-ink-strong dark:text-white">
              {{ detail.action }}
            </span>
          </div>

          <div class="mt-3 flex items-center gap-2 rounded-lg bg-white px-3 py-2 ring-1 ring-line dark:bg-surface dark:ring-line-strong">
            <span class="rounded bg-surface-muted px-1.5 py-0.5 font-mono text-[11px] font-bold text-ink dark:bg-surface-muted dark:text-gray-200">
              {{ detail.method }}
            </span>
            <span class="break-all font-mono text-xs text-ink dark:text-ink-muted">{{ detail.path }}</span>
          </div>

          <div class="mt-3 flex flex-wrap items-center gap-x-5 gap-y-1.5 text-xs text-ink-muted dark:text-ink-muted">
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <div class="rounded-xl bg-surface-muted p-4 dark:bg-canvas">
            ...
            <div class="mt-1 break-all text-sm font-medium text-ink-strong dark:text-white">
              {{ detail.actor_email || '—' }}
            </div>
            <div class="mt-0.5 text-xs text-ink-muted">{{ detail.actor_role }}</div>
          </div>
          ...
          <div class="rounded-xl bg-surface-muted p-4 dark:bg-canvas">
            ...
            <div v-if="detail.credential_masked" class="mt-0.5 break-all font-mono text-xs text-ink-muted">
        ...
        <!-- User-Agent -->
        <section>
          <h4 class="mb-1.5 text-xs font-bold uppercase tracking-wider text-ink-muted">
            {{ t('admin.audit.detail.userAgent') }}
          </h4>
          <div class="break-all rounded-xl bg-surface-muted p-3 font-mono text-xs leading-relaxed text-ink dark:bg-canvas dark:text-ink-muted">
            {{ detail.user_agent || '—' }}
          </div>
        </section>
        ...
        <section v-if="detail.request_body">
          ...
          <pre class="max-h-72 overflow-auto rounded-xl bg-surface-muted p-4 font-mono text-xs leading-relaxed text-ink dark:bg-canvas dark:text-ink-muted">{{ prettyBody(detail.request_body) }}</pre>
        </section>
        ...
        <section v-if="detail.extra && Object.keys(detail.extra).length">
          ...
          <pre class="max-h-48 overflow-auto rounded-xl bg-surface-muted p-4 font-mono text-xs leading-relaxed text-ink dark:bg-canvas dark:text-ink-muted">{{ JSON.stringify(detail.extra, null, 2) }}</pre>
        </section>
      </div>
    </BaseDialog>
    ...
    >
      <div class="py-2">
        <p class="text-sm text-ink-muted dark:text-ink-muted">{{ t('admin.audit.clearConfirm.totpHint') }}</p>
        <input
