<template>
  <AppLayout>
    <div class="mx-auto max-w-[1600px] space-y-6">
      ...
      <div v-if="error" class="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700 dark:border-red-900/50 dark:bg-red-900/20 dark:text-red-300">{{ error }}</div>
          ...
          <div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
            <div class="rounded-xl border border-gray-200 p-4 dark:border-dark-700">
              ...
              <div class="mt-4 grid grid-cols-2 gap-3">
                <label class="text-sm text-gray-600 dark:text-dark-300">{{ t('admin.checkin.normalMin') }}<input v-model="form.normal_min" class="input mt-2" type="number" min="0" max="100" step="0.00000001" /></label>
                <label class="text-sm text-gray-600 dark:text-dark-300">{{ t('admin.checkin.normalMax') }}<input v-model="form.normal_max" class="input mt-2" type="number" min="0" max="100" step="0.00000001" /></label>
              </div>
            </div>
            <div class="rounded-xl border border-gray-200 p-4 dark:border-dark-700">
              <h3 class="font-medium text-gray-900 dark:text-white">{{ t('admin.checkin.lucky') }}</h3>
              <div class="mt-4 grid grid-cols-2 gap-3">
                <label class="text-sm text-gray-600 dark:text-dark-300">{{ t('admin.checkin.luckyMin') }}<input v-model="form.lucky_min_multiplier" class="input mt-2" type="number" min="-1" max="1" step="0.00000001" /></label>
                <label class="text-sm text-gray-600 dark:text-dark-300">{{ t('admin.checkin.luckyMax') }}<input v-model="form.lucky_max_multiplier" class="input mt-2" type="number" min="-1" max="1" step="0.00000001" /></label>
              </div>
            </div>
          </div>
          ...
          <table class="min-w-[1050px] w-full text-left text-sm">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-dark-700">
              <tr v-for="record in records" :key="record.id" class="text-gray-700 dark:text-dark-300">
                ...
                <td class="px-5 py-4 font-mono">{{ record.random_value.toFixed(4) }}</td>
...
function errorMessage(value: unknown) { const response = (value as { response?: { data?: { message?: string; detail?: string } } })?.response; return response?.data?.message || response?.data?.detail || t('admin.checkin.loadFailed') }

async function loadRecords(page = pagination.page) {
  recordsLoading.value = true
  try { const response = await adminAPI.checkin.getRecords({ page, page_size: pagination.page_size, date: filters.date || undefined, email: filters.email || undefined }); records.value = response.items || []; pagination.page = response.page; pagination.total = response.total } catch (value) { error.value = errorMessage(value) } finally { recordsLoading.value = false }
}
async function load() {
  loading.value = true; error.value = ''
  try { const [nextConfig, nextOverview] = await Promise.all([adminAPI.checkin.getConfig(), adminAPI.checkin.getOverview(filters.date || undefined)]); applyConfig(nextConfig); overview.value = nextOverview; await loadRecords(1) } catch (value) { error.value = errorMessage(value) } finally { loading.value = false }
}
async function saveConfig() {
  if (!config.value || saving.value) return
  saving.value = true
  try {
    const next = await checkinStepUp.run(() => adminAPI.checkin.updateConfig({ ...form, expected_config_version: config.value!.config_version }))
    applyConfig(next)
    form.change_reason = ''
