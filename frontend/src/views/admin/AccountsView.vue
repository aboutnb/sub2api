  ...
  <AppLayout>
            ...
            <template #after>
              ...
              <div class="relative" ref="autoRefreshDropdownRef">
                ...
                <div
                  ...
                  class="absolute right-0 z-50 mt-2 w-56 origin-top-right rounded-lg border border-line bg-white shadow-lg dark:border-line dark:bg-surface"
                >
                  <div class="p-2">
                    <button
                      @click="setAutoRefreshEnabled(!autoRefreshEnabled)"
                      class="flex w-full items-center justify-between rounded-md px-3 py-2 text-sm text-ink hover:bg-surface-muted dark:text-gray-200 dark:hover:bg-dark-700"
                    >
              ...
              <div class="relative" ref="accountToolsDropdownRef">
                ...
                <Teleport to="body">
                  <div
                    ...
                    class="fixed z-[9999] origin-top-right overflow-hidden rounded-lg border border-line bg-white shadow-xl dark:border-line dark:bg-surface"
                    :style="accountToolsDropdownStyle"
                    @click.stop
                  >
                    <div class="overflow-y-auto p-2" :style="{ maxHeight: `${accountToolsDropdownPosition.maxHeight}px` }">
                      <div class="px-2 py-2">
                        <div class="text-xs font-semibold uppercase tracking-wide text-ink-muted dark:text-ink-muted">
                      ...
                      <div class="my-2 border-t border-line dark:border-line"></div>
                      <div class="px-2 py-2">
                        <div class="text-xs font-semibold uppercase tracking-wide text-ink-muted dark:text-ink-muted">
                      ...
                      <div class="px-2 py-2">
                        <div class="flex items-center justify-between gap-3">
                          <span class="text-xs font-semibold uppercase tracking-wide text-ink-muted dark:text-ink-muted">
                            {{ t('admin.accounts.viewColumns') }}
                          </span>
                          <Icon name="grid" size="sm" class="text-ink-muted" />
                        </div>
                      </div>
                      <div class="grid grid-cols-1 gap-1">
                        <button
                          ...
                          class="flex w-full items-center justify-between rounded-md px-3 py-2 text-sm text-ink transition-colors hover:bg-surface-muted dark:text-gray-200 dark:hover:bg-dark-700"
                        >
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-select="{ row }">
            <input type="checkbox" :checked="isSelected(row.id)" @change="toggleSel(row.id)" class="rounded border-line-strong text-primary-600 focus:ring-primary-500" />
          </template>
          <template #cell-id="{ value }">
            <span class="font-mono text-xs text-ink-muted dark:text-ink-muted">#{{ value }}</span>
          </template>
              ...
              >
                <template #trigger>
                  <a
                    ...
                    class="border-b border-dotted border-line-strong font-medium text-ink-strong dark:border-line-strong dark:text-white"
                  >
                    {{ value }}
                  </a>
                </template>
              </HelpTooltip>
              <span v-else class="font-medium text-ink-strong dark:text-white">{{ value }}</span>
              <span
                v-if="accountDisplayEmail(row)"
                class="text-xs text-ink-muted dark:text-ink-muted truncate max-w-[200px]"
          ...
          <template #cell-notes="{ value }">
            <span v-if="value" :title="value" class="block max-w-xs truncate text-sm text-ink dark:text-ink-muted">{{ value }}</span>
          ...
          <template #cell-schedulable="{ row }">
            <button @click="handleToggleSchedulable(row)" :disabled="togglingSchedulable === row.id" class="relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 dark:focus:ring-offset-dark-800" :class="[row.schedulable ? 'bg-primary-500 hover:bg-primary-600' : 'bg-line hover:bg-line-strong dark:bg-line-strong dark:hover:bg-dark-500']" :title="row.schedulable ? t('admin.accounts.schedulableEnabled') : t('admin.accounts.schedulableDisabled')">
          ...
          <template #cell-proxy="{ row }">
            <div class="flex flex-col gap-1">
              <div v-if="row.proxy" class="flex items-center gap-2">
                <span class="text-sm text-ink dark:text-ink-muted">{{ row.proxy.name }}</span>
                <span v-if="row.proxy.country_code" class="text-xs text-ink-muted dark:text-ink-muted">
                  ({{ row.proxy.country_code }})
                </span>
              </div>
              <span v-else class="text-sm text-ink-muted dark:text-ink-muted">-</span>
              <div v-if="row.proxy && row.proxy.expires_at" class="flex items-center gap-2 text-xs">
                <span class="text-ink dark:text-ink-muted">{{ formatDateTime(row.proxy.expires_at) }}</span>
              ...
              <div v-if="row.proxy_fallback_origin_id" class="flex items-center gap-1">
                ...
                <button class="text-xs px-1.5 py-0.5 rounded border border-line-strong dark:border-line-strong text-ink dark:text-ink-muted hover:bg-surface-muted dark:hover:bg-dark-700" @click="onRevertFallback(row)">{{ t('admin.accounts.revertProxy') }}</button>
              </div>
            </div>
          </template>
          <template #cell-rate_multiplier="{ row }">
            <span class="inline-flex items-center gap-1 text-sm font-mono text-ink dark:text-ink-muted">
          ...
          <template #cell-scheduler_score="{ row }">
            <div v-if="getSchedulerScoreRows(row).length" class="flex min-w-[7rem] flex-col gap-0.5 font-mono text-[11px] leading-4">
              <div
                ...
                class="flex items-center gap-1 whitespace-nowrap text-ink dark:text-ink-muted"
                :title="`${formatSchedulerScoreGroup(score)} / ${formatSchedulerScore(score.base_score)} / ${formatStickySchedulerScore(score)}`"
              >
                <span class="max-w-[4.75rem] truncate text-ink-muted dark:text-ink-muted">{{ formatSchedulerScoreGroup(score) }}</span>
                <span class="text-ink-muted dark:text-ink">/</span>
          ...
          <template #cell-last_used_at="{ value }">
            <span class="text-sm text-ink-muted dark:text-ink-muted">{{ formatRelativeTime(value) }}</span>
          </template>
          ...
          <template #cell-expires_at="{ row, value }">
            <div class="flex flex-col items-start gap-1">
              <span class="text-sm text-ink-muted dark:text-ink-muted">{{ formatExpiresAt(value) }}</span>
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              <button @click="handleEdit(row)" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-ink-muted transition-colors hover:bg-surface-muted hover:text-primary-600 dark:hover:bg-dark-700 dark:hover:text-primary-400">
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" /></svg>
                <span class="text-xs">{{ t('common.edit') }}</span>
              </button>
              <button @click="handleDelete(row)" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-ink-muted transition-colors hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20 dark:hover:text-red-400">
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" /></svg>
                <span class="text-xs">{{ t('common.delete') }}</span>
              </button>
              <button @click="openMenu(row, $event)" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-ink-muted transition-colors hover:bg-surface-muted hover:text-ink-strong dark:hover:bg-dark-700 dark:hover:text-white">
    ...
    <ConfirmDialog :show="showExportDataDialog" :title="t('admin.accounts.dataExport')" :message="t('admin.accounts.dataExportConfirmMessage')" :confirm-text="t('admin.accounts.dataExportConfirm')" :cancel-text="t('common.cancel')" @confirm="handleExportData" @cancel="showExportDataDialog = false">
      <label class="flex items-center gap-2 text-sm text-ink dark:text-ink-muted">
        <input type="checkbox" class="h-4 w-4 rounded border-line-strong text-primary-600 focus:ring-primary-500" v-model="includeProxyOnExport" />
...
// Keep batched usage loading aligned with DataTable's desktop table breakpoint.
...
function getAntigravityTierClass(row: any): string {
  const tier = getAntigravityTierFromRow(row)
  switch (tier) {
    case 'free-tier': return 'bg-surface-muted text-ink dark:bg-surface-muted dark:text-ink-muted'
...
.account-tools-menu-item {
  @apply flex w-full items-center gap-3 rounded-md px-3 py-2 text-sm text-ink transition-colors hover:bg-surface-muted dark:text-gray-200 dark:hover:bg-dark-700;
}
