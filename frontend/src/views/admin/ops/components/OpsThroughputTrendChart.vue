...
const colors = computed(() => ({
  ...
  blueAlpha: chartTheme.value.seriesSoft[0],
  green: chartTheme.value.series[7],
  greenAlpha: chartTheme.value.seriesSoft[7],
...
<template>
  <div class="flex h-full min-w-0 flex-col rounded-2xl border-2 border-line bg-white p-6 shadow-card dark:border-line dark:bg-surface">
    <div
    ...
    >
      <h3 class="flex min-w-0 items-center gap-2 text-sm font-bold text-ink-strong dark:text-white">
      ...
      <div
        ...
        class="flex w-full min-w-0 flex-wrap items-center gap-2 text-xs text-ink-muted dark:text-ink-muted sm:w-auto sm:justify-end"
      >
    ...
    <div v-if="(props.topGroups?.length ?? 0) > 0" class="mb-3 flex flex-wrap gap-2">
      <button
        ...
        :key="g.group_id"
        type="button"
        class="inline-flex items-center gap-2 rounded-full border border-line bg-white px-3 py-1 text-[11px] font-semibold text-ink hover:bg-surface-muted dark:border-line dark:bg-canvas dark:text-gray-200 dark:hover:bg-dark-800"
        @click="emit('selectGroup', g.group_id)"
      >
        <span class="max-w-[180px] truncate">{{ g.group_name || `#${g.group_id}` }}</span>
        <span class="text-ink-muted dark:text-ink-muted">{{ formatNumber(g.request_count) }}</span>
      </button>
    </div>
    ...
    <div v-else-if="(props.byPlatform?.length ?? 0) > 0" class="mb-3 flex flex-wrap gap-2">
      ...
      >
        ...
        <span class="text-ink-muted dark:text-ink-muted">{{ formatNumber(p.request_count) }}</span>
      </button>
    </div>
