  ...
  <BaseDialog :show="show" :title="title" width="full" :close-on-click-outside="true" @close="close">
    <div v-if="loading" class="flex items-center justify-center py-16">
      <div class="flex flex-col items-center gap-3">
        ...
        <div class="text-sm font-medium text-ink-muted dark:text-ink-muted">{{ t('admin.ops.errorDetail.loading') }}</div>
      </div>
    </div>

    <div v-else-if="!detail" class="py-10 text-center text-sm text-ink-muted dark:text-ink-muted">
      {{ emptyText }}
    </div>
    ...
    <div v-else class="space-y-6 p-6">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div class="rounded-xl bg-surface-muted p-4 dark:bg-canvas">
          <div class="text-xs font-bold uppercase tracking-wider text-ink-muted">{{ t('admin.ops.errorDetail.requestId') }}</div>
          <div class="mt-1 break-all font-mono text-sm font-medium text-ink-strong dark:text-white">
            {{ requestId || '—' }}
          </div>
        </div>

        <div class="rounded-xl bg-surface-muted p-4 dark:bg-canvas">
          <div class="text-xs font-bold uppercase tracking-wider text-ink-muted">{{ t('admin.ops.errorDetail.time') }}</div>
          <div class="mt-1 text-sm font-medium text-ink-strong dark:text-white">
            {{ formatDateTime(detail.created_at) }}
          </div>
        </div>

        <div class="rounded-xl bg-surface-muted p-4 dark:bg-canvas">
          <div class="text-xs font-bold uppercase tracking-wider text-ink-muted">
        ...
        <div class="rounded-xl bg-surface-muted p-4 dark:bg-canvas">
          <div class="text-xs font-bold uppercase tracking-wider text-ink-muted">{{ t('admin.ops.errorDetail.platform') }}</div>
          <div class="mt-1 text-sm font-medium text-ink-strong dark:text-white">
            {{ detail.platform || '—' }}
          </div>
        </div>
        ...
        <div class="rounded-xl bg-surface-muted p-4 dark:bg-canvas">
          <div class="text-xs font-bold uppercase tracking-wider text-ink-muted">{{ t('admin.ops.errorDetail.group') }}</div>
          <div class="mt-1 text-sm font-medium text-ink-strong dark:text-white">
        ...
        <div class="rounded-xl bg-surface-muted p-4 dark:bg-canvas">
          <div class="text-xs font-bold uppercase tracking-wider text-ink-muted">{{ t('admin.ops.errorDetail.model') }}</div>
          <div class="mt-1 text-sm font-medium text-ink-strong dark:text-white">
        ...
        <div class="rounded-xl bg-surface-muted p-4 dark:bg-canvas">
          <div class="text-xs font-bold uppercase tracking-wider text-ink-muted">{{ t('admin.ops.errorDetail.inboundEndpoint') }}</div>
          <div class="mt-1 break-all font-mono text-sm font-medium text-ink-strong dark:text-white">
        ...
        <div class="rounded-xl bg-surface-muted p-4 dark:bg-canvas">
          <div class="text-xs font-bold uppercase tracking-wider text-ink-muted">{{ t('admin.ops.errorDetail.upstreamEndpoint') }}</div>
          <div class="mt-1 break-all font-mono text-sm font-medium text-ink-strong dark:text-white">
        ...
        <div class="rounded-xl bg-surface-muted p-4 dark:bg-canvas">
          <div class="text-xs font-bold uppercase tracking-wider text-ink-muted">{{ t('admin.ops.errorDetail.status') }}</div>
          <div class="mt-1">
        ...
        <div class="rounded-xl bg-surface-muted p-4 dark:bg-canvas">
          <div class="text-xs font-bold uppercase tracking-wider text-ink-muted">{{ t('admin.ops.errorDetail.upstreamStatus') }}</div>
          <div class="mt-1">
        ...
        <div class="rounded-xl bg-surface-muted p-4 dark:bg-canvas">
          <div class="text-xs font-bold uppercase tracking-wider text-ink-muted">{{ t('admin.ops.errorDetail.requestType') }}</div>
          <div class="mt-1 text-sm font-medium text-ink-strong dark:text-white">
        ...
        <div class="rounded-xl bg-surface-muted p-4 dark:bg-canvas">
          <div class="text-xs font-bold uppercase tracking-wider text-ink-muted">{{ t('admin.ops.errorDetail.message') }}</div>
          <div class="mt-1 break-words text-sm font-medium text-ink-strong dark:text-white" :title="rootCauseMessage">
            {{ rootCauseMessage || '—' }}
          </div>
        </div>

        <div v-if="detail.api_key_prefix" class="rounded-xl bg-surface-muted p-4 dark:bg-canvas">
          <div class="text-xs font-bold uppercase tracking-wider text-ink-muted">{{ t('admin.ops.errorDetail.apiKeyPrefix') }}</div>
          <div class="mt-1 font-mono text-sm font-medium text-ink-strong dark:text-white">
      ...
      <div class="rounded-xl bg-surface-muted p-6 dark:bg-canvas">
        <h3 class="text-sm font-black uppercase tracking-wider text-ink-strong dark:text-white">{{ t('admin.ops.errorDetail.diagnosticPayloads') }}</h3>
        <div v-if="!diagnosticPayloadSections.length" class="mt-4 text-sm text-ink-muted dark:text-ink-muted">{{ t('common.noData') }}</div>
        <div v-else class="mt-4 space-y-4">
          <div v-for="section in diagnosticPayloadSections" :key="section.key">
            <div class="mb-2 text-xs font-bold uppercase tracking-wider text-ink-muted dark:text-ink-muted">{{ diagnosticPayloadLabel(section.key) }}</div>
            <pre class="max-h-[520px] overflow-auto rounded-xl border border-line bg-white p-4 text-xs text-ink-strong dark:border-line dark:bg-surface dark:text-gray-100"><code>{{ prettyJSON(section.value) }}</code></pre>
          </div>
        </div>
      </div>

      <!-- Upstream errors list (only for request errors) -->
      <div v-if="showUpstreamList" class="rounded-xl bg-surface-muted p-6 dark:bg-canvas">
        <div class="flex flex-wrap items-center justify-between gap-2">
          <h3 class="text-sm font-black uppercase tracking-wider text-ink-strong dark:text-white">{{ t('admin.ops.errorDetails.upstreamErrors') }}</h3>
          <div class="text-xs text-ink-muted dark:text-ink-muted" v-if="correlatedUpstreamLoading">{{ t('common.loading') }}</div>
        </div>

        <div v-if="!correlatedUpstreamLoading && !correlatedUpstreamErrors.length" class="mt-3 text-sm text-ink-muted dark:text-ink-muted">
          {{ t('common.noData') }}
        </div>
        ...
        <div v-else class="mt-4 space-y-3">
          <div
            ...
            :key="ev.id"
            class="rounded-xl border border-line bg-white p-4 dark:border-line dark:bg-surface"
          >
            <div class="flex flex-wrap items-center justify-between gap-2">
              <div class="text-xs font-black text-ink-strong dark:text-white">
                #{{ idx + 1 }}
                <span v-if="ev.type" class="ml-2 rounded-md bg-surface-muted px-2 py-0.5 font-mono text-[10px] font-bold text-ink dark:bg-surface-muted dark:text-gray-200">{{ ev.type }}</span>
              </div>
              <div class="flex items-center gap-2">
                <div class="font-mono text-xs text-ink-muted dark:text-ink-muted">
                  {{ ev.status_code ?? '—' }}
                </div>
                <button
                  type="button"
            ...
            <div class="mt-3 grid grid-cols-1 gap-2 text-xs text-ink dark:text-ink-muted sm:grid-cols-2">
              <div>
                <span class="text-ink-muted">{{ t('admin.ops.errorDetail.upstreamEvent.status') }}:</span>
                <span class="ml-1 font-mono">{{ ev.status_code ?? '—' }}</span>
              </div>
              <div>
                <span class="text-ink-muted">{{ t('admin.ops.errorDetail.upstreamEvent.requestId') }}:</span>
                <span class="ml-1 font-mono">{{ ev.request_id || ev.client_request_id || '—' }}</span>
              </div>
            </div>

            <div v-if="ev.message" class="mt-3 break-words text-sm font-medium text-ink-strong dark:text-white">{{ ev.message }}</div>

            <pre
              v-if="expandedUpstreamDetailIds.has(ev.id)"
              class="mt-3 max-h-[240px] overflow-auto rounded-xl border border-line bg-surface-muted p-3 text-xs text-ink-strong dark:border-line dark:bg-canvas dark:text-gray-100"
