...
<template>
  <div class="rounded-2xl border-2 border-line bg-white p-6 shadow-card dark:border-line dark:bg-surface">
    <div class="mb-4 flex flex-wrap items-start justify-between gap-3 sm:gap-4">
      <div>
        <h3 class="text-sm font-bold text-ink-strong dark:text-white">{{ t('admin.ops.alertRules.title') }}</h3>
        <p class="mt-1 text-xs text-ink-muted dark:text-ink-muted">{{ t('admin.ops.alertRules.description') }}</p>
      </div>
    ...
    <div v-if="loading" class="py-10 text-center text-sm text-ink-muted dark:text-ink-muted">
      {{ t('admin.ops.alertRules.loading') }}
    </div>

    <div v-else-if="sortedRules.length === 0" class="rounded-xl border border-dashed border-line p-8 text-center text-sm text-ink-muted dark:border-line dark:text-ink-muted">
      {{ t('admin.ops.alertRules.empty') }}
    </div>

    <div v-else class="max-h-[520px] overflow-hidden rounded-xl border border-line dark:border-line">
      <div class="max-h-[520px] overflow-y-auto">
        <div v-if="!isDesktopViewport" class="divide-y divide-line dark:divide-line">
          <div v-for="row in sortedRules" :key="row.id" class="space-y-2 p-4">
            <div class="flex items-start justify-between gap-2">
              <div class="min-w-0">
                <div class="text-xs font-bold text-ink-strong dark:text-white">{{ row.name }}</div>
                <div v-if="row.description" class="mt-0.5 line-clamp-2 text-[11px] text-ink-muted dark:text-ink-muted">
                  {{ row.description }}
                </div>
              </div>
              <span class="shrink-0 text-xs font-bold text-ink dark:text-gray-200">{{ row.severity }}</span>
            </div>
            <div class="text-xs text-ink dark:text-gray-200">
              <span class="font-mono">{{ row.metric_type }}</span>
              <span class="mx-1 text-ink-muted">{{ row.operator }}</span>
              <span class="font-mono">{{ row.threshold }}</span>
            </div>
            <div class="flex items-center justify-between gap-2">
              <span class="text-xs text-ink dark:text-gray-200">
            ...
            <div v-if="row.updated_at" class="text-[10px] text-ink-muted">
        ...
        <table v-else class="min-w-full divide-y divide-line dark:divide-line">
          ...
          <tbody class="divide-y divide-line bg-white dark:divide-line dark:bg-surface">
            <tr v-for="row in sortedRules" :key="row.id" class="hover:bg-surface-muted dark:hover:bg-dark-700/50">
              <td class="px-4 py-3">
                <div class="text-xs font-bold text-ink-strong dark:text-white">{{ row.name }}</div>
                <div v-if="row.description" class="mt-0.5 line-clamp-2 text-[11px] text-ink-muted dark:text-ink-muted">
                  {{ row.description }}
                </div>
                <div v-if="row.updated_at" class="mt-1 text-[10px] text-ink-muted">
                  {{ formatDateTime(row.updated_at) }}
                </div>
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs text-ink dark:text-gray-200">
                <span class="font-mono">{{ row.metric_type }}</span>
                <span class="mx-1 text-ink-muted">{{ row.operator }}</span>
                <span class="font-mono">{{ row.threshold }}</span>
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs font-bold text-ink dark:text-gray-200">
                {{ row.severity }}
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs text-ink dark:text-gray-200">
                {{ row.enabled ? t('common.enabled') : t('common.disabled') }}
      ...
      <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <div v-if="selectedMetricDefinition" class="mt-1 space-y-0.5 text-xs text-ink-muted dark:text-ink-muted">
          ...
          <div class="flex items-center justify-between rounded-xl bg-surface-muted px-4 py-3 dark:bg-surface/50 md:col-span-2">
            <span class="text-xs font-bold text-ink dark:text-gray-200">{{ t('admin.ops.alertRules.form.enabled') }}</span>
            <input v-model="draft!.enabled" type="checkbox" class="h-4 w-4 rounded border-line-strong text-primary-600 focus:ring-primary-500" />
          </div>

          <div class="flex items-center justify-between rounded-xl bg-surface-muted px-4 py-3 dark:bg-surface/50 md:col-span-2">
            <span class="text-xs font-bold text-ink dark:text-gray-200">{{ t('admin.ops.alertRules.form.notifyEmail') }}</span>
            <input v-model="draft!.notify_email" type="checkbox" class="h-4 w-4 rounded border-line-strong text-primary-600 focus:ring-primary-500" />
          </div>
        </div>
      </div>
