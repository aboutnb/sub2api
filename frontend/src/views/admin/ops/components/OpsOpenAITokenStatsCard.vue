...
<template>
  <section class="card p-4 md:p-5">
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
      <h3 class="text-sm font-bold text-ink-strong dark:text-white">
    ...
    <div v-if="loading" class="py-8 text-center text-sm text-ink-muted dark:text-ink-muted">
    ...
    <div v-else class="space-y-3">
        ...
        <div class="max-h-[420px] overflow-auto">
          <div v-if="!isDesktopViewport" class="divide-y divide-line dark:divide-line">
            <div v-for="row in items" :key="row.model" class="space-y-2 p-3">
              <div class="break-all text-xs font-medium text-ink-strong dark:text-gray-100">{{ row.model }}</div>
              <div class="grid grid-cols-2 gap-x-3 gap-y-1 text-xs">
                <div class="flex items-baseline justify-between gap-2">
                  <span class="text-ink-muted dark:text-ink-muted">{{ t('admin.ops.openaiTokenStats.table.requestCount') }}</span>
                  <span class="text-ink dark:text-gray-200">{{ formatInt(row.request_count) }}</span>
                </div>
                <div class="flex items-baseline justify-between gap-2">
                  <span class="text-ink-muted dark:text-ink-muted">{{ t('admin.ops.openaiTokenStats.table.avgTokensPerSec') }}</span>
                  <span class="text-ink dark:text-gray-200">{{ formatRate(row.avg_tokens_per_sec) }}</span>
                </div>
                <div class="flex items-baseline justify-between gap-2">
                  <span class="text-ink-muted dark:text-ink-muted">{{ t('admin.ops.openaiTokenStats.table.avgFirstTokenMs') }}</span>
                  <span class="text-ink dark:text-gray-200">{{ formatRate(row.avg_first_token_ms) }}</span>
                </div>
                <div class="flex items-baseline justify-between gap-2">
                  <span class="text-ink-muted dark:text-ink-muted">{{ t('admin.ops.openaiTokenStats.table.totalOutputTokens') }}</span>
                  <span class="text-ink dark:text-gray-200">{{ formatInt(row.total_output_tokens) }}</span>
                </div>
                <div class="flex items-baseline justify-between gap-2">
                  <span class="text-ink-muted dark:text-ink-muted">{{ t('admin.ops.openaiTokenStats.table.avgDurationMs') }}</span>
                  <span class="text-ink dark:text-gray-200">{{ formatInt(row.avg_duration_ms) }}</span>
                </div>
                <div class="flex items-baseline justify-between gap-2">
                  <span class="text-ink-muted dark:text-ink-muted">{{ t('admin.ops.openaiTokenStats.table.requestsWithFirstToken') }}</span>
                  <span class="text-ink dark:text-gray-200">{{ formatInt(row.requests_with_first_token) }}</span>
                </div>
              </div>
            </div>
          </div>
          <table v-else class="min-w-full text-left text-xs md:text-sm">
            <thead class="sticky top-0 z-10 bg-white dark:bg-surface">
            ...
            <tbody>
              <tr
                v-for="row in items"
                :key="row.model"
                class="border-b border-line text-ink last:border-b-0 dark:border-line dark:text-gray-200"
              >
      ...
      <div v-if="viewMode === 'topn'" class="mt-3 text-xs text-ink-muted dark:text-ink-muted">
