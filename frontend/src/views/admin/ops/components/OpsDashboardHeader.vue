...
const healthScoreClass = computed(() => {
  if (isSystemIdle.value) return 'text-ink-muted'
  const score = healthScoreValue.value
...
const cpuPercentClass = computed(() => {
  ...
  if (v == null) return 'text-ink-strong dark:text-white'
...
const dbMiddleClass = computed(() => {
  ...
  return 'text-ink-strong dark:text-white'
})
...
<template>
  <div :class="['flex flex-col gap-4 rounded-2xl border-2 border-line bg-white shadow-card dark:border-line dark:bg-surface', props.fullscreen ? 'p-8' : 'p-6']">
    <!-- Top Toolbar -->
    <div class="flex flex-wrap items-center justify-between gap-4 border-b border-line pb-4 dark:border-line">
      <div>
        <h1 class="flex items-center gap-2 text-xl font-black text-ink-strong dark:text-white">
        ...
        <div v-if="!props.fullscreen" class="mt-1 flex items-center gap-3 text-xs text-ink-muted dark:text-ink-muted">
      ...
      <div class="flex flex-wrap items-center gap-3">
        <template v-if="!props.fullscreen">
          ...
          <div class="mx-1 hidden h-4 w-[1px] bg-line dark:bg-surface-muted sm:block"></div>
        ...
        <button
          ...
          type="button"
          class="flex h-8 w-8 items-center justify-center rounded-lg bg-surface-muted text-ink-muted transition-colors hover:bg-line dark:bg-surface-muted dark:text-ink-muted dark:hover:bg-dark-600"
          :disabled="loading"
        ...
        <div v-if="!props.fullscreen" class="mx-1 hidden h-4 w-[1px] bg-line dark:bg-surface-muted sm:block"></div>
        ...
        <button
          v-if="!props.fullscreen"
          type="button"
          class="flex h-8 items-center gap-1.5 rounded-lg bg-surface-muted px-3 text-xs font-bold text-ink transition-colors hover:bg-line dark:bg-surface-muted dark:text-ink-muted dark:hover:bg-dark-600"
        ...
        <button
          v-if="!props.fullscreen"
          type="button"
          class="flex h-8 w-8 items-center justify-center rounded-lg bg-surface-muted text-ink transition-colors hover:bg-line dark:bg-surface-muted dark:text-ink-muted dark:hover:bg-dark-600"
    ...
    <div v-if="overview" class="grid grid-cols-1 gap-6 lg:grid-cols-12">
      ...
      <div :class="['rounded-2xl bg-surface-muted dark:bg-canvas lg:col-span-5', props.fullscreen ? 'p-6' : 'p-4']">
        <div class="grid h-full grid-cols-1 gap-6 md:grid-cols-[200px_1fr] md:items-center">
          <!-- 1) Health Score -->
          <div
            class="group relative flex cursor-pointer flex-col items-center justify-center rounded-xl py-2 transition-all hover:bg-white/60 dark:hover:bg-dark-800/60 md:border-r md:border-line md:pr-6 dark:md:border-line"
          >
            ...
            >
              <div class="rounded-xl bg-white p-4 shadow-xl ring-1 ring-black/5 dark:bg-surface dark:ring-white/10">
                <h4 class="mb-3 border-b border-line pb-2 text-sm font-bold text-ink-strong dark:border-line dark:text-white flex items-center gap-2">
                ...
                <div class="space-y-3">
                  <div v-for="(item, idx) in diagnosisReport" :key="idx" class="flex gap-3">
                    ...
                    <div class="flex-1">
                      <div class="text-xs font-semibold text-ink-strong dark:text-white">{{ item.message }}</div>
                      <div class="mt-0.5 text-[11px] text-ink-muted dark:text-ink-muted">{{ item.impact }}</div>
                ...
                <div class="mt-3 border-t border-line pt-2 text-[10px] text-ink-muted dark:border-line">
            ...
            <div class="relative flex items-center justify-center">
              ...
              <div class="absolute flex flex-col items-center">
                ...
                <span :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase tracking-wider text-ink-muted']">{{ t('admin.ops.health') }}</span>
              </div>
            </div>

            <div class="mt-4 text-center" v-if="!props.fullscreen">
              <div class="flex items-center justify-center gap-1 text-xs font-medium text-ink-muted">
          ...
          <div class="flex h-full flex-col justify-center py-2">
            <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
              <div class="flex items-center gap-2">
                ...
                <h3 class="text-xs font-bold uppercase tracking-wider text-ink-muted">{{ t('admin.ops.realtime.title') }}</h3>
            ...
            <div :class="props.fullscreen ? 'space-y-4' : 'space-y-3'">
              <!-- Row 1: Current -->
              <div>
                <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase text-ink-muted']">{{ t('admin.ops.current') }}</div>
                <div class="mt-1 flex flex-wrap items-baseline gap-x-4 gap-y-2">
                  <div class="flex items-baseline gap-1.5">
                    <span :class="[props.fullscreen ? 'text-4xl' : 'text-xl sm:text-2xl', 'font-black text-ink-strong dark:text-white']">{{ displayRealTimeQps.toFixed(1) }}</span>
                    <span :class="[props.fullscreen ? 'text-sm' : 'text-xs', 'font-bold text-ink-muted']">QPS</span>
                  </div>
                  <div class="flex items-baseline gap-1.5">
                    <span :class="[props.fullscreen ? 'text-4xl' : 'text-xl sm:text-2xl', 'font-black text-ink-strong dark:text-white']">{{ displayRealTimeTps.toFixed(1) }}</span>
                    <span :class="[props.fullscreen ? 'text-sm' : 'text-xs', 'font-bold text-ink-muted']">{{ t('admin.ops.tps') }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="grid grid-cols-2 gap-3">
                <!-- Peak -->
                <div>
                  <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase text-ink-muted']">{{ t('admin.ops.peak') }}</div>
                  <div :class="[props.fullscreen ? 'text-base' : 'text-sm', 'mt-1 space-y-0.5 font-medium text-ink dark:text-ink-muted']">
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-ink-strong dark:text-white">{{ realtimeQpsPeakLabel }}</span>
                      <span class="text-xs">QPS</span>
                    </div>
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-ink-strong dark:text-white">{{ realtimeTpsPeakLabel }}</span>
                ...
                <!-- Average -->
                <div>
                  <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase text-ink-muted']">{{ t('admin.ops.average') }}</div>
                  <div :class="[props.fullscreen ? 'text-base' : 'text-sm', 'mt-1 space-y-0.5 font-medium text-ink dark:text-ink-muted']">
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-ink-strong dark:text-white">{{ realtimeQpsAvgLabel }}</span>
                      <span class="text-xs">QPS</span>
                    </div>
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-ink-strong dark:text-white">{{ realtimeTpsAvgLabel }}</span>
                      <span class="text-xs">{{ t('admin.ops.tps') }}</span>
                    </div>
                  </div>
                </div>
      ...
      <div class="grid h-full grid-cols-1 content-center gap-4 sm:grid-cols-2 lg:col-span-7 lg:grid-cols-3">
        <!-- Card 1: Requests -->
        <div class="rounded-2xl bg-surface-muted p-4 dark:bg-canvas" style="order: 1;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-ink-muted">{{ t('admin.ops.requestsTitle') }}</span>
          ...
          <div class="mt-2 space-y-2 text-xs">
            <div class="flex justify-between">
              <span class="text-ink-muted">{{ t('admin.ops.requests') }}:</span>
              <span class="font-bold text-ink-strong dark:text-white">{{ totalRequestsLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-ink-muted">{{ t('admin.ops.tokens') }}:</span>
              <span class="font-bold text-ink-strong dark:text-white">{{ totalTokensLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-ink-muted">{{ t('admin.ops.avgQps') }}:</span>
              <span class="font-bold text-ink-strong dark:text-white">{{ qpsAvgLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-ink-muted">{{ t('admin.ops.avgTps') }}:</span>
              <span class="font-bold text-ink-strong dark:text-white">{{ tpsAvgLabel }}</span>
            </div>
          </div>
        </div>

        <!-- Card 2: SLA -->
        <div class="rounded-2xl bg-surface-muted p-4 dark:bg-canvas" style="order: 2;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              <span class="text-[10px] font-bold uppercase text-ink-muted">{{ t('admin.ops.sla') }}</span>
          ...
          <div class="mt-3 h-2 w-full overflow-hidden rounded-full bg-line dark:bg-surface-muted">
          ...
          <div class="mt-3 text-xs">
            <div class="flex justify-between">
              <span class="text-ink-muted">{{ t('admin.ops.exceptions') }}:</span>
              <span class="font-bold text-ink-strong dark:text-white">{{ formatNumber((overview.request_count_sla ?? 0) - (overview.success_count ?? 0)) }}</span>
            </div>
          </div>
        </div>

        <!-- Card 4: Request Duration -->
        <div class="rounded-2xl bg-surface-muted p-4 dark:bg-canvas" style="order: 4;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-ink-muted">{{ t('admin.ops.latencyDuration') }}</span>
          ...
          <div class="mt-2 flex items-baseline gap-2">
            <div class="text-3xl font-black text-ink-strong dark:text-white">
              {{ durationP99Ms ?? '-' }}
            </div>
            <span class="text-xs font-bold text-ink-muted">ms (P99)</span>
          </div>
          <div class="mt-3 grid grid-cols-1 gap-x-3 gap-y-1 text-xs 2xl:grid-cols-2">
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-ink-muted">P95:</span>
              <span class="font-bold text-ink-strong dark:text-white">{{ durationP95Ms ?? '-' }}</span>
              <span class="text-ink-muted">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-ink-muted">P90:</span>
              <span class="font-bold text-ink-strong dark:text-white">{{ durationP90Ms ?? '-' }}</span>
              <span class="text-ink-muted">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-ink-muted">P50:</span>
              <span class="font-bold text-ink-strong dark:text-white">{{ durationP50Ms ?? '-' }}</span>
              <span class="text-ink-muted">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-ink-muted">Avg:</span>
              <span class="font-bold text-ink-strong dark:text-white">{{ durationAvgMs ?? '-' }}</span>
              <span class="text-ink-muted">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-ink-muted">Max:</span>
              <span class="font-bold text-ink-strong dark:text-white">{{ durationMaxMs ?? '-' }}</span>
              <span class="text-ink-muted">ms</span>
            </div>
          </div>
        </div>
        ...
        <!-- Card 5: TTFT -->
        <div class="rounded-2xl bg-surface-muted p-4 dark:bg-canvas" style="order: 5;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-ink-muted">TTFT</span>
        ...
        <div class="rounded-2xl bg-surface-muted p-4 dark:bg-canvas" style="order: 3;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-ink-muted">{{ t('admin.ops.requestErrors') }}</span>
          ...
          <div class="mt-3 space-y-1 text-xs">
            <div class="flex justify-between">
              <span class="text-ink-muted">{{ t('admin.ops.errorCount') }}:</span>
              <span class="font-bold text-ink-strong dark:text-white">{{ formatNumber(overview.error_count_sla ?? 0) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-ink-muted">{{ t('admin.ops.businessLimited') }}:</span>
              <span class="font-bold text-ink-strong dark:text-white">{{ formatNumber(overview.business_limited_count ?? 0) }}</span>
            </div>
          </div>
        </div>

        <!-- Card 6: Upstream Errors -->
        <div class="rounded-2xl bg-surface-muted p-4 dark:bg-canvas" style="order: 6;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-ink-muted">{{ t('admin.ops.upstreamErrors') }}</span>
          ...
          <div class="mt-3 space-y-1 text-xs">
            <div class="flex justify-between">
              <span class="text-ink-muted">{{ t('admin.ops.errorCountExcl429529') }}:</span>
              <span class="font-bold text-ink-strong dark:text-white">{{ formatNumber(overview.upstream_error_count_excl_429_529 ?? 0) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-ink-muted">429/529:</span>
              <span class="font-bold text-ink-strong dark:text-white">{{ formatNumber((overview.upstream_429_count ?? 0) + (overview.upstream_529_count ?? 0)) }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-if="overview" class="mt-2 border-t border-line pt-4 dark:border-line">
      <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        <!-- CPU -->
        <div class="rounded-xl bg-surface-muted p-3 dark:bg-canvas">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-ink-muted">CPU</div>
          ...
          <div v-if="!props.fullscreen" class="mt-1 text-[10px] text-ink-muted dark:text-ink-muted">
        ...
        <div class="rounded-xl bg-surface-muted p-3 dark:bg-canvas">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-ink-muted">{{ t('admin.ops.memory') }}</div>
        ...
        <div class="rounded-xl bg-surface-muted p-3 dark:bg-canvas">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-ink-muted">{{ t('admin.ops.db') }}</div>
        ...
        <div class="rounded-xl bg-surface-muted p-3 dark:bg-canvas">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-ink-muted">Redis</div>
        ...
        <div class="rounded-xl bg-surface-muted p-3 dark:bg-canvas">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-ink-muted">{{ t('admin.ops.goroutines') }}</div>
        ...
        <div class="rounded-xl bg-surface-muted p-3 dark:bg-canvas">
          <div class="flex items-center justify-between gap-2">
            <div class="flex items-center gap-1">
              <div class="text-[10px] font-bold uppercase tracking-wider text-ink-muted">{{ t('admin.ops.jobs') }}</div>
    ...
    <BaseDialog :show="showJobsDetails" :title="t('admin.ops.jobs')" width="wide" @close="showJobsDetails = false">
      <div v-if="!jobHeartbeats.length" class="text-sm text-ink-muted dark:text-ink-muted">
        {{ t('admin.ops.noData') }}
      </div>
      <div v-else class="space-y-3">
        <div
          ...
          :key="hb.job_name"
          class="rounded-xl border border-line bg-white p-4 dark:border-line dark:bg-canvas"
        >
          <div class="flex items-center justify-between gap-3">
            <div class="truncate text-sm font-semibold text-ink-strong dark:text-white">{{ hb.job_name }}</div>
            <div class="flex items-center gap-3 text-xs text-ink-muted dark:text-ink-muted">
              <span v-if="hb.last_duration_ms != null" class="font-mono">{{ hb.last_duration_ms }}ms</span>
              <span>{{ formatTimeShort(hb.updated_at) }}</span>
            </div>
          </div>

          <div class="mt-2 grid grid-cols-1 gap-2 text-xs text-ink dark:text-ink-muted sm:grid-cols-2">
            <div>
    ...
    <BaseDialog :show="showCustomTimeRangeDialog" :title="t('admin.ops.timeRange.custom')" width="narrow" @close="handleCustomTimeRangeCancel">
      <div class="space-y-4">
        <div>
          <label class="block text-sm font-medium text-ink dark:text-ink-muted mb-1">
            {{ t('admin.ops.customTimeRange.startTime') }}
          </label>
          <input
            v-model="customStartTimeInput"
            type="datetime-local"
            class="w-full rounded-lg border border-line-strong bg-white px-3 py-2 text-sm text-ink-strong focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-line-strong dark:bg-surface dark:text-white"
          />
        </div>
        <div>
          <label class="block text-sm font-medium text-ink dark:text-ink-muted mb-1">
        ...
        <div class="flex justify-end gap-3 pt-2">
          <button
            type="button"
            class="rounded-lg bg-surface-muted px-4 py-2 text-sm font-medium text-ink hover:bg-line dark:bg-surface-muted dark:text-ink-muted dark:hover:bg-dark-600"
