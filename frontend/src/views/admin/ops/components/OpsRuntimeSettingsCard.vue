...
<template>
  <div class="rounded-2xl border-2 border-line bg-white p-6 shadow-card dark:border-line dark:bg-surface">
    <div class="mb-4 flex items-start justify-between gap-4">
      <div>
        <h3 class="text-sm font-bold text-ink-strong dark:text-white">{{ t('admin.ops.runtime.title') }}</h3>
        <p class="mt-1 text-xs text-ink-muted dark:text-ink-muted">{{ t('admin.ops.runtime.description') }}</p>
      </div>
      <button
    ...
    <div v-if="!alertSettings" class="text-sm text-ink-muted dark:text-ink-muted">
      <span v-if="loading">{{ t('admin.ops.runtime.loading') }}</span>
      <span v-else>{{ t('admin.ops.runtime.noData') }}</span>
    </div>

    <div v-else class="space-y-6">
      <div class="rounded-2xl bg-surface-muted p-4 dark:bg-surface-muted/50">
        <div class="mb-3 flex items-center justify-between">
          <h4 class="text-sm font-semibold text-ink-strong dark:text-white">{{ t('admin.ops.runtime.alertTitle') }}</h4>
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div class="text-xs text-ink dark:text-ink-muted">
            ...
            <span class="ml-1 font-medium text-ink-strong dark:text-white">{{ alertSettings.evaluation_interval_seconds }}s</span>
          </div>
          <div
            v-if="alertSettings.silencing?.enabled && alertSettings.silencing.global_until_rfc3339"
            class="text-xs text-ink dark:text-ink-muted md:col-span-2"
          >
            {{ t('admin.ops.runtime.silencing.globalUntil') }}:
            <span class="ml-1 font-mono text-ink-strong dark:text-white">{{ alertSettings.silencing.global_until_rfc3339 }}</span>
          </div>
          ...
          <details class="col-span-1 md:col-span-2">
            ...
            <div class="mt-2 grid grid-cols-1 gap-3 rounded-lg bg-surface-muted p-3 dark:bg-surface md:grid-cols-2">
              <div class="text-xs text-ink-muted dark:text-ink-muted">
                {{ t('admin.ops.runtime.lockEnabled') }}:
                <span class="ml-1 font-mono text-ink dark:text-ink-muted">{{ alertSettings.distributed_lock.enabled }}</span>
              </div>
              <div class="text-xs text-ink-muted dark:text-ink-muted">
                {{ t('admin.ops.runtime.lockKey') }}:
                <span class="ml-1 font-mono text-ink dark:text-ink-muted">{{ alertSettings.distributed_lock.key }}</span>
              </div>
              <div class="text-xs text-ink-muted dark:text-ink-muted">
                {{ t('admin.ops.runtime.lockTTLSeconds') }}:
                <span class="ml-1 font-mono text-ink dark:text-ink-muted">{{ alertSettings.distributed_lock.ttl_seconds }}s</span>
              </div>
            </div>
          </details>
        </div>
  ...
  <BaseDialog :show="showAlertEditor" :title="t('admin.ops.runtime.alertTitle')" width="extra-wide" @close="showAlertEditor = false">
    <div v-if="draftAlert" class="space-y-4">
      ...
      <div>
        <div class="mb-1 text-xs font-medium text-ink dark:text-ink-muted">{{ t('admin.ops.runtime.evalIntervalSeconds') }}</div>
        <input
        ...
        />
        <p class="mt-1 text-xs text-ink-muted">{{ t('admin.ops.runtime.evalIntervalHint') }}</p>
      </div>

      <div class="rounded-2xl bg-surface-muted p-4 dark:bg-surface-muted/50">
        <div class="mb-2 text-sm font-semibold text-ink-strong dark:text-white">{{ t('admin.ops.runtime.metricThresholds') }}</div>
        <p class="mb-4 text-xs text-ink-muted dark:text-ink-muted">{{ t('admin.ops.runtime.metricThresholdsHint') }}</p>

        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <div class="mb-1 text-xs font-medium text-ink dark:text-ink-muted">{{ t('admin.ops.runtime.slaMinPercent') }}</div>
            <input
              ...
              placeholder="99.5"
            />
            <p class="mt-1 text-xs text-ink-muted dark:text-ink-muted">{{ t('admin.ops.runtime.slaMinPercentHint') }}</p>
          </div>



          <div>
            <div class="mb-1 text-xs font-medium text-ink dark:text-ink-muted">{{ t('admin.ops.runtime.ttftP99MaxMs') }}</div>
            <input
              ...
              placeholder="500"
            />
            <p class="mt-1 text-xs text-ink-muted dark:text-ink-muted">{{ t('admin.ops.runtime.ttftP99MaxMsHint') }}</p>
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-ink dark:text-ink-muted">{{ t('admin.ops.runtime.requestErrorRateMaxPercent') }}</div>
            <input
              ...
              placeholder="5"
            />
            <p class="mt-1 text-xs text-ink-muted dark:text-ink-muted">{{ t('admin.ops.runtime.requestErrorRateMaxPercentHint') }}</p>
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-ink dark:text-ink-muted">{{ t('admin.ops.runtime.upstreamErrorRateMaxPercent') }}</div>
            <input
              ...
              placeholder="5"
            />
            <p class="mt-1 text-xs text-ink-muted dark:text-ink-muted">{{ t('admin.ops.runtime.upstreamErrorRateMaxPercentHint') }}</p>
          </div>
        </div>
      </div>

      <div class="rounded-2xl bg-surface-muted p-4 dark:bg-surface-muted/50">
        <div class="mb-2 text-sm font-semibold text-ink-strong dark:text-white">{{ t('admin.ops.runtime.silencing.title') }}</div>

        <label class="inline-flex items-center gap-2 text-sm text-ink dark:text-ink-muted">
          <input v-model="draftAlert.silencing.enabled" type="checkbox" class="h-4 w-4 rounded border-line-strong" />
          <span>{{ t('admin.ops.runtime.silencing.enabled') }}</span>
        </label>

        <div v-if="draftAlert.silencing.enabled" class="mt-4 space-y-4">
          <div>
            <div class="mb-1 text-xs font-medium text-ink dark:text-ink-muted">{{ t('admin.ops.runtime.silencing.globalUntil') }}</div>
            <input
            ...
            />
            <p class="mt-1 text-xs text-ink-muted dark:text-ink-muted">{{ t('admin.ops.runtime.silencing.untilHint') }}</p>
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-ink dark:text-ink-muted">{{ t('admin.ops.runtime.silencing.reason') }}</div>
            <input
          ...
          <div class="rounded-xl border border-line bg-white p-4 dark:border-line dark:bg-surface">
            <div class="flex items-start justify-between gap-4">
              <div>
                <div class="text-xs font-bold text-ink-strong dark:text-white">{{ t('admin.ops.runtime.silencing.entries.title') }}</div>
                <p class="text-[11px] text-ink-muted dark:text-ink-muted">{{ t('admin.ops.runtime.silencing.entries.hint') }}</p>
              </div>
            ...
            <div v-if="!draftAlert.silencing.entries?.length" class="mt-3 rounded-lg bg-surface-muted p-3 text-xs text-ink-muted dark:bg-canvas dark:text-ink-muted">
            ...
            <div v-else class="mt-4 space-y-4">
              <div
                ...
                :key="idx"
                class="rounded-lg border border-line bg-surface-muted p-4 dark:border-line dark:bg-canvas"
              >
                <div class="mb-3 flex items-center justify-between">
                  <div class="text-xs font-bold text-ink-strong dark:text-white">
                ...
                <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                  <div>
                    <div class="mb-1 text-xs font-medium text-ink dark:text-ink-muted">{{ t('admin.ops.runtime.silencing.entries.ruleId') }}</div>
                    <input
                  ...
                  <div>
                    <div class="mb-1 text-xs font-medium text-ink dark:text-ink-muted">{{ t('admin.ops.runtime.silencing.entries.severities') }}</div>
                    <input
                  ...
                  <div>
                    <div class="mb-1 text-xs font-medium text-ink dark:text-ink-muted">{{ t('admin.ops.runtime.silencing.entries.until') }}</div>
                    <input
              ...
              placeholder="2026-01-05T00:00:00Z"
                  ...
                  <div>
                    <div class="mb-1 text-xs font-medium text-ink dark:text-ink-muted">{{ t('admin.ops.runtime.silencing.entries.reason') }}</div>
                    <input
      ...
      <details class="rounded-lg border border-line bg-surface-muted p-3 dark:border-line-strong dark:bg-surface">
        <summary class="cursor-pointer text-xs font-medium text-ink dark:text-ink-muted">{{ t('admin.ops.runtime.advancedSettingsSummary') }}</summary>
        <div class="mt-3 grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <label class="inline-flex items-center gap-2 text-xs text-ink dark:text-ink-muted">
              <input v-model="draftAlert.distributed_lock.enabled" type="checkbox" class="h-4 w-4 rounded border-line-strong" />
              <span>{{ t('admin.ops.runtime.lockEnabled') }}</span>
            </label>
          </div>
          <div class="md:col-span-2">
            <div class="mb-1 text-xs font-medium text-ink-muted">{{ t('admin.ops.runtime.lockKey') }}</div>
            <input v-model="draftAlert.distributed_lock.key" type="text" class="input text-xs font-mono" />
            <p v-if="draftAlert.distributed_lock.enabled" class="mt-1 text-[11px] text-ink-muted dark:text-ink-muted">
              {{ t('admin.ops.runtime.validation.lockKeyHint', { prefix: 'ops:' }) }}
            </p>
          </div>
          <div>
            <div class="mb-1 text-xs font-medium text-ink-muted">{{ t('admin.ops.runtime.lockTTLSeconds') }}</div>
