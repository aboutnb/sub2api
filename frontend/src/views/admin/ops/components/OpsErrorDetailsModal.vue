...
<template>
  <BaseDialog :show="show" :title="modalTitle" width="full" @close="close">
    <div class="flex h-full min-h-0 flex-col">
      <!-- Filters -->
      <div class="mb-4 flex-shrink-0 border-b border-line pb-4 dark:border-line">
        <div class="grid grid-cols-2 gap-2 md:grid-cols-8">
          <div class="col-span-2 compact-select">
            <div class="relative group">
              <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                <svg
                  class="h-3.5 w-3.5 text-ink-muted transition-colors group-focus-within:text-blue-500"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
              ...
              <input
                v-model="q"
                type="text"
                class="w-full rounded-lg border-line bg-surface-muted/50 py-1.5 pl-9 pr-3 text-xs font-medium text-ink transition-all focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-500/10 dark:border-line dark:bg-canvas dark:text-ink-muted dark:focus:bg-dark-800"
          ...
          <div class="flex items-center justify-end">
            <button type="button" class="rounded-lg bg-surface-muted px-3 py-1.5 text-xs font-semibold text-ink transition-colors hover:bg-line dark:bg-surface-muted dark:text-ink-muted dark:hover:bg-dark-600" @click="resetFilters">
              {{ t('common.reset') }}
            </button>
          </div>
        </div>
      ...
      <div class="flex min-h-0 flex-1 flex-col">
        <div class="mb-2 flex-shrink-0 text-xs text-ink-muted dark:text-ink-muted">
