...
<template>
  <div class="rounded-2xl border-2 border-line bg-white p-6 shadow-card dark:border-line dark:bg-surface">
    <div class="mb-4 flex items-start justify-between gap-4">
      <div>
        <h3 class="text-sm font-bold text-ink-strong dark:text-white">{{ t('admin.ops.email.title') }}</h3>
        <p class="mt-1 text-xs text-ink-muted dark:text-ink-muted">{{ t('admin.ops.email.description') }}</p>
      </div>
    ...
    <div v-if="!config" class="text-sm text-ink-muted dark:text-ink-muted">
    ...
    <div v-else class="space-y-6">
      <div class="rounded-2xl bg-surface-muted p-4 dark:bg-surface-muted/50">
        <h4 class="mb-2 text-sm font-semibold text-ink-strong dark:text-white">{{ t('admin.ops.email.alertTitle') }}</h4>
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div class="text-xs text-ink dark:text-ink-muted">
            {{ t('common.enabled') }}:
            <span class="ml-1 font-medium text-ink-strong dark:text-white">
              {{ config.alert.enabled ? t('common.enabled') : t('common.disabled') }}
            </span>
          </div>
          <div class="text-xs text-ink dark:text-ink-muted">
            {{ t('admin.ops.email.recipients') }}:
            <span class="ml-1 font-medium text-ink-strong dark:text-white">{{ config.alert.recipients.length }}</span>
          </div>
          <div class="text-xs text-ink dark:text-ink-muted">
          ...
          <div class="text-xs text-ink dark:text-ink-muted">
            ...
            <span class="ml-1 font-medium text-ink-strong dark:text-white">{{ config.alert.rate_limit_per_hour }}</span>
          </div>
        </div>
      </div>

      <div class="rounded-2xl bg-surface-muted p-4 dark:bg-surface-muted/50">
        <h4 class="mb-2 text-sm font-semibold text-ink-strong dark:text-white">{{ t('admin.ops.email.reportTitle') }}</h4>
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div class="text-xs text-ink dark:text-ink-muted">
            {{ t('common.enabled') }}:
            <span class="ml-1 font-medium text-ink-strong dark:text-white">
          ...
          <div class="text-xs text-ink dark:text-ink-muted">
            {{ t('admin.ops.email.recipients') }}:
            <span class="ml-1 font-medium text-ink-strong dark:text-white">{{ config.report.recipients.length }}</span>
          </div>
        </div>
      </div>
    </div>
  ...
  <BaseDialog :show="showEditor" :title="t('admin.ops.email.title')" width="extra-wide" @close="showEditor = false">
    <div v-if="draft" class="space-y-6">
      ...
      <div class="rounded-2xl bg-surface-muted p-4 dark:bg-surface-muted/50">
        <h4 class="mb-3 text-sm font-semibold text-ink-strong dark:text-white">{{ t('admin.ops.email.alertTitle') }}</h4>
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <div class="mb-1 text-xs font-medium text-ink dark:text-ink-muted">{{ t('common.enabled') }}</div>
            <label class="inline-flex items-center gap-2 text-sm text-ink dark:text-ink-muted">
              <input v-model="draft.alert.enabled" type="checkbox" class="h-4 w-4 rounded border-line-strong" />
              <span>{{ draft.alert.enabled ? t('common.enabled') : t('common.disabled') }}</span>
            </label>
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-ink dark:text-ink-muted">{{ t('admin.ops.email.minSeverity') }}</div>
            <Select v-model="draft.alert.min_severity" :options="severityOptions" />
          </div>

          <div class="md:col-span-2">
            <div class="mb-1 text-xs font-medium text-ink dark:text-ink-muted">{{ t('admin.ops.email.recipients') }}</div>
            ...
            <div class="mt-1 text-xs text-ink-muted dark:text-ink-muted">{{ t('admin.ops.email.recipientsHint') }}</div>
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-ink dark:text-ink-muted">{{ t('admin.ops.email.rateLimitPerHour') }}</div>
            <input v-model.number="draft.alert.rate_limit_per_hour" type="number" min="0" max="100000" class="input" />
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-ink dark:text-ink-muted">{{ t('admin.ops.email.batchWindowSeconds') }}</div>
            <input v-model.number="draft.alert.batching_window_seconds" type="number" min="0" max="86400" class="input" />
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-ink dark:text-ink-muted">{{ t('admin.ops.email.includeResolved') }}</div>
            <label class="inline-flex items-center gap-2 text-sm text-ink dark:text-ink-muted">
              <input v-model="draft.alert.include_resolved_alerts" type="checkbox" class="h-4 w-4 rounded border-line-strong" />
      ...
      <div class="rounded-2xl bg-surface-muted p-4 dark:bg-surface-muted/50">
        <h4 class="mb-3 text-sm font-semibold text-ink-strong dark:text-white">{{ t('admin.ops.email.reportTitle') }}</h4>
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <div class="mb-1 text-xs font-medium text-ink dark:text-ink-muted">{{ t('common.enabled') }}</div>
            <label class="inline-flex items-center gap-2 text-sm text-ink dark:text-ink-muted">
              <input v-model="draft.report.enabled" type="checkbox" class="h-4 w-4 rounded border-line-strong" />
          ...
          <div class="md:col-span-2">
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                <div class="mb-1 text-xs font-medium text-ink dark:text-ink-muted">{{ t('admin.ops.email.dailySummary') }}</div>
                <div class="flex items-center gap-2">
                  <label class="inline-flex items-center gap-2 text-sm text-ink dark:text-ink-muted">
                    <input v-model="draft.report.daily_summary_enabled" type="checkbox" class="h-4 w-4 rounded border-line-strong" />
                  </label>
                  <input v-model="draft.report.daily_summary_schedule" type="text" class="input" :placeholder="t('admin.ops.email.cronPlaceholder')" />
                </div>
              </div>
              <div>
                <div class="mb-1 text-xs font-medium text-ink dark:text-ink-muted">{{ t('admin.ops.email.weeklySummary') }}</div>
                <div class="flex items-center gap-2">
                  <label class="inline-flex items-center gap-2 text-sm text-ink dark:text-ink-muted">
                    <input v-model="draft.report.weekly_summary_enabled" type="checkbox" class="h-4 w-4 rounded border-line-strong" />
                  </label>
                  <input v-model="draft.report.weekly_summary_schedule" type="text" class="input" :placeholder="t('admin.ops.email.cronPlaceholder')" />
                </div>
              </div>
              <div>
                <div class="mb-1 text-xs font-medium text-ink dark:text-ink-muted">{{ t('admin.ops.email.errorDigest') }}</div>
                <div class="flex items-center gap-2">
                  <label class="inline-flex items-center gap-2 text-sm text-ink dark:text-ink-muted">
                    <input v-model="draft.report.error_digest_enabled" type="checkbox" class="h-4 w-4 rounded border-line-strong" />
                  </label>
                  <input v-model="draft.report.error_digest_schedule" type="text" class="input" :placeholder="t('admin.ops.email.cronPlaceholder')" />
                </div>
              </div>
              <div>
                <div class="mb-1 text-xs font-medium text-ink dark:text-ink-muted">{{ t('admin.ops.email.errorDigestMinCount') }}</div>
                <input v-model.number="draft.report.error_digest_min_count" type="number" min="0" max="1000000" class="input" />
              </div>
              <div>
                <div class="mb-1 text-xs font-medium text-ink dark:text-ink-muted">{{ t('admin.ops.email.accountHealth') }}</div>
                <div class="flex items-center gap-2">
                  <label class="inline-flex items-center gap-2 text-sm text-ink dark:text-ink-muted">
                    <input v-model="draft.report.account_health_enabled" type="checkbox" class="h-4 w-4 rounded border-line-strong" />
                  </label>
                  <input v-model="draft.report.account_health_schedule" type="text" class="input" :placeholder="t('admin.ops.email.cronPlaceholder')" />
                </div>
              </div>
              <div>
                <div class="mb-1 text-xs font-medium text-ink dark:text-ink-muted">{{ t('admin.ops.email.accountHealthThreshold') }}</div>
                <input v-model.number="draft.report.account_health_error_rate_threshold" type="number" min="0" max="100" step="0.1" class="input" />
              </div>
            </div>
            <div class="mt-2 text-xs text-ink-muted dark:text-ink-muted">{{ t('admin.ops.email.reportHint') }}</div>
          </div>
        </div>
      </div>
    </div>
