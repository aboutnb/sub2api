...
function severityBadgeClass(severity: string | undefined): string {
  ...
  if (s === 'p3') return 'bg-surface-muted text-ink dark:bg-surface-muted dark:text-ink-muted'
...
function statusBadgeClass(status: string | undefined): string {
  ...
  return 'bg-surface-muted text-ink ring-gray-600/20 dark:bg-canvas/30 dark:text-ink-muted dark:ring-line-strong/30'
}
...
<template>
  <div class="rounded-2xl border-2 border-line bg-white p-6 shadow-card dark:border-line dark:bg-surface">
    <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
      <div>
        <h3 class="text-sm font-bold text-ink-strong dark:text-white">{{ t('admin.ops.alertEvents.title') }}</h3>
        <p class="mt-1 text-xs text-ink-muted dark:text-ink-muted">{{ t('admin.ops.alertEvents.description') }}</p>
      </div>
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <button
          class="flex items-center gap-1.5 rounded-lg bg-surface-muted px-3 py-1.5 text-xs font-bold text-ink transition-colors hover:bg-line disabled:cursor-not-allowed disabled:opacity-50 dark:bg-surface-muted dark:text-ink-muted dark:hover:bg-dark-600"
          :disabled="loading"
          @click="loadFirstPage"
        >
    ...
    <div v-if="loading" class="flex items-center gap-2 text-sm text-ink-muted dark:text-ink-muted">
    ...
    <div v-else-if="empty" class="rounded-xl border border-dashed border-line p-8 text-center text-sm text-ink-muted dark:border-line dark:text-ink-muted">
      {{ t('admin.ops.alertEvents.empty') }}
    </div>

    <div v-else class="overflow-hidden rounded-xl border border-line dark:border-line">
      <div class="max-h-[600px] overflow-y-auto" @scroll="onScroll">
        <div v-if="!isDesktopViewport" class="divide-y divide-line dark:divide-line">
          <div
            v-for="row in events"
            :key="row.id"
            class="cursor-pointer space-y-2 p-4 hover:bg-surface-muted dark:hover:bg-dark-700/50"
          ...
          >
            <div class="flex flex-wrap items-center gap-2">
              ...
              <span class="ml-auto text-[11px] text-ink-muted dark:text-ink-muted">
                {{ formatDateTime(row.fired_at || row.created_at) }}
              </span>
            </div>
            <div class="text-xs font-semibold text-ink-strong dark:text-white">{{ row.title || '-' }}</div>
            <div v-if="row.description" class="line-clamp-2 text-[11px] text-ink-muted dark:text-ink-muted">
              {{ row.description }}
            </div>
            <div class="flex flex-wrap items-center justify-between gap-2 text-[11px] text-ink-muted dark:text-ink-muted">
            ...
            <div class="text-[11px] text-ink-muted dark:text-ink-muted">{{ formatDimensionsSummary(row) }}</div>
          </div>
        </div>
        <table v-else class="min-w-full divide-y divide-line dark:divide-line">
          <thead class="sticky top-0 z-10 bg-surface-muted dark:bg-canvas">
            <tr>
              ...
              <th class="px-4 py-3 text-right text-[11px] font-bold uppercase tracking-wider text-ink-muted dark:text-ink-muted">
          ...
          <tbody class="divide-y divide-line bg-white dark:divide-line dark:bg-surface">
            <tr
              v-for="row in events"
              :key="row.id"
              class="cursor-pointer hover:bg-surface-muted dark:hover:bg-dark-700/50"
              @click="openDetail(row)"
              :title="row.title || ''"
            >
              ...
              <td class="min-w-[260px] px-4 py-3 text-xs text-ink dark:text-gray-200">
                <div class="font-semibold truncate max-w-[360px]">{{ row.title || '-' }}</div>
                <div v-if="row.description" class="mt-0.5 line-clamp-2 text-[11px] text-ink-muted dark:text-ink-muted">
                  {{ row.description }}
                </div>
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs text-ink dark:text-ink-muted">
              ...
              <td class="whitespace-nowrap px-4 py-3 text-[11px] text-ink-muted dark:text-ink-muted">
              ...
              <td class="whitespace-nowrap px-4 py-3 text-right text-xs">
                ...
                >
                  ...
                  <Icon
                    ...
                    class="text-ink-muted dark:text-ink-muted"
                  />
                  <span class="text-[11px] font-bold text-ink dark:text-ink-muted">
                    {{ row.email_sent ? t('admin.ops.alertEvents.table.emailSent') : t('admin.ops.alertEvents.table.emailIgnored') }}
                  </span>
                </span>
              </td>
        ...
        <div v-if="loadingMore" class="flex items-center justify-center gap-2 py-3 text-xs text-ink-muted dark:text-ink-muted">
          <svg class="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
          </svg>
        ...
        <div v-else-if="!hasMore && events.length > 0" class="py-3 text-center text-xs text-ink-muted">
          -
        </div>
      </div>
    </div>
    ...
    <BaseDialog
      ...
      @close="closeDetail"
    >
      <div v-if="detailLoading" class="flex items-center justify-center py-10 text-sm text-ink-muted dark:text-ink-muted">
        {{ t('admin.ops.alertEvents.detail.loading') }}
      </div>

      <div v-else-if="!selected" class="py-10 text-center text-sm text-ink-muted dark:text-ink-muted">
        {{ t('admin.ops.alertEvents.detail.empty') }}
      </div>

      <div v-else class="space-y-5">
        <div class="rounded-xl bg-surface-muted p-4 dark:bg-canvas">
          <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <div class="mt-2 text-sm font-semibold text-ink-strong dark:text-white">
                {{ selected.title || '-' }}
              </div>
              <div v-if="selected.description" class="mt-1 whitespace-pre-wrap text-xs text-ink dark:text-ink-muted">
                {{ selected.description }}
              </div>
            </div>

            <div class="flex flex-wrap gap-2">
              <div class="flex items-center gap-2 rounded-lg bg-white px-2 py-1 ring-1 ring-line dark:bg-surface dark:ring-line">
                <span class="text-[11px] font-bold text-ink dark:text-ink-muted">{{ t('admin.ops.alertEvents.detail.silence') }}</span>
                <Select
          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div class="rounded-xl bg-surface-muted p-4 dark:bg-canvas">
              <div class="text-xs font-bold uppercase tracking-wider text-ink-muted">{{ t('admin.ops.alertEvents.detail.firedAt') }}</div>
              <div class="mt-1 text-sm font-medium text-ink-strong dark:text-white">{{ formatDateTime(selected.fired_at || selected.created_at) }}</div>
            </div>
            <div class="rounded-xl bg-surface-muted p-4 dark:bg-canvas">
              <div class="text-xs font-bold uppercase tracking-wider text-ink-muted">{{ t('admin.ops.alertEvents.detail.resolvedAt') }}</div>
              <div class="mt-1 text-sm font-medium text-ink-strong dark:text-white">{{ selected.resolved_at ? formatDateTime(selected.resolved_at) : '-' }}</div>
            </div>
            <div class="rounded-xl bg-surface-muted p-4 dark:bg-canvas">
              <div class="text-xs font-bold uppercase tracking-wider text-ink-muted">{{ t('admin.ops.alertEvents.detail.ruleId') }}</div>
              <div class="mt-1 flex flex-wrap items-center gap-2">
                <div class="font-mono text-sm font-bold text-ink-strong dark:text-white">#{{ selected.rule_id }}</div>
                <a
                  class="inline-flex items-center gap-1 rounded-md bg-white px-2 py-1 text-[11px] font-bold text-ink ring-1 ring-line hover:bg-surface-muted dark:bg-surface dark:text-gray-200 dark:ring-line dark:hover:bg-dark-700"
            ...
            <div class="rounded-xl bg-surface-muted p-4 dark:bg-canvas">
              <div class="text-xs font-bold uppercase tracking-wider text-ink-muted">{{ t('admin.ops.alertEvents.detail.dimensions') }}</div>
              <div class="mt-1 text-sm text-ink-strong dark:text-white">
        ...
        <div class="rounded-xl border border-line bg-white p-4 dark:border-line dark:bg-surface">
          <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
            <div>
              <div class="text-sm font-bold text-ink-strong dark:text-white">{{ t('admin.ops.alertEvents.detail.historyTitle') }}</div>
              <div class="mt-0.5 text-xs text-ink-muted dark:text-ink-muted">{{ t('admin.ops.alertEvents.detail.historyHint') }}</div>
            </div>
            <Select :model-value="historyRange" :options="historyRangeOptions" class="w-[140px]" @change="historyRange = String($event || '7d')" />
          </div>

          <div v-if="historyLoading" class="py-6 text-center text-xs text-ink-muted dark:text-ink-muted">
            {{ t('admin.ops.alertEvents.detail.historyLoading') }}
          </div>
          <div v-else-if="history.length === 0" class="py-6 text-center text-xs text-ink-muted dark:text-ink-muted">
            {{ t('admin.ops.alertEvents.detail.historyEmpty') }}
          </div>
          <div v-else class="overflow-hidden rounded-lg border border-line dark:border-line">
            <table class="min-w-full divide-y divide-line dark:divide-line">
              <thead class="bg-surface-muted dark:bg-canvas">
                <tr>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-ink-muted dark:text-ink-muted">{{ t('admin.ops.alertEvents.table.time') }}</th>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-ink-muted dark:text-ink-muted">{{ t('admin.ops.alertEvents.table.status') }}</th>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-ink-muted dark:text-ink-muted">{{ t('admin.ops.alertEvents.table.metric') }}</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-line dark:divide-line">
                <tr v-for="it in history" :key="it.id" class="hover:bg-surface-muted dark:hover:bg-dark-700/50">
                  <td class="px-3 py-2 text-xs text-ink dark:text-ink-muted">{{ formatDateTime(it.fired_at || it.created_at) }}</td>
                  ...
                  <td class="px-3 py-2 text-xs text-ink dark:text-ink-muted">
