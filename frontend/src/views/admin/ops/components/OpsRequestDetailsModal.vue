      ...
      <div class="flex h-full min-h-0 flex-col">
        ...
        <div v-if="loading" class="flex flex-1 items-center justify-center py-16">
          <div class="flex flex-col items-center gap-3">
            ...
            <span class="text-sm font-medium text-ink-muted dark:text-ink-muted">{{ t('common.loading') }}</span>
          </div>
        </div>

        <!-- Table -->
        <div v-else class="flex min-h-0 flex-1 flex-col">
          <div v-if="items.length === 0" class="rounded-xl border border-dashed border-line p-10 text-center dark:border-line">
            <div class="text-sm font-medium text-ink dark:text-ink-muted">{{ t('admin.ops.requestDetails.empty') }}</div>
            <div class="mt-1 text-xs text-ink-muted">{{ t('admin.ops.requestDetails.emptyHint') }}</div>
          </div>
          ...
          <div v-else class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-line dark:border-line">
            <div
              ...
              role="region"
              :aria-label="t('admin.ops.requestDetails.title')"
              tabindex="0"
            >
              <table class="w-full min-w-[980px] divide-y divide-line dark:divide-line">
                <caption class="sr-only">{{ t('admin.ops.requestDetails.title') }}</caption>
                ...
                <tbody class="divide-y divide-line bg-white dark:divide-line dark:bg-surface">
                  <tr v-for="(row, idx) in items" :key="idx" class="hover:bg-surface-muted dark:hover:bg-dark-700/50">
                  ...
                  <td class="whitespace-nowrap px-4 py-3 text-xs font-medium text-ink dark:text-gray-200">
                    {{ (row.platform || 'unknown').toUpperCase() }}
                  </td>
                  <td class="max-w-[240px] truncate px-4 py-3 text-xs text-ink dark:text-ink-muted" :title="row.model || ''">
                    {{ row.model || '-' }}
                  </td>
                  <td class="whitespace-nowrap px-4 py-3 text-xs text-ink dark:text-ink-muted">
                  ...
                  <td class="px-4 py-3">
                    <div v-if="row.request_id" class="flex items-center gap-2">
                      <span class="max-w-[220px] truncate font-mono text-[11px] text-ink dark:text-gray-200" :title="row.request_id">
                        {{ row.request_id }}
                      </span>
                      <button
                        type="button"
                        class="inline-flex min-h-9 items-center rounded-lg bg-surface-muted px-2 text-[10px] font-bold text-ink hover:bg-line focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500/40 dark:bg-surface-muted dark:text-ink-muted dark:hover:bg-dark-600"
                  ...
                  <td class="whitespace-nowrap px-4 py-3 text-right">
                    <button
                      ...
                      type="button"
                      class="inline-flex min-h-9 items-center rounded-lg bg-red-50 px-3 text-xs font-bold text-red-600 hover:bg-red-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500/40 dark:bg-red-900/20 dark:text-red-300 dark:hover:bg-red-900/30"
