<template>
  <AppLayout>
    <div class="space-y-6">
      <section
        class="flex flex-col gap-4 border-b border-line pb-5 dark:border-line sm:flex-row sm:items-end sm:justify-between"
      >
        <div class="min-w-0">
          <h2 class="text-lg font-semibold text-ink-strong dark:text-white">
            {{ t("admin.plugins.title") }}
          </h2>
          <p class="mt-1 max-w-3xl text-sm text-ink-muted dark:text-ink-muted">
            {{ t("admin.plugins.description") }}
          </p>
          <div
            class="mt-3 flex flex-wrap gap-2 text-xs text-ink dark:text-ink-muted"
          >
            <span class="rounded bg-surface-muted px-2 py-1 dark:bg-surface-muted">{{
      ...
      <div
        ...
        class="flex min-h-56 flex-col items-center justify-center border border-dashed border-line-strong px-6 text-center dark:border-line-strong"
      >
        <Icon name="cube" size="xl" class="text-ink-muted" />
        <p class="mt-3 font-medium text-ink-strong dark:text-gray-200">
          {{ t("admin.plugins.empty") }}
        </p>
        <p class="mt-1 max-w-lg text-sm text-ink-muted dark:text-ink-muted">
      ...
      <div v-else class="grid grid-cols-1 gap-4 xl:grid-cols-2">
        <article
          ...
          :key="plugin.id"
          class="card overflow-hidden border border-line dark:border-line"
        >
          <div
            class="flex flex-wrap items-start justify-between gap-3 border-b border-line p-5 dark:border-line"
          >
            <div class="min-w-0">
              <div class="flex flex-wrap items-center gap-2">
                <h3
                  class="truncate text-base font-semibold text-ink-strong dark:text-white"
                >
                  {{ plugin.name }}
                </h3>
              ...
              <p
                ...
                class="mt-2 text-sm text-ink dark:text-ink-muted"
              >
                {{ plugin.description }}
              </p>
            </div>
          ...
          <div class="grid grid-cols-1 gap-x-6 gap-y-4 p-5 md:grid-cols-2">
            <div>
              <p class="text-xs font-medium uppercase text-ink-muted">
              ...
              >
                ...
                <dd class="font-mono text-ink-strong dark:text-gray-200">
            ...
            <div>
              ...
              <div class="mt-2 flex flex-wrap gap-2 text-xs">
                ...
                <span
                  class="rounded bg-surface-muted px-2 py-0.5 text-ink dark:bg-surface-muted dark:text-ink-muted"
                >
              ...
              <p
                ...
                class="mt-3 break-words text-xs text-ink-muted"
              >
            ...
            <div class="md:col-span-2">
              <label
                class="flex items-center justify-between gap-4 text-xs font-medium text-ink dark:text-ink-muted"
              >
          ...
          <div
            class="flex flex-wrap justify-end gap-2 border-t border-line px-5 py-4 dark:border-line"
          >
            <button
              type="button"
              class="btn btn-secondary btn-sm"
      ...
      >
        <div
          class="relative min-h-[520px] overflow-hidden bg-surface-muted dark:bg-canvas"
          :style="{ height: `${iframeHeight}px` }"
        >
          <div
            v-if="uiLoading"
            class="absolute inset-0 z-10 flex items-center justify-center text-sm text-ink-muted"
          >
          ...
          >
            ...
            <p class="mt-1 max-w-xl text-sm text-ink-muted">{{ uiError }}</p>
          </div>
          <iframe
            v-if="uiSession"
            ref="pluginFrame"
            ...
            class="h-full w-full border-0 bg-white dark:bg-canvas"
            :title="
              t('admin.plugins.configTitle', { name: configPlugin?.name || '' })
            "
