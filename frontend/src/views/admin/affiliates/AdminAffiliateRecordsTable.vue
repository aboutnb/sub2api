  ...
  <AppLayout>
      ...
      <template #filters>
        <div class="flex flex-wrap items-center gap-3">
          <div class="relative w-full md:w-80">
            <Icon name="search" size="md" class="absolute left-3 top-1/2 -translate-y-1/2 text-ink-muted" />
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-aff_code="{ row }">
            <span class="font-mono text-sm text-ink dark:text-ink-muted">{{ row.aff_code || '-' }}</span>
          </template>
          <template #cell-order="{ row }">
            <span v-if="row.order_id == null" class="text-sm text-ink-muted dark:text-ink-muted">-</span>
            <div v-else class="space-y-0.5">
              <div class="font-mono text-sm text-ink-strong dark:text-white">#{{ row.order_id }}</div>
              <div class="max-w-56 truncate text-sm text-ink-muted dark:text-ink-muted">{{ row.out_trade_no }}</div>
            </div>
          </template>
          ...
          <template #cell-pay_amount="{ row }">
            <span v-if="row.pay_amount == null" class="text-sm text-ink-muted dark:text-ink-muted">-</span>
            <span v-else class="text-sm text-ink-strong dark:text-white">¥{{ formatAmount(row.pay_amount) }}</span>
          </template>
          ...
          <template #cell-created_at="{ row }">
            <span class="text-sm text-ink dark:text-ink-muted">{{ formatDateTime(row.created_at) }}</span>
          </template>
        </DataTable>
      </template>
    ...
    >
      ...
      <div v-else-if="selectedOverview" class="space-y-4">
        <div class="rounded-lg border border-line bg-surface-muted p-4 dark:border-line dark:bg-surface">
          <div class="font-mono text-sm text-ink-strong dark:text-white">#{{ selectedOverview.user_id }}</div>
          <div class="mt-1 text-sm font-medium text-ink-strong dark:text-white">{{ selectedOverview.email || '-' }}</div>
          <div class="mt-0.5 text-sm text-ink-muted dark:text-ink-muted">{{ selectedOverview.username || '-' }}</div>
        </div>
...
const UserCell = defineComponent({
  ...
  setup(cellProps, { emit }) {
    return () => h('div', { class: 'space-y-0.5' }, [
      h('div', { class: 'font-mono text-sm text-ink-strong dark:text-white' }, `#${cellProps.id}`),
      h(cellProps.clickable ? 'button' : 'div', {
        class: cellProps.clickable
          ...
          : 'max-w-56 truncate text-sm text-ink dark:text-ink-muted',
        type: cellProps.clickable ? 'button' : undefined,
        onClick: cellProps.clickable ? () => emit('open', cellProps.id) : undefined,
      }, cellProps.email || '-'),
      h('div', { class: 'max-w-56 truncate text-sm text-ink-muted dark:text-ink-muted' }, cellProps.username || '-'),
    ])
  },
})
  ...
  setup(amountProps) {
    return () => h('span', {
      class: amountProps.strong
        ...
        : 'text-sm text-ink-strong dark:text-white',
  ...
  setup(amountProps) {
    return () => {
      ...
      if (value === null || value === undefined) {
        return h('span', { class: 'text-sm text-ink-muted dark:text-ink-muted' }, '-')
      }
...
const OverviewStat = defineComponent({
  ...
  setup(statProps) {
    return () => h('div', { class: 'rounded-lg border border-line bg-white p-3 dark:border-line dark:bg-canvas' }, [
      h('div', { class: 'text-sm text-ink-muted dark:text-ink-muted' }, statProps.label),
      h('div', {
        class: statProps.mono
          ? 'mt-1 font-mono text-base font-semibold text-ink-strong dark:text-white'
          : 'mt-1 text-base font-semibold text-ink-strong dark:text-white',
      }, statProps.value),
    ])
  },
})
