<template>
  <AppLayout>
    <div class="mx-auto flex max-w-md flex-col items-center space-y-6 py-8">
      ...
      <div v-if="qrUrl" class="card p-6">
        <canvas ref="qrCanvas" class="mx-auto"></canvas>
      </div>
      <!-- Scan prompt for QR code -->
      <p v-if="qrUrl && !expired && scanHint" class="text-center text-sm text-ink-muted dark:text-ink-muted">
        {{ scanHint }}
      </p>
      ...
      <div v-else class="text-center">
        <p class="text-sm text-ink-muted dark:text-ink-muted">{{ qrUrl ? t('payment.qr.expiresIn') : t('payment.qr.payInNewWindowHint') }}</p>
        <p class="mt-1 text-2xl font-bold tabular-nums text-ink-strong dark:text-white">{{ countdownDisplay }}</p>
        <p class="mt-2 text-sm text-ink-muted dark:text-ink-muted">{{ t('payment.qr.waitingPayment') }}</p>
      </div>
