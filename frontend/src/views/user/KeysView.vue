  ...
  <AppLayout>
      ...
      <template #table>
        ...
        >
            ...
            <div class="flex items-center gap-2">
              ...
              <button
                ...
                :class="
                  copiedKeyId === row.id
                    ? 'text-green-500'
                    : 'text-ink-muted hover:text-ink dark:hover:text-ink-muted'
                "
          ...
          <template #cell-group="{ row }">
            ...
            <div v-else class="group/dropdown relative">
              <button
                ...
                class="-mx-2 -my-1 flex cursor-pointer items-center gap-2 rounded-lg px-2 py-1 transition-all duration-200 hover:bg-surface-muted dark:hover:bg-dark-700"
              ...
              >
                ...
                />
                <span v-else class="text-sm text-ink-muted dark:text-ink-muted">{{
                  t('keys.noGroup')
                }}</span>
                <span class="text-xs text-ink-muted dark:text-ink-muted">{{ t('keys.selectGroup') }}</span>
                <svg
                  class="h-3.5 w-3.5 text-ink-muted opacity-60 transition-opacity group-hover/dropdown:opacity-100"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                  stroke-width="2"
          ...
          <template #cell-usage="{ row }">
            <div class="text-sm">
              <div class="flex items-center gap-1.5">
                <span class="text-ink-muted dark:text-ink-muted">{{ t('keys.today') }}:</span>
                <span class="font-medium text-ink-strong dark:text-white">
                  ${{ (usageStats[row.id]?.today_actual_cost ?? 0).toFixed(4) }}
                </span>
              </div>
              <div class="mt-0.5 flex items-center gap-1.5">
                <span class="text-ink-muted dark:text-ink-muted">{{ t('keys.total') }}:</span>
                <span class="font-medium text-ink-strong dark:text-white">
              ...
              <div v-if="row.quota > 0" class="mt-1.5">
                <div class="flex items-center gap-1.5">
                  <span class="text-ink-muted dark:text-ink-muted">{{ t('keys.quota') }}:</span>
                  <span :class="[
                    'font-medium',
                ...
                <div class="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-line dark:bg-line-strong">
                  <div
                    :class="[
            ...
            <div v-if="row.rate_limit_5h > 0 || row.rate_limit_1d > 0 || row.rate_limit_7d > 0" class="space-y-1.5 min-w-[140px]">
              ...
              <div v-if="row.rate_limit_5h > 0">
                <div class="flex items-center justify-between text-xs">
                  <span class="text-ink-muted dark:text-ink-muted">5h</span>
                  <span :class="[
                ...
                <div class="h-1 w-full overflow-hidden rounded-full bg-line dark:bg-line-strong">
                  <div
                    :class="[
                      'h-full rounded-full transition-all',
                ...
                <div v-if="row.reset_5h_at && formatResetTime(row.reset_5h_at)" class="text-[10px] text-ink-muted dark:text-ink-muted tabular-nums">
              ...
              <div v-if="row.rate_limit_1d > 0">
                <div class="flex items-center justify-between text-xs">
                  <span class="text-ink-muted dark:text-ink-muted">1d</span>
                  <span :class="[
                    'font-medium tabular-nums',
                ...
                <div v-if="row.reset_1d_at && formatResetTime(row.reset_1d_at)" class="text-[10px] text-ink-muted dark:text-ink-muted tabular-nums">
              ...
              <div v-if="row.rate_limit_7d > 0">
                <div class="flex items-center justify-between text-xs">
                  <span class="text-ink-muted dark:text-ink-muted">7d</span>
                  <span :class="[
                    'font-medium tabular-nums',
                ...
                <div v-if="row.reset_7d_at && formatResetTime(row.reset_7d_at)" class="text-[10px] text-ink-muted dark:text-ink-muted tabular-nums">
              ...
              <button
                ...
                class="mt-0.5 inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-xs text-ink-muted transition-colors hover:bg-surface-muted hover:text-primary-600 dark:hover:bg-dark-700 dark:hover:text-primary-400"
          ...
          <template #cell-expires_at="{ value }">
            <span v-if="value" :class="[
              'text-sm',
              new Date(value) < new Date() ? 'text-red-500 dark:text-red-400' : 'text-ink-muted dark:text-ink-muted'
            ]">
              {{ formatDateTime(value) }}
            </span>
            <span v-else class="text-sm text-ink-muted dark:text-ink-muted">{{ t('keys.noExpiration') }}</span>
          </template>
          ...
          <template #cell-last_used_at="{ value }">
            <span v-if="value" class="text-sm text-ink-muted dark:text-ink-muted">
              {{ formatDateTime(value) }}
            </span>
            <span v-else class="text-sm text-ink-muted dark:text-ink-muted">-</span>
          </template>
        ...
        <div class="space-y-3">
          ...
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                ...
                formData.enable_quota ? 'bg-primary-600' : 'bg-line dark:bg-line-strong'
              ]"
            >
              <span
                :class="[
          ...
          <div class="space-y-4">
            ...
            <div v-if="showEditModal && selectedKey && selectedKey.quota > 0">
              ...
              <div class="flex items-center gap-2">
                <div class="flex-1 rounded-lg bg-surface-muted px-3 py-2 dark:bg-surface-muted">
                  <span class="font-medium text-ink-strong dark:text-white">
          ...
          <div v-if="formData.enable_rate_limit" class="space-y-4 pt-2">
            ...
            <div>
              ...
              <div v-if="showEditModal && selectedKey && selectedKey.rate_limit_5h > 0" class="mt-2">
                <div class="flex items-center gap-2">
                  <div class="flex-1 rounded-lg bg-surface-muted px-3 py-2 dark:bg-surface-muted text-sm">
                    <span :class="[
                      'font-medium',
        ...
        <div class="space-y-3">
          ...
          <div v-if="formData.enable_expiration" class="space-y-4 pt-2">
            ...
            <div v-if="showEditModal && selectedKey?.expires_at" class="text-sm">
              <span class="text-ink-muted dark:text-ink-muted">{{ t('keys.currentExpiration') }}: </span>
              <span class="font-medium text-ink-strong dark:text-white">
          ...
          {{ t('keys.ccsClientSelect.description') }}
	        ...
	        <div class="grid grid-cols-2 gap-3">
	          <button
	            ...
	            class="flex flex-col items-center gap-2 p-4 rounded-xl border-2 border-line dark:border-line-strong hover:border-primary-500 dark:hover:border-primary-500 hover:bg-primary-50 dark:hover:bg-primary-900/20 transition-all"
	          >
	            <Icon name="terminal" size="xl" class="text-ink dark:text-ink-muted" />
	            <span class="font-medium text-ink-strong dark:text-white">{{
	          ...
	          <button
	            ...
	            class="flex flex-col items-center gap-2 p-4 rounded-xl border-2 border-line dark:border-line-strong hover:border-primary-500 dark:hover:border-primary-500 hover:bg-primary-50 dark:hover:bg-primary-900/20 transition-all"
	          >
	            <Icon name="sparkles" size="xl" class="text-ink dark:text-ink-muted" />
	            <span class="font-medium text-ink-strong dark:text-white">{{
    ...
    <Teleport to="body">
      <div
        ...
        ref="dropdownRef"
        class="animate-in fade-in slide-in-from-top-2 fixed z-[100000020] w-max max-w-[calc(100vw-16px)] overflow-hidden rounded-xl bg-white shadow-lg ring-1 ring-black/5 duration-200 sm:min-w-[380px] dark:bg-surface dark:ring-white/10"
      ...
      >
        <!-- Search box -->
        <div class="border-b border-line p-2 dark:border-line">
          <div class="relative">
            <svg class="absolute left-2.5 top-1/2 h-4 w-4 -translate-y-1/2 text-ink-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
            <input
              v-model="groupSearchQuery"
              type="text"
              class="w-full rounded-lg border border-line bg-surface-muted py-1.5 pl-8 pr-3 text-sm text-ink-strong placeholder-gray-400 outline-none focus:border-primary-300 focus:ring-1 focus:ring-primary-300 dark:border-line-strong dark:bg-surface-muted dark:text-white dark:placeholder-gray-500 dark:focus:border-primary-600 dark:focus:ring-primary-600"
        ...
        <div class="max-h-80 overflow-y-auto p-1.5">
          <button
            ...
            :class="[
              ...
              'border-b border-line last:border-0 dark:border-line',
              selectedKeyForGroup?.group_id === option.value ||
              (!selectedKeyForGroup?.group_id && option.value === null)
                ? 'bg-primary-50 dark:bg-primary-900/20'
                : 'hover:bg-surface-muted dark:hover:bg-dark-700'
            ]"
          ...
          <div v-if="filteredGroupOptions.length === 0" class="py-4 text-center text-sm text-ink-muted dark:text-ink-muted">
            {{ t('keys.noGroupFound') }}
          </div>
        </div>
      </div>
...
const formRoutingGroups = computed(() => showEditModal.value
  ? groups.value
