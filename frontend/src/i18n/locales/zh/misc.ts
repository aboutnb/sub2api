export default {
  ...
  onboarding: {
    ...
    admin: {
      ...
      accountPriority: {
        title: '⚖️ 4. 优先级（可选）',
        description:
          '<div style="line-height: 1.7;"><p style="margin-bottom: 12px;">设置账号的调用优先级。</p><div style="padding: 8px 12px; background: #eff6ff; border-left: 3px solid #3b82f6; border-radius: 4px; font-size: 13px; margin-bottom: 12px;"><b>📊 优先级规则：</b><ul style="margin: 8px 0 0 16px;"><li>1 为最高优先级</li><li>数值越小，系统越优先使用</li><li>相同优先级再按其他调度条件选择</li></ul></div><p style="padding: 8px 12px; background: #f0fdf4; border-left: 3px solid #10b981; border-radius: 4px; font-size: 13px;"><b>💡 使用场景：</b>主账号设置较小数值（推荐 1），备用账号设置较大数值</p></div>',
        nextBtn: '下一步'
      },
      accountGroups: {
        title: '🎯 5. 分配分组',
  ...
  payment: {
    ...
    fee: '手续费',
    actualPay: '实付金额',
    checkoutSummary: '订单核对',
    feeCreditedNotice: '手续费将计入到账余额，本次支付 {pay}，到账 {credited}。',
    createOrder: '确认支付',
    tabUSDT: 'USDT 支付',
    usdt: {
      moduleLabel: '独立链上支付',
      loadFailed: 'USDT 支付暂时无法加载',
      unavailableTitle: 'USDT 充值暂未开放',
      title: 'USDT 链上支付',
      network: '支付网络',
      selectedNetwork: '本次订单支付网络',
      networkInstruction: '请仅使用该网络转账：USDT · {network}（{standard}）。',
      fiatAmount: '人民币金额',
      notice: '请只在选定网络转账，并按精确数量支付。少付、多付或转错网络都可能导致订单无法自动匹配。',
      unavailable: '当前没有健康可用的 USDT 网络，请稍后再试。',
      create: '创建 USDT 订单',
      confirmPayment: '确认支付 {amount} USDT',
      realtimeConversion: '实时折算',
      bonusLine: '充值赠送 {percent}%',
      estimatedBalance: '预计增加余额（含赠送）',
      createFailed: 'USDT 订单创建失败，请稍后再试。',
      payToAddress: '转账至收款地址',
      exactAmount: '精确支付数量',
      cashierTitle: 'USDT 原生收银台',
      cashierHint: '请在原生收银台完成支付，Sub2API 会自动同步订单状态。',
      cashierPopupTitle: '原生收银台已准备就绪',
      cashierPopupHint: '收银台会在新窗口打开，请在其中完成付款。订单金额和赠送规则保持不变。',
      cashierPopupBlockedHint: '如果浏览器拦截了弹窗，请点击下方按钮重新打开。',
      cashierPopupBlocked: '浏览器拦截了收银台弹窗，请点击“打开收银台”。',
      openCashier: '打开收银台',
      cancel: '取消订单',
      cancelConfirm: '确定取消此 USDT 订单吗？请确认尚未发起转账；取消后不能继续等待该订单。',
      cancelPrompt: '仅在尚未发起转账时取消。订单取消后将不能继续支付。',
      confirmCancel: '确认取消',
      keepPaying: '继续支付',
      cancelFailed: '取消订单失败，请稍后重试。',
      terminalHint: '该订单已结束。如需继续支付，请创建新订单；此前已发起的转账仍会按支付规则核验。',
      exchangeRate: '实时汇率',
      rateUpdatedAt: '更新于 {time}',
      rateUnavailable: '实时汇率暂不可用，暂不能创建订单。',
      statusHint: '支付页面关闭后，订单仍会保留。系统会通过回调和主动对账持续确认到账状态。',
      openCheckout: '打开支付页',
      newOrder: '创建新订单',
    },
    methods: {
    ...
    invoice: {
      ...
      feePolicy: '开票费用规则',
      taxMode: '开票费承担方',
      orderStatusHint: '该订单已关联开票申请，不能重复合并开票。',
      orderStatus: {
        draft: '待完成开票',
        failed: '开票待重试',
        submitting: '开票提交中',
        submission_unknown: '开票结果待确认',
        pending: '开票审核中',
        approved: '已审核待开票',
        completed: '已开票',
      },
      ...
      taxRequired: '用户支付费用',
      platformPaysTaxNotice: '用户按订单金额开票，收款平台从商户侧扣除开票费。',
      userPaysTaxNotice: '用户需先支付开票费，发票价税合计为订单金额加开票费。',
      draftPending: '有一笔未完成的开票申请',
      draftSummary: '已保留 {count} 笔订单及当前开票进度。',
      resumeDraft: '继续申请',
      abandonDraft: '放弃申请',
      saveAndClose: '暂存并关闭',
      selectOrder: '选择开票',
      selectOrdersStep: '选择订单',
      taxReviewStep: '税费核对',
      buyerInfoStep: '填写资料',
      applicationTitle: '提交开票申请',
      orderCount: '订单数量',
      orderAmount: '订单金额',
      invoiceAmount: '发票价税合计',
      taxAmount: '开票费',
      taxDue: '待付开票费',
      taxPending: '待支付',
      payTaxTitle: '支付开票费',
      payTaxNotice: '本次需支付 {fee}，到账后即可继续填写资料。',
      openCashier: '打开收银台',
      returnToConfirm: '完成付款后，请返回此处确认到账状态。',
      checkTaxPayment: '确认付款状态',
      buyerType: '购买方类型',
      requiredInfo: '必填信息',
      optionalInfo: '补充信息',
      company: '企业',
      individual: '个人',
      invoiceTitle: '发票抬头',
      taxpayerId: '纳税人识别号 / 身份证号',
      recipientEmail: '接收邮箱',
      emailNotice: '开票完成后，PDF 发票将同时发送至该邮箱。',
      buyerAddress: '注册地址（选填）',
      buyerPhone: '注册电话（选填）',
      buyerBank: '开户银行（选填）',
      buyerBankAccount: '银行账号（选填）',
      submitApplication: '提交开票申请',
      recordsTitle: '开票记录',
      ...
      validationFailed: '订单校验失败',
      taxPaymentNotConfirmed: '暂未查询到税费付款，请稍后重试。',
      taxReconciliationPending: '已确认本笔付款，但税费仍未结清，请继续完成剩余税费。',
      taxPaymentConfirmed: '税费付款已确认',
      taxCheckFailed: '税费付款状态查询失败',
      applicationSubmitted: '开票申请已提交',
      submitFailed: '开票申请提交失败',
      recordsFailed: '开票记录加载失败',
      confirmCancelApplication: '确定撤销这条开票申请吗？',
      applicationCanceled: '开票申请已撤销',
      cancelFailed: '开票申请撤销失败',
    ...
    errors: {
      ...
      USDT_PAYMENT_SECRET_DECRYPT_FAILED: '已保存的 USDT 商户密钥无法解密，请恢复原 TOTP_ENCRYPTION_KEY，或重新输入商户 HMAC Secret 并保存',
      USDT_PAYMENT_SECRET_ENCRYPTION_UNAVAILABLE: 'USDT 商户密钥加密不可用，请先配置固定的 TOTP_ENCRYPTION_KEY',
      USDT_PAYMENT_CONFIG_UNAVAILABLE: 'USDT 支付配置无法读取，请检查数据库配置记录和服务器加密密钥',
      USDT_PAYMENT_CREDENTIALS_REQUIRED: 'USDT 支付需要填写 Key ID 和商户 HMAC Secret',
      USDT_PAYMENT_CREDENTIALS_REJECTED: 'BEpusdt 拒绝了商户凭据，请检查 Key ID 和商户 HMAC Secret',
      USDT_PAYMENT_UPSTREAM_BLOCKED: 'BEpusdt 接口被上游边缘网络拦截，请改用服务端可访问的私有 API 地址',
      USDT_PAYMENT_MERCHANT_API_MISSING: '当前 BEpusdt 地址未提供商户接口，请检查 API 地址或部署商户版 BEpusdt',
      USDT_PAYMENT_UPSTREAM_TIMEOUT: 'BEpusdt 接口请求超时，请检查服务器到支付网关的网络和防火墙',
      USDT_PAYMENT_UPSTREAM_UNAVAILABLE: 'BEpusdt 接口不可用，请检查服务状态、API 地址和服务器网络',
      USDT_PAYMENT_ORDER_TIMEOUT_INVALID: 'USDT 订单超时时间必须在 180 至 3600 秒之间',
      USDT_PAYMENT_MINIMUM_AMOUNT_INVALID: 'USDT 最低支付金额必须在 0.01 至 1000000 之间',
      USDT_AMOUNT_TOO_LOW: 'USDT 支付金额低于系统设置的最低金额',
      USDT_PAYMENT_LATE_WINDOW_INVALID: 'USDT 迟到支付容忍时间必须在 0 至 1440 分钟之间',
      USDT_PAYMENT_REQUEST_TIMEOUT_INVALID: 'USDT 请求超时时间必须在 1 至 30 秒之间',
      USDT_PAYMENT_RECONCILE_INTERVAL_INVALID: 'USDT 对账间隔必须在 1 至 60 秒之间',
      USDT_PAYMENT_RECONCILE_BATCH_INVALID: 'USDT 对账批量数必须在 1 至 1000 之间',
      USDT_PAYMENT_WEBHOOK_SKEW_INVALID: 'USDT 回调时钟容差必须在 30 至 900 秒之间',
      USDT_PAYMENT_URL_INVALID: 'USDT 支付地址格式无效，请检查 API 地址和公网回调地址',
      USDT_PAYMENT_FIAT_INVALID: 'USDT 支付目前仅支持 CNY 结算',
      USDT_PAYMENT_NETWORKS_REQUIRED: '启用 USDT 支付时至少需要选择一个网络',
      USDT_PAYMENT_NETWORK_INVALID: 'USDT 支付网络配置无效，请检查已启用的网络',
      INVOICE_NOT_CONFIGURED: '开票服务暂未开放',
      INVOICE_INVALID_ORDERS: '请选择 1 至 20 笔订单',
      INVOICE_DUPLICATE_ORDER: '选择的订单存在重复',
      INVOICE_ORDER_NOT_ELIGIBLE: '部分订单不满足开票条件，请重新选择',
      INVOICE_ORDER_ALREADY_CLAIMED: '部分订单已申请开票，请刷新开票记录',
      INVOICE_DRAFT_ACTIVE: '请先继续或放弃当前未完成的开票申请',
      INVOICE_DRAFT_NOT_ACTIVE: '该开票草稿已不在进行中',
      INVOICE_TAX_DRAFT_MUST_RESUME: '该申请可能已有开票费付款，请继续完成申请',
      INVOICE_TAX_ORDER_REQUIRED: '开票税费尚未结清',
      INVOICE_INVALID_TAX_ORDERS: '税费订单无效，请重新发起开票',
      INVOICE_TAX_PAYMENT_NOT_ELIGIBLE: '税费付款尚未确认或不满足要求',
      INVOICE_TAX_PAYMENT_EXCEEDS_REQUIRED: '已付税费超过当前订单所需税费，请恢复原订单组合',
      INVOICE_TAX_ORDER_ALREADY_CLAIMED: '该税费订单已用于其他开票申请',
      INVOICE_INVALID_FORM: '开票信息格式有误，请检查后重试',
      INVOICE_CANNOT_CANCEL: '当前状态无法撤销开票申请',
      INVOICE_PDF_NOT_READY: '电子发票尚未生成',
      INVOICE_SUBMISSION_IN_PROGRESS: '开票申请正在提交，请勿重复操作',
      INVOICE_SUBMISSION_UNKNOWN: '提交结果待确认，请刷新开票记录',
      INVOICE_SERVICE_UNAVAILABLE: '开票服务暂时不可用，请稍后重试',
      INVOICE_ORDER_RATE_LIMITED: '订单校验过于频繁，请稍后重试',
      INVOICE_RATE_LIMITED: '开票申请过于频繁，请稍后重试',
      INVOICE_TAX_STATUS_RATE_LIMITED: '税费状态查询过于频繁，请稍后重试',
