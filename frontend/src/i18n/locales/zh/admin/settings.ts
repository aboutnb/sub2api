export default {
    settings: {
      ...
      site: {
        ...
        communityGroupName: '交流群名称',
        communityGroupNamePlaceholder: '交流群',
        communityGroupNameHint: '顶部入口显示的名称。留空时显示“交流群”。',
        communityGroupIcon: '交流群图标',
        communityGroupIconHint: '支持 PNG、JPG 或 SVG，最大 100KB。留空时使用默认联系图标。',
        communityGroupUrl: '交流群链接',
        communityGroupUrlPlaceholder: 'https://example.com/community',
        communityGroupUrlHint: '交流群的跳转链接。留空则隐藏顶部交流群入口。',
        docUrl: '文档链接',
      ...
      payment: {
        ...
        usdtCheckoutModeHint: '固定模式由 Sub2API 选择网络；原生收银台模式内嵌 BEpusdt，由用户在收银台选择网络，金额与入账规则不变。',
        usdtCheckoutFixed: '固定网络（Sub2API 选择）',
        usdtCheckoutCashier: '原生收银台（用户选择网络）',
        ...
        usdt: {
          title: 'USDT 支付',
          description: '通过 BEpusdt 接入 USDT 多网络收款。配置保存后立即生效。',
          apiBase: 'BEpusdt 服务地址',
          apiBaseHint: 'Sub2API 服务端访问地址；同一 Docker 网络可使用 http://bepusdt:8080。',
          publicBaseUrl: '支付页面公网地址',
          publicBaseUrlHint: '用户浏览器打开的 BEpusdt 地址。',
          callbackBaseUrl: 'Sub2API 公网地址',
          callbackBaseUrlHint: 'BEpusdt 回调和用户返回页面必须能访问此地址。',
          keyId: 'Key ID',
          apiSecret: '商户 HMAC Secret',
          secretPlaceholder: '输入 BEpusdt 商户 HMAC Secret',
          secretConfiguredPlaceholder: '已配置，留空保持不变',
          secretHint: '必须与 BEpusdt 的 BEPUSDT_HMAC_SECRET 一致；不是后台“对接令牌”。密钥仅在服务端加密保存，不会回显。',
          fiat: '结算币种',
          networks: '启用网络',
          networksHint: '仅允许健康且已配置钱包的网络创建订单。',
          minimumAmount: '最低支付金额',
          minimumAmountHint: '用户单笔 USDT 支付不得低于此金额，默认 5 USDT。',
          bonusPercent: 'USDT 赠送比例',
          bonusPercentHint: '仅作用于 USDT 余额充值到账，不参与通用充值手续费。设置 5 表示支付等值 100 元后到账 105 元。',
          orderTimeout: '订单超时（秒）',
          lateWindow: '迟到支付容忍（分钟）',
          requestTimeout: '请求超时（秒）',
          reconcileInterval: '对账间隔（秒）',
          reconcileBatch: '对账批量数',
          webhookSkew: '回调时钟容差（秒）',
          configWarningsTitle: '检测到可能的迁移配置风险',
          webhookHint: '请将以下地址配置到 BEpusdt 的 Webhook：',
          encryptionKeyHint: '保存 API Secret 前，请先在部署环境配置固定的 TOTP_ENCRYPTION_KEY。',
          testConnection: '测试连接',
          testing: '测试中',
          ready: '支付服务可接单',
          degraded: '支付服务未就绪',
          ...
          testSuccess: 'BEpusdt 连接、汇率和网络状态验证通过',
          testDegraded: 'BEpusdt 可访问，但当前没有可接单网络',
          testFailed: 'BEpusdt 连接测试失败',
        },
        invoice: {
          title: '自助开票',
          description: '连接 XZNOAuth 开票服务，用户可在“我的订单”中申请、查询和下载发票。',
          enabled: '启用开票',
          configured: '配置完整',
          incomplete: '缺少凭据',
          baseUrl: '服务地址',
          ...
          seconds: '秒',
          feePayer: '开票费承担策略',
          feePayerCustomer: '用户承担（默认）',
          feePayerPlatform: '平台承担',
          feePayerUserChoice: '允许用户选择',
          feePayerCustomerHint: '用户提交开票申请前支付开票费，发票金额为订单金额加开票费。',
          feePayerPlatformHint: '用户按订单金额开票，开票平台从商户侧扣除开票费。',
          feePayerUserChoiceHint: '用户每次申请时可选择由自己支付开票费，或由平台承担。',
          secretPlaceholder: '输入 Client Secret',
          secretConfiguredPlaceholder: '已配置，留空保持不变',
          secretEncryptionKeyRequired: '保存新的或替换后的 Client Secret 前，需先在部署环境配置固定的 TOTP_ENCRYPTION_KEY。',
          requirement: 'XZNOAuth 应用需已获批 invoice.apply 权限；开票金额和开票费均以上游校验结果为准。',
          incompleteError: '启用自助开票前，请填写服务地址、Client ID 和 Client Secret。',
          timeoutError: '开票请求超时必须是 1 到 120 秒之间的整数。',
        },
        orderTimeout: '订单超时时间',
