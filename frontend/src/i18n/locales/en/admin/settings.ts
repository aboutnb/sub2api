export default {
    settings: {
      ...
      site: {
        ...
        communityGroupName: 'Community Name',
        communityGroupNamePlaceholder: 'Community',
        communityGroupNameHint: 'Name shown in the header. Defaults to “Community” when empty.',
        communityGroupIcon: 'Community Icon',
        communityGroupIconHint: 'PNG, JPG, or SVG up to 100KB. The default contact icon is used when empty.',
        communityGroupUrl: 'Community Group URL',
        communityGroupUrlPlaceholder: 'https://example.com/community',
        communityGroupUrlHint: 'Link to the community group. Leave empty to hide it from the header.',
      ...
      payment: {
        ...
        usdtCheckoutModeHint: 'Fixed mode selects the network in Sub2API. Cashier mode embeds BEpusdt so the customer selects the network there; amount and settlement rules stay unchanged.',
        usdtCheckoutFixed: 'Fixed network (Sub2API)',
        usdtCheckoutCashier: 'Native cashier (customer selects network)',
        ...
        subscriptionFeeEnabled: 'Charge fee on subscriptions',
        subscriptionFeeEnabledHint: 'When enabled, subscription purchases include the fee rate above. When disabled, subscriptions charge only the plan amount; balance top-ups are unchanged.',
        ...
        usdt: {
          title: 'USDT payments',
          description: 'Accept USDT on multiple networks through BEpusdt. Changes apply immediately after saving.',
          apiBase: 'BEpusdt service URL',
          apiBaseHint: 'Server-side URL; use http://bepusdt:8080 when both services share a Docker network.',
          publicBaseUrl: 'Public payment URL',
          publicBaseUrlHint: 'BEpusdt URL that users can open in their browser.',
          callbackBaseUrl: 'Public Sub2API URL',
          callbackBaseUrlHint: 'BEpusdt webhooks and return redirects must reach this URL.',
          keyId: 'Key ID',
          apiSecret: 'Merchant HMAC Secret',
          secretPlaceholder: 'Enter the BEpusdt merchant HMAC secret',
          secretConfiguredPlaceholder: 'Configured; leave blank to keep it',
          secretHint: 'Must match BEPUSDT_HMAC_SECRET; it is not the BEpusdt admin legacy API token. Stored encrypted on the server and never returned to the browser.',
          fiat: 'Settlement currency',
          networks: 'Enabled networks',
          networksHint: 'Orders are accepted only on healthy networks with configured wallets.',
          minimumAmount: 'Minimum payment amount',
          minimumAmountHint: 'A single USDT payment cannot be lower than this amount. The default is 5 USDT.',
          bonusPercent: 'USDT bonus percentage',
          bonusPercentHint: 'Applied only to USDT balance credits; it does not participate in the generic recharge fee. 5 means CNY 100 equivalent credits CNY 105.',
          orderTimeout: 'Order timeout (sec)',
          lateWindow: 'Late payment window (min)',
          requestTimeout: 'Request timeout (sec)',
          reconcileInterval: 'Reconcile interval (sec)',
          reconcileBatch: 'Reconcile batch size',
          webhookSkew: 'Webhook clock skew (sec)',
          configWarningsTitle: 'Potential migrated configuration risk detected',
          webhookHint: 'Configure this Webhook URL in BEpusdt:',
          encryptionKeyHint: 'Set a fixed TOTP_ENCRYPTION_KEY in the deployment before saving the API Secret.',
          testConnection: 'Test connection',
          testing: 'Testing',
          ready: 'Payment service ready',
          degraded: 'Payment service not ready',
          networkReady: 'Ready',
          networkUnavailable: 'Unavailable',
          testSuccess: 'BEpusdt connectivity, rate, and network health checks passed',
          testDegraded: 'BEpusdt is reachable, but no network is ready for orders',
          testFailed: 'BEpusdt connection test failed',
        },
        invoice: {
          title: 'Self-service Invoicing',
          description: 'Connect XZNOAuth so users can apply for, track, and download invoices from My Orders.',
          enabled: 'Enable invoicing',
          configured: 'Ready',
          ...
          seconds: 'sec',
          feePayer: 'Invoice fee policy',
          feePayerCustomer: 'Customer pays (default)',
          feePayerPlatform: 'Platform pays',
          feePayerUserChoice: 'Let customer choose',
          feePayerCustomerHint: 'The customer pays the invoice fee before submitting. The invoice total is the order amount plus the fee.',
          feePayerPlatformHint: 'The customer invoices only the order amount, and the invoice provider charges the merchant.',
          feePayerUserChoiceHint: 'The customer may choose who pays the invoice fee for each application.',
          secretPlaceholder: 'Enter Client Secret',
          secretConfiguredPlaceholder: 'Configured; leave blank to keep it',
          secretEncryptionKeyRequired: 'Configure a fixed TOTP_ENCRYPTION_KEY in the deployment before saving a new or replacement Client Secret.',
          requirement: 'The XZNOAuth app must have the invoice.apply scope. Invoice totals and fees always use the upstream validation result.',
          incompleteError: 'Enter the service URL, Client ID, and Client Secret before enabling self-service invoicing.',
          timeoutError: 'Invoice request timeout must be an integer from 1 to 120 seconds.',
        },
