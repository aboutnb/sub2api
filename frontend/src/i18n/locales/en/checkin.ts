export default {
  checkin: {
    title: 'Check-in Center',
    description: 'Claim today\'s check-in reward',
    currentBalance: 'Current balance',
    notCheckedToday: 'Not checked in',
    checkedToday: 'Checked in',
    quickAction: 'Quick check-in',
    submitting: 'Checking in',
    openPage: 'Check-in details',
    normal: 'Normal check-in',
    lucky: 'Lucky check-in',
    normalAction: 'Check in now',
    normalHint: 'Receive a balance reward from the current configuration; the final result is shown after submission.',
    normalConfirmTitle: 'Confirm normal check-in',
    normalConfirmOnce: 'You can check in once per day. After confirming, the reward uses the current configuration.',
    normalConfirmAction: 'Confirm normal check-in',
    luckyHint: 'The result may increase or decrease your balance, which will not go below 0.',
    luckyConfirmTitle: 'Confirm lucky check-in',
    luckyPossibleMultiplier: 'Possible balance-change multiplier',
    luckyMultiplierRisk: 'Lucky check-in is settled against your current balance. It may add to or deduct from the balance, which will not go below 0.',
    luckyAmountRisk: 'Lucky check-in may add to or deduct from your balance, which will not go below 0.',
    luckyConfirmOnce: 'You can check in once per day. After confirming, you cannot switch to normal check-in.',
    luckyConfirmAction: 'Confirm lucky check-in',
    checkedIn: 'Checked in today',
    success: 'Check-in complete',
    failedDescription: 'Check-in failed. Please try again later.',
    change: 'Balance change',
    mode: 'Mode',
    resultMultiplier: 'Check-in multiplier',
    balanceAfter: 'Balance after',
    multiplierResult: 'Multiplier {value}',
    calendarTitle: 'Check-in calendar',
    calendarHint: 'Daily check-in balance changes',
    calendarChecked: 'Checked in',
    calendarToday: 'Today',
    calendarUnavailable: 'Unavailable',
    monthSummary: 'This month',
    daysCount: '{count} days',
    netChange: 'Net change',
    timesCount: '{count} times',
    history: 'Check-in history',
    historyHint: 'Most recent activity first',
    pageInfo: 'Page {page} / {pages}',
    previous: 'Previous',
    next: 'Next',
    retry: 'Retry',
    noHistory: 'No check-in records yet',
    unavailableTitle: 'Check-in unavailable',
    unavailable: 'Check-in is not available for this account right now.',
    noModesAvailable: 'No check-in mode is currently available.',
    modeDisabled: 'This check-in mode has been disabled. Refresh the page and try again.',
    accountTooNew: 'This account was created recently and cannot check in yet.',
    negativeBalance: 'Check-in is not available for the current balance state.',
    grantRestricted: 'This account must complete a real recharge before it can check in again.',
    sourceLimited: 'Several accounts have checked in from this network recently. Please try again later.',
    rateLimited: 'Too many attempts. Please try again later.',
    riskUnavailable: 'Check-in security verification is temporarily unavailable. Please try again later.',
    securityUnavailable: 'The check-in security service is temporarily unavailable. Please try again later.',
    entropyUnavailable: 'Check-in settlement is temporarily unavailable. Please try again later.',
    invalidRequest: 'This check-in request is no longer valid. Refresh the page and try again.',
    turnstileRequired: 'Please complete the verification first.',
    normalVerification: 'Normal check-in verification',
    luckyVerification: 'Lucky check-in verification',
    verificationComplete: 'Verified',
    turnstileFailed: 'Verification failed. Please try again.',
    turnstileUnavailable: 'The verification service is temporarily unavailable. Please try again later.',
    turnstileNotConfigured: 'Check-in verification is not configured yet. Please contact an administrator.'
  }
}
