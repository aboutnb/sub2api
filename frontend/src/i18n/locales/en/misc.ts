export default {
  ...
  onboarding: {
    ...
    admin: {
      ...
      accountPriority: {
        ...
        description: '<div style="line-height: 1.7;"><p style="margin-bottom: 12px;">Set the account call priority.</p><div style="padding: 8px 12px; background: #eff6ff; border-left: 3px solid #3b82f6; border-radius: 4px; font-size: 13px; margin-bottom: 12px;"><b>📊 Priority Rules:</b><ul style="margin: 8px 0 0 16px;"><li>Priority 1 is highest</li><li>Lower values are used first</li><li>Equal priorities use the other scheduling rules</li></ul></div><p style="padding: 8px 12px; background: #f0fdf4; border-left: 3px solid #10b981; border-radius: 4px; font-size: 13px;"><b>💡 Use Case:</b> Set the main account to a lower value (1 is highest) and backup accounts to higher values</p></div>',
        nextBtn: 'Next'
      },
      accountGroups: {
  ...
  payment: {
    ...
    tabUSDT: 'USDT',
    usdt: {
      moduleLabel: 'Independent on-chain payment',
      loadFailed: 'USDT payment is temporarily unavailable',
      unavailableTitle: 'USDT recharge is not available',
      title: 'USDT on-chain payment',
      network: 'Network',
      selectedNetwork: 'Payment network for this order',
      networkInstruction: 'Send only USDT on this network: USDT · {network} ({standard}).',
      ...
      create: 'Create USDT order',
      confirmPayment: 'Confirm payment {amount} USDT',
      realtimeConversion: 'Real-time conversion',
      bonusLine: 'Recharge bonus {percent}%',
      estimatedBalance: 'Estimated balance added (including bonus)',
      createFailed: 'Could not create the USDT order. Try again later.',
      payToAddress: 'Transfer to receiving address',
      exactAmount: 'Exact amount to pay',
      cashierTitle: 'Native USDT cashier',
      cashierHint: 'Complete payment in the native cashier. Sub2API will synchronize the order status automatically.',
      cashierPopupTitle: 'Native cashier is ready',
      cashierPopupHint: 'The cashier opens in a new window. Complete payment there; the order amount and bonus rules remain unchanged.',
      cashierPopupBlockedHint: 'If the browser blocks the popup, use the button below to open it again.',
      cashierPopupBlocked: 'The browser blocked the cashier popup. Use “Open cashier” to try again.',
      openCashier: 'Open cashier',
      cancel: 'Cancel order',
      cancelConfirm: 'Cancel this USDT order? Confirm that you have not sent the transfer; this order will stop waiting for payment.',
      cancelPrompt: 'Only cancel if you have not sent any transfer. A cancelled order can no longer be paid.',
      confirmCancel: 'Confirm cancellation',
      keepPaying: 'Keep paying',
      cancelFailed: 'Could not cancel the order. Try again later.',
      terminalHint: 'This order is closed. Create a new order to continue; transfers already sent are still checked under the payment rules.',
      exchangeRate: 'Live exchange rate',
      rateUpdatedAt: 'Updated {time}',
      rateUnavailable: 'The live exchange rate is unavailable. Order creation is temporarily disabled.',
      statusHint: 'Closing the payment page does not cancel the order. Callback delivery and active reconciliation continue on the server.',
    ...
    invoice: {
      ...
      orderStatusHint: 'This order is already linked to an invoice request and cannot be invoiced again.',
      orderStatus: {
        draft: 'Invoice in progress',
        failed: 'Invoice retry needed',
        submitting: 'Submitting invoice',
        submission_unknown: 'Invoice result pending',
        pending: 'Invoice under review',
        approved: 'Approved, awaiting issue',
        completed: 'Invoiced',
      },
      taxNotRequired: 'Platform pays fee',
      taxRequired: 'Customer pays fee',
      platformPaysTaxNotice: 'The invoice covers the order amount; the payment platform deducts the invoice fee from the merchant.',
      userPaysTaxNotice: 'The customer pays the invoice fee first; the invoice total is the order amount plus that fee.',
      draftPending: 'An invoice request is in progress',
      draftSummary: '{count} selected orders and the current progress are retained.',
      resumeDraft: 'Resume request',
      abandonDraft: 'Abandon request',
      saveAndClose: 'Save and close',
      selectOrder: 'Select',
      selectOrdersStep: 'Select Orders',
      taxReviewStep: 'Review Tax',
      buyerInfoStep: 'Buyer Details',
      applicationTitle: 'Submit Invoice Request',
      orderCount: 'Orders',
      orderAmount: 'Order Amount',
      invoiceAmount: 'Invoice Total',
      taxAmount: 'Invoice Fee',
      taxDue: 'Invoice Fee Due',
      taxPending: 'Payment required',
      payTaxTitle: 'Pay Invoice Fee',
      payTaxNotice: 'Pay {fee}. Once confirmed, you can continue with the invoice details.',
      openCashier: 'Open cashier',
      returnToConfirm: 'After paying, return here to confirm the payment status.',
      checkTaxPayment: 'Check Payment Status',
      buyerType: 'Buyer Type',
      requiredInfo: 'Required Details',
      optionalInfo: 'Additional Details',
      company: 'Company',
      individual: 'Individual',
      invoiceTitle: 'Invoice Title',
      taxpayerId: 'Taxpayer / Identity Number',
      recipientEmail: 'Recipient Email',
      emailNotice: 'The completed PDF invoice will also be sent to this address.',
      buyerAddress: 'Registered Address (optional)',
      buyerPhone: 'Registered Phone (optional)',
      buyerBank: 'Bank Name (optional)',
      buyerBankAccount: 'Bank Account (optional)',
      submitApplication: 'Submit Invoice Request',
      ...
      taxPaymentNotConfirmed: 'The tax payment has not been confirmed yet. Try again shortly.',
      taxReconciliationPending: 'This payment is confirmed, but tax is still due. Complete the remaining payment.',
      taxPaymentConfirmed: 'Tax payment confirmed',
      taxCheckFailed: 'Failed to check the tax payment',
      applicationSubmitted: 'Invoice request submitted',
      submitFailed: 'Failed to submit the invoice request',
      recordsFailed: 'Failed to load invoice records',
      confirmCancelApplication: 'Cancel this invoice request?',
      applicationCanceled: 'Invoice request canceled',
      cancelFailed: 'Failed to cancel the invoice request',
      ...
      status: {
        ...
        submission_unknown: 'Confirmation Pending',
    ...
    errors: {
      ...
      USDT_PAYMENT_SECRET_DECRYPT_FAILED: 'The saved USDT merchant secret cannot be decrypted. Restore the original TOTP_ENCRYPTION_KEY or enter and save a new merchant HMAC secret.',
      USDT_PAYMENT_SECRET_ENCRYPTION_UNAVAILABLE: 'USDT merchant-secret encryption is unavailable. Configure a fixed TOTP_ENCRYPTION_KEY first.',
      USDT_PAYMENT_CONFIG_UNAVAILABLE: 'USDT payment configuration could not be read. Check the database row and server encryption key.',
      USDT_PAYMENT_CREDENTIALS_REQUIRED: 'USDT payment requires a Key ID and merchant HMAC secret.',
      USDT_PAYMENT_CREDENTIALS_REJECTED: 'BEpusdt rejected the merchant credentials. Check the Key ID and merchant HMAC secret.',
      USDT_PAYMENT_UPSTREAM_BLOCKED: 'The BEpusdt API was blocked by the upstream edge. Use a private server-side API URL.',
      USDT_PAYMENT_MERCHANT_API_MISSING: 'The BEpusdt address does not expose the merchant API. Check the API URL or deploy a merchant-enabled build.',
      USDT_PAYMENT_UPSTREAM_TIMEOUT: 'The BEpusdt API request timed out. Check server connectivity and firewall rules.',
      USDT_PAYMENT_UPSTREAM_UNAVAILABLE: 'The BEpusdt API is unavailable. Check its status, API URL, and server network.',
      USDT_PAYMENT_ORDER_TIMEOUT_INVALID: 'USDT order timeout must be between 180 and 3600 seconds.',
      USDT_PAYMENT_MINIMUM_AMOUNT_INVALID: 'The USDT minimum payment amount must be between 0.01 and 1000000.',
      USDT_AMOUNT_TOO_LOW: 'The USDT payment amount is below the configured minimum.',
      USDT_PAYMENT_LATE_WINDOW_INVALID: 'USDT late-payment window must be between 0 and 1440 minutes.',
      USDT_PAYMENT_REQUEST_TIMEOUT_INVALID: 'USDT request timeout must be between 1 and 30 seconds.',
      USDT_PAYMENT_RECONCILE_INTERVAL_INVALID: 'USDT reconciliation interval must be between 1 and 60 seconds.',
      USDT_PAYMENT_RECONCILE_BATCH_INVALID: 'USDT reconciliation batch size must be between 1 and 1000.',
      USDT_PAYMENT_WEBHOOK_SKEW_INVALID: 'USDT webhook clock skew must be between 30 and 900 seconds.',
      USDT_PAYMENT_URL_INVALID: 'USDT payment URL is invalid. Check the API and public callback URLs.',
      USDT_PAYMENT_FIAT_INVALID: 'USDT payment currently supports CNY settlement only.',
      USDT_PAYMENT_NETWORKS_REQUIRED: 'Select at least one network when USDT payment is enabled.',
      USDT_PAYMENT_NETWORK_INVALID: 'The enabled USDT network configuration is invalid.',
      INVOICE_NOT_CONFIGURED: 'Invoice service is not available.',
      INVOICE_INVALID_ORDERS: 'Select between 1 and 20 orders.',
      INVOICE_DUPLICATE_ORDER: 'The order selection contains duplicates.',
      INVOICE_ORDER_NOT_ELIGIBLE: 'Some orders are not eligible for invoicing. Select again.',
      INVOICE_ORDER_ALREADY_CLAIMED: 'Some orders already have an invoice request. Refresh the records.',
      INVOICE_DRAFT_ACTIVE: 'Resume or abandon the current unfinished invoice request first.',
      INVOICE_DRAFT_NOT_ACTIVE: 'This invoice draft is no longer active.',
      INVOICE_TAX_DRAFT_MUST_RESUME: 'This request may have an invoice-fee payment. Resume it to finish the request.',
      INVOICE_TAX_ORDER_REQUIRED: 'Invoice tax has not been fully paid.',
      INVOICE_INVALID_TAX_ORDERS: 'The tax order is invalid. Restart the invoice request.',
      INVOICE_TAX_PAYMENT_NOT_ELIGIBLE: 'The tax payment is unconfirmed or ineligible.',
      INVOICE_TAX_PAYMENT_EXCEEDS_REQUIRED: 'Paid tax exceeds the current requirement. Restore the original order set.',
      INVOICE_TAX_ORDER_ALREADY_CLAIMED: 'This tax order is already attached to another invoice request.',
      INVOICE_INVALID_FORM: 'Check the invoice information and try again.',
      INVOICE_CANNOT_CANCEL: 'This invoice request can no longer be canceled.',
      INVOICE_PDF_NOT_READY: 'The PDF invoice is not ready.',
      INVOICE_SUBMISSION_IN_PROGRESS: 'The invoice request is already being submitted.',
      INVOICE_SUBMISSION_UNKNOWN: 'Submission confirmation is pending. Refresh invoice records.',
      INVOICE_SERVICE_UNAVAILABLE: 'Invoice service is temporarily unavailable.',
      INVOICE_ORDER_RATE_LIMITED: 'Order validation is temporarily rate limited.',
      INVOICE_RATE_LIMITED: 'Invoice submission is temporarily rate limited.',
      INVOICE_TAX_STATUS_RATE_LIMITED: 'Tax status checks are temporarily rate limited.',
