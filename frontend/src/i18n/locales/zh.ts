  ...
  admin: {
    ...
    proxies: {
      ...
      projectMihomo: {
        ...
        subscriptionUrlsHint: '支持多条订阅链接，逐条添加后由 mihomo 统一聚合使用；只填名称不填链接会创建静态 YAML 源',
        proxySources: '代理源',
        proxySourceName: '名称',
        ...
        fetchMode: '拉取方式',
        fetchModeMihomo: 'mihomo 拉取',
        fetchModeMihomoHint: '由 mihomo 直接请求订阅，保留 mihomo/Clash 的请求特征，适合会自动识别 UA 或拦截后端 HTTP 客户端的订阅源。',
        fetchModeBackend: '后端静态文件',
        fetchModeBackendHint: '由 sub2api 后端下载并保存为 provider 静态文件。订阅 URL 过期或刷新失败时保留旧文件继续使用，删除该订阅源时会清理对应文件。',
