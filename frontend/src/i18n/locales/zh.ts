export default {
  ...
  auth: {
    ...
    errors: {
      ...
      EMAIL_ALIAS_NOT_ALLOWED: '不支持使用邮箱别名注册，请使用邮箱主地址。',
      EMAIL_DISPOSABLE_NOT_ALLOWED: '不支持使用临时邮箱注册，请使用长期可用的邮箱。',
      REGISTRATION_CHALLENGE_REQUIRED: '注册验证已失效，请刷新页面后重试。',
      REGISTRATION_CHALLENGE_INVALID: '注册验证未通过，请刷新页面后重试。',
      REGISTRATION_TOO_MANY_ATTEMPTS: '注册请求过于频繁，请稍后再试。',
      REGISTRATION_RISK_CONTROL_UNAVAILABLE: '注册验证服务暂时不可用，请稍后再试。',
      VERIFY_CODE_CONTEXT_MISMATCH: '验证码环境不一致，请重新获取验证码。',
    },
  ...
  admin: {
    ...
    accounts: {
      ...
      dataImportWarning: '导入将创建新账号与代理；可在下方选择分组，导入账号会立即绑定。',
      dataImportFile: '数据文件',
      dataImportGroupsLoading: '正在加载分组...',
      dataImportGroupsLoadFailed: '分组加载失败',
      dataImportUseProjectMihomo: '导入后改用 Project Mihomo 代理池分配',
      dataImportUseProjectMihomoHint:
        '开启后，本次导入的账号会自动分配到已配置的 Mihomo 监听端口，并按当前占用做均衡。',
      ...
      proxy: '代理',
      noProxy: '无代理',
      proxyModeDirect: '指定单个代理',
      proxyModeDirectHint: '手动选择某一个已配置代理，适合需要固定出口的账号。',
      proxyModeProjectMihomo: 'Project Mihomo 代理池',
      proxyModeProjectMihomoHint: '从已配置的 Mihomo 监听端口中自动分配更优路线。',
      proxyModeProjectMihomoSummary:
        '保存后将自动从 Project Mihomo 已同步的端口池中选择当前更空闲、延迟更优的代理；批量创建时会自动均衡分散。',
      concurrency: '并发数',
      loadFactor: '负载因子',
      ...
      batchTest: {
        ...
        testModel: '测试模型',
        defaultModel: '默认测试模型（按账号）',
        customModel: '指定统一模型',
        customModelPlaceholder: '例如 claude-sonnet-4-5 或 gpt-5.4',
        concurrency: '测试并发',
        concurrencyHint: '同时测试的账号数量，最大 50。',
        resultFilter: '结果筛选',
        filters: {
          all: '全部（{count}）',
          success: '成功（{count}）',
        ...
        empty: '暂无账号',
        noFilteredResults: '当前筛选下没有结果',
        noResult: '-',
        runningResult: '测试中...',
        successResult: '测试完成',
        stoppedResult: '已停止',
        noResponseBody: '响应体为空',
        streamEnded: '测试流结束但未返回完成事件',
        streamEndedWithModel: '测试流结束但未返回完成事件，模型：{model}'
      },
      // Stats Modal
      viewStats: '查看统计',
    ...
    proxies: {
      ...
      projectMihomo: {
        title: '多端口代理',
        manage: '配置',
        subscriptionUrl: '代理节点订阅地址',
        subscriptionUrls: '订阅链接',
        subscriptionUrlsHint: '支持多条订阅链接，URL 订阅由后端直连下载为本地 provider 文件，避免订阅刷新走代理 IP；只填名称不填链接会创建静态 YAML 源',
        proxySources: '代理源',
        proxySourceName: '名称',
        proxySourceNamePlaceholder: '例如 日本订阅',
        proxySourcePlaceholder: '粘贴 Clash 订阅链接',
        proxySourceFallback: '代理源 {index}',
        providerKey: '稳定源 ID',
        deleteSource: '删除此代理源',
        fetchMode: '拉取方式',
        fetchModeMihomo: '后端直连拉取',
        fetchModeMihomoHint: '兼容旧配置：保存时会转为后端直连拉取，避免订阅刷新经过代理 IP。',
        fetchModeBackend: '后端直连拉取',
        fetchModeBackendHint: '由 sub2api 后端直连订阅服务器下载并保存为 provider 静态文件，不经过当前代理节点。订阅 URL 过期或刷新失败时保留旧文件继续使用，删除该订阅源时会清理对应文件。',
        fetchModeStatic: '静态 YAML',
        fetchModeStaticHint: '直接保存粘贴的 Clash/Mihomo YAML 为本地 provider 文件，不再请求订阅链接；删除该订阅源时会清理对应文件。',
        staticYaml: 'YAML 内容',
        staticYamlPlaceholder: '粘贴包含 proxies 的 Clash/Mihomo YAML 内容',
        uploadYaml: '上传 YAML',
        yamlFileLoaded: '已载入 YAML 文件：{name}',
        yamlFileEmpty: 'YAML 文件内容为空',
        yamlFileTooLarge: 'YAML 文件不能超过 16MB',
        yamlFileLoadFailed: '读取 YAML 文件失败',
        targetHost: '项目代理主机',
        controllerUrl: '控制器地址',
        startPort: '起始端口',
        listenerCount: '端口数量',
        ...
        removeListener: '删除端口',
        autoRoute: '自动优选线路',
        autoRouteHint: '开启后，每个端口会在所选范围内按延迟自动切换更优节点，排除关键字仍然生效。',
        autoRouteTolerance: '切换容差（ms）',
        autoRouteInterval: '检测间隔（秒）',
        autoRouteAnywhere: '使用过滤后的全部节点',
        autoRouteRegionMeta: '{count} 个节点',
        nodeExclude: '排除节点',
        nodeExcludeHint: '开启后按关键字隐藏并跳过香港、台湾等节点，每行一个关键字。',
        nodeExcludePlaceholder: '香港\nHong Kong\nHK\n台湾\n台灣\nTaiwan\nTW',
        summary: '本地多端口代理分配',
        summaryHint: '{count} 个节点',
        listenerSummaryHint: '{count} 个端口',
        save: '保存配置',
        sync: '同步到代理',
        saved: '多端口代理配置已保存',
        saveFailed: '多端口代理配置保存失败',
        proxyInUseConfirm: '删除的 Mihomo 代理已有 {count} 个账号使用。\n代理：{proxies}\n账号：{accounts}\n继续后会清空这些账号的代理选择并删除该代理，是否继续？',
        syncDone: '多端口代理同步完成，新增 {created} 个，复用 {reused} 个，分配 {assigned} 个节点',
        syncFailed: '多端口代理同步失败',
        testNodeLatency: '测延迟',
        testingNodeLatency: '测延迟中...',
        testLatencyDone: '节点延迟检测完成，共 {count} 条',
        testLatencyDoneWithSelected: '节点延迟检测完成，代理源 {count} 条；当前已选 {selected} 个端口节点也已检测',
        testLatencyDoneWithSelectedPartial: '节点延迟检测完成，代理源 {count} 条；当前已选端口节点检测 {selected} 个，失败 {failed} 个',
        testLatencyDoneSelectedFailed: '节点延迟检测完成，代理源 {count} 条；当前已选端口节点批量检测失败',
        testLatencyFailed: '节点延迟检测失败',
        testSingleNodeLatency: '单独测延迟',
        testingSingleNodeLatency: '检测中...',
        testSingleNodeLatencyDone: '节点 {name} 延迟检测完成',
        testSingleNodeLatencyFailed: '节点延迟检测失败',
        protocolLabel: '协议 {protocol}',
        configPath: '配置文件 {path}',
        listenerRegion: '端口 {port} 地区',
        listenerNode: '端口 {port} 节点',
        listenerRoute: '端口 {port} 线路',
        regionAuto: '自动',
        regionPlaceholder: '从当前订阅节点中选择地区',
        nodePlaceholder: '从当前订阅节点中选择节点',
        routePlaceholder: '选择自动范围或具体节点',
        legacyRegion: '已保存地区',
        regionUnknown: '未知地区',
        latencyFailedShort: '失败',
        latencyUnknown: '未测延迟',
        latencyFailed: '延迟检测失败',
        listenerPorts: '监听端口',
        saveAndSync: '保存并同步'
      },
      // Batch import
