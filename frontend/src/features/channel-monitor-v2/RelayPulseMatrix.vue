<template>
  <section
    class="card flex min-h-[360px] flex-col overflow-visible !p-6"
  >
    <div class="card-header mb-4 flex shrink-0 flex-wrap items-start justify-between gap-3 !border-0 !p-0">
      ...
      <div class="flex w-full min-w-0 flex-wrap items-center justify-end gap-2 text-xs text-ink-muted dark:text-ink-muted sm:w-auto">
        ...
        <span class="hidden text-[11px] text-ink-muted dark:text-ink-muted sm:inline">{{ t('channelMonitorV2.matrix.wheelZoomX') }}</span>
        <button
          type="button"
    ...
    <div class="card-body min-h-0 flex-1 !p-0">
      <div
        v-if="rows.length"
        ref="scrollRef"
        class="matrix-scroll max-h-[min(42vh,420px)] max-w-full overflow-auto rounded-2xl bg-surface-muted/60 p-2 dark:bg-canvas/30"
        @wheel="onMatrixWheel"
      >
        <div class="matrix-table w-full" :style="tableStyle">
          <div
            class="matrix-header matrix-row sticky top-0 z-[3] bg-surface-muted text-[10px] font-semibold uppercase tracking-wide text-ink-muted dark:bg-canvas dark:text-ink-muted"
          ...
          <div
            ...
            class="matrix-row border-b border-line/80 dark:border-line/60"
            :class="showThroughput ? 'matrix-row--with-tps' : ''"
          >
            <div class="dimension-cell flex min-w-0 items-center gap-2 bg-white dark:bg-surface" :title="rowLabel(entry.row)">
              <span :class="['status-dot', cellClass(entry.row.health, entry.row.metrics.request_count)]"></span>
              <strong class="truncate text-xs font-semibold text-ink-strong dark:text-gray-100">{{ rowLabel(entry.row) }}</strong>
            </div>
            <strong class="summary-value bg-white text-xs font-medium tabular-nums text-ink dark:bg-surface dark:text-ink-muted">
              {{ successRate(entry.row.metrics) }}
            </strong>
            <strong
              class="summary-value bg-white text-xs font-medium tabular-nums text-ink dark:bg-surface dark:text-ink-muted"
      ...
      <div class="mt-4 flex flex-col gap-2" :aria-label="t('channelMonitorV2.matrix.legendAria')">
        <div class="flex items-center gap-2 text-[11px] text-ink-muted dark:text-ink-muted">
        ...
        <div class="flex flex-wrap gap-4 text-[11px] text-ink-muted dark:text-ink-muted">
