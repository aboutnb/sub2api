<template>
  <section aria-labelledby="prompt-policy-title" class="py-6">
    <div>
      ...
      <p class="mt-1 text-sm text-ink-muted dark:text-ink">{{ t('admin.promptAudit.policy.description') }}</p>
    </div>

    <div class="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(260px,0.45fr)]">
      <div class="rounded-xl border border-line p-4 dark:border-line/60 dark:bg-canvas/20 sm:p-5">
        <fieldset>
          <legend class="text-sm font-medium text-ink-strong dark:text-white">{{ t('admin.promptAudit.policy.scope') }}</legend>
          <div class="mt-3 flex flex-wrap gap-5 text-sm text-ink dark:text-ink-strong">
        ...
        <div v-if="!draft.all_groups" class="mt-4">
          <label class="block text-sm text-ink dark:text-ink-strong">
          ...
          <div class="mt-3 max-h-52 overflow-y-auto rounded-lg border border-line p-2 dark:border-line">
            <label v-for="group in filteredGroups" :key="group.id" class="flex cursor-pointer items-center justify-between gap-3 rounded-md px-2 py-2 text-sm hover:bg-surface-muted dark:hover:bg-dark-800">
              <span class="flex items-center gap-2 text-ink-strong dark:text-ink-strong">
                <input type="checkbox" :checked="draft.group_ids.includes(group.id)" @change="toggleGroup(group.id)" />
                {{ group.name }}
              </span>
              <span class="text-xs text-ink-muted dark:text-ink-muted">{{ group.platform }} · {{ group.status }}</span>
            </label>
            <p v-if="filteredGroups.length === 0" class="px-2 py-4 text-center text-sm text-ink-muted">{{ t('admin.promptAudit.policy.noGroups') }}</p>
          </div>
          ...
          <p class="mt-2 text-xs text-ink-muted dark:text-ink-muted">{{ t('admin.promptAudit.policy.selectedCount', { count: draft.group_ids.length }) }}</p>
        </div>

        <fieldset class="mt-5 border-t border-line pt-5 dark:border-line">
          <legend class="text-sm font-medium text-ink-strong dark:text-white">{{ t('admin.promptAudit.policy.scanners') }}</legend>
          <div class="mt-3 grid gap-2 sm:grid-cols-2">
            <label v-for="scanner in SCANNER_CATALOG" :key="scanner.id" class="flex items-center gap-2 rounded-md px-2 py-1.5 text-sm text-ink hover:bg-surface-muted dark:text-ink-strong dark:hover:bg-dark-800">
      ...
      <div class="space-y-4 rounded-xl border border-line p-4 dark:border-line/60 dark:bg-canvas/20 sm:p-5">
        <label class="block text-sm text-ink dark:text-ink-strong">
        ...
        <div class="rounded-lg bg-surface-muted px-4 py-3 text-sm text-ink dark:bg-canvas/50 dark:text-ink">
          <p class="font-medium text-ink-strong dark:text-ink-strong">{{ t('admin.promptAudit.policy.strategy') }}</p>
