<template>
  <section aria-labelledby="prompt-runtime-title" class="border-b border-line py-6 dark:border-line/60">
    <div class="flex flex-wrap items-start justify-between gap-3">
      <div>
        ...
        <p class="mt-1 text-sm text-ink-muted dark:text-ink">
    ...
    <div v-else-if="loading && !runtime" class="mt-5 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6" aria-busy="true">
      <div v-for="index in 6" :key="index" class="h-16 animate-pulse rounded-xl bg-surface-muted dark:bg-surface" />
    </div>
    <template v-else-if="runtime">
      <dl class="mt-5 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
        <div v-for="item in statusItems" :key="item.label" class="rounded-xl border border-line bg-surface-muted/80 px-3 py-3 dark:border-line/60 dark:bg-canvas/40">
          <dt class="text-xs text-ink-muted dark:text-ink-muted">{{ item.label }}</dt>
          <dd class="mt-1.5 flex items-center gap-2 text-sm font-semibold text-ink-strong dark:text-white">
      ...
      <div class="mt-4 grid gap-3 lg:grid-cols-[minmax(0,1.4fr)_minmax(220px,0.6fr)]">
        <div class="rounded-xl border border-line px-4 py-3 dark:border-line/60 dark:bg-canvas/20">
          <h3 class="text-sm font-medium text-ink-strong dark:text-white">{{ t('admin.promptAudit.runtime.guardMetrics') }}</h3>
          <div class="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
            <div v-for="metric in guardMetricItems" :key="metric.label" class="rounded-lg bg-surface-muted px-2.5 py-2 dark:bg-canvas/60">
              <p class="text-[11px] text-ink-muted dark:text-ink-muted">{{ metric.label }}</p>
              <p class="mt-0.5 text-sm font-semibold tabular-nums text-ink-strong dark:text-white">{{ metric.value }}</p>
            </div>
          </div>
          <p class="mt-3 text-xs leading-5 text-ink-muted dark:text-ink-muted">
            ...
            <span class="mx-1.5 text-ink-muted dark:text-dark-600">·</span>
        ...
        <div class="rounded-xl border border-line px-4 py-3 dark:border-line/60 dark:bg-canvas/20">
          <h3 class="text-sm font-medium text-ink-strong dark:text-white">{{ t('admin.promptAudit.runtime.latest') }}</h3>
          <p class="mt-2 text-sm text-ink dark:text-ink">
