<template>
  <section aria-labelledby="prompt-pool-title" class="border-b border-line py-6 dark:border-line/60">
    <div class="flex flex-wrap items-start justify-between gap-3">
      <div>
        ...
        <p class="mt-1 text-sm text-ink-muted dark:text-ink">{{ t('admin.promptAudit.pool.description') }}</p>
      </div>
    ...
    <div v-if="endpoints.length === 0" class="mt-5 rounded-xl border border-dashed border-line-strong px-5 py-10 text-center text-sm text-ink-muted dark:border-line-strong dark:bg-canvas/20 dark:text-ink">
      {{ t('admin.promptAudit.pool.empty') }}
    </div>
    <div v-else class="mt-5 overflow-hidden rounded-xl border border-line bg-white dark:border-line/60 dark:bg-canvas/20">
      <div class="hidden grid-cols-[minmax(260px,1.45fr)_minmax(210px,1fr)_minmax(190px,.8fr)_minmax(230px,1.15fr)_auto] gap-5 border-b border-l-[3px] border-b-gray-200 border-l-transparent bg-surface-muted/80 px-5 py-2.5 text-[11px] font-semibold uppercase tracking-[0.08em] text-ink-muted dark:border-b-dark-700/60 dark:bg-canvas/70 dark:text-ink-muted xl:grid">
        ...
        >
          <div class="flex min-w-0 items-center gap-3">
            <button
              ...
              :class="endpoint.enabled ? 'bg-primary-600' : 'bg-line dark:bg-line-strong'"
            ...
            <div class="min-w-0">
              <div class="flex min-w-0 items-center gap-2">
                ...
                <span class="h-1.5 w-1.5 shrink-0 rounded-full" :class="endpoint.enabled ? 'bg-emerald-500' : 'bg-line-strong dark:bg-dark-500'" aria-hidden="true" />
              </div>
              <p class="mt-0.5 truncate font-mono text-[11px] text-ink-muted dark:text-ink-muted" :title="endpoint.base_url">{{ endpoint.base_url }}</p>
            </div>
          </div>

          <div class="min-w-0 xl:block">
            <p class="mb-1 text-[10px] font-semibold uppercase tracking-wider text-ink-muted xl:hidden">{{ t('admin.promptAudit.pool.model') }}</p>
            <p class="truncate text-sm font-medium text-ink dark:text-ink-strong" :title="endpoint.model">{{ endpoint.model }}</p>
          </div>

          <div>
            <p class="mb-1 text-[10px] font-semibold uppercase tracking-wider text-ink-muted xl:hidden">{{ t('admin.promptAudit.pool.limits') }}</p>
            <div class="flex flex-wrap gap-1.5 text-xs text-ink dark:text-ink">
              <span class="rounded-md bg-surface-muted px-2 py-1 tabular-nums dark:bg-surface">{{ endpoint.timeout_ms }} ms</span>
              <span class="rounded-md bg-surface-muted px-2 py-1 tabular-nums dark:bg-surface">{{ endpoint.input_limit }} chars</span>
            </div>
          </div>

          <div class="min-w-0">
            <p class="mb-1 text-[10px] font-semibold uppercase tracking-wider text-ink-muted xl:hidden">{{ t('admin.promptAudit.pool.credential') }}</p>
            <div class="flex items-center gap-1.5 text-xs font-medium" :class="credentialInvalid(endpoint) ? 'text-red-600 dark:text-red-300' : hasCredential(endpoint) ? 'text-emerald-700 dark:text-emerald-300' : 'text-ink-muted dark:text-ink-muted'">
              <span class="h-1.5 w-1.5 rounded-full" :class="credentialInvalid(endpoint) ? 'bg-red-500' : hasCredential(endpoint) ? 'bg-emerald-500' : 'bg-line-strong dark:bg-dark-500'" aria-hidden="true" />
          ...
          <div class="flex flex-wrap items-center justify-end gap-1 border-t border-line pt-3 dark:border-line xl:flex-nowrap xl:border-0 xl:pt-0">
    ...
    <BaseDialog :show="Boolean(editing)" :title="editingIndex < 0 ? t('admin.promptAudit.pool.add') : t('admin.promptAudit.pool.edit')" width="wide" @close="closeEditor">
      <form v-if="editing" class="grid gap-4 sm:grid-cols-2" @submit.prevent="saveEditor">
        <label class="space-y-1 text-sm text-ink dark:text-ink-strong">
        ...
        <label class="space-y-1 text-sm text-ink dark:text-ink-strong sm:col-span-2">
        ...
        <label class="space-y-1 text-sm text-ink dark:text-ink-strong sm:col-span-2">
          ...
          <span class="block text-xs text-ink-muted dark:text-ink-muted">{{ t('admin.promptAudit.pool.secretHint') }}</span>
        </label>
