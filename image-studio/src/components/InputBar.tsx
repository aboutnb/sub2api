...
import { studioCapabilities, studioConfig } from '../lib/studioBridge'
...
export default function InputBar() {
  ...
  const handleDownloadSelected = useCallback(async () => {
    ...
    try {
      ...
      if (successCount === 0) {
        ...
        showToast(successCount > 1 ? `下载成功：${successCount} 张图片` : studioText('下载成功'), 'success')
      }
    } catch (err) {
      console.error(err)
      showToast(studioText('下载失败'), 'error')
  ...
  const [mobileCollapsed, setMobileCollapsed] = useState(() => Boolean(studioConfig()))
  ...
  const hasSubmitApiConfig = Boolean(activeProfile.apiKey) || activeProfile.id.startsWith('studio:')
  const API_MAX_IMAGES = studioCapabilities(activeProfile)?.max_input_images ?? 16
  const canSubmit = Boolean(prompt.trim() && hasSubmitApiConfig && !activeAgentIsRunning)
  const submitButtonAriaLabel = activeAgentIsRunning
    ? studioText('停止生成')
    : hasSubmitApiConfig
    ? maskDraft ? studioText('遮罩编辑') : studioText('生成图像')
    : studioText('请先配置 API')
  const submitTooltipText = activeAgentIsRunning ? studioText('停止生成') : '尚未完成 API 配置，请在右上角设置中进行'
  const promptPlaceholder = studioText('描述你想生成的图片，可输入 @ 来指定参考图...')
  ...
  const uploadImageTooltipText = atImageLimit ? `参考图数量已达上限（${API_MAX_IMAGES} 张），无法继续添加` : studioText('上传图片')
  ...
  const renderClearAllButton = () => (
    ...
    >
      ...
      <span className="text-[8px] leading-none">{maskTargetImage ? studioText('清空全部') : studioText('清空')}</span>
    </button>
  )
        ...
        <div ref={cardRef} className={`studio-composer bg-white/70 dark:bg-gray-900/70 backdrop-blur-2xl border border-white/50 dark:border-white/[0.08] shadow-[0_8px_30px_rgb(0,0,0,0.08)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.3)] rounded-2xl sm:rounded-3xl p-3 sm:p-4 ring-1 ring-black/5 dark:ring-white/10${promptExpanded ? ' flex min-h-0 flex-1 flex-col' : ''}`}>
          ...
          <div className={`relative grid${promptExpanded ? ' min-h-0 flex-1' : ''}`}>
            ...
            {prompt.length > 0 && (
              ...
              >
                <ButtonTooltip visible={clearPromptHover} text={studioText("清空文本")} />
                <button
                  type="button"
                  onClick={() => {
          ...
          <div className="mt-3">
            {/* 桌面端布局 */}
            <div className="studio-desktop-params hidden sm:flex items-end justify-between gap-3">
              {renderParams('grid-cols-6')}

              <div className="studio-parameter-actions flex gap-2 flex-shrink-0">
                <div
                  className="relative"
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  >
                    ...
                    {activeAgentIsRunning ? studioText('停止生成') : maskDraft ? studioText('遮罩编辑') : studioText('生成图像')}
                  </button>
                </div>
              </div>
            </div>
