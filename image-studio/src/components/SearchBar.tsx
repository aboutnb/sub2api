...
export default function SearchBar() {
  ...
  const favoriteTooltip = activeFavoriteCollectionId ? studioText('返回收藏夹') : filterFavorite ? studioText('退出收藏夹') : studioText('收藏夹')
  ...
  return (
    <div ref={rootRef} data-no-drag-select className="studio-search mt-6 mb-4 flex gap-3">
          ...
          <>
            <div className="relative w-[88px]">
              <Select
                ...
                options={[
                  { label: studioText('全部'), value: 'all' },
                  { label: studioText('已完成'), value: 'done' },
                  { label: studioText('生成中'), value: 'running' },
                  { label: studioText('失败'), value: 'error' },
                ]}
            ...
            {isFailedFilter && (
              <button
                ...
                title={failedCount > 0 ? `清除 ${failedCount} 条失败记录` : studioText('没有失败记录')}
                aria-label={failedCount > 0 ? `清除 ${failedCount} 条失败记录` : studioText('没有失败记录')}
      ...
      <div className="relative z-10 flex-1">
        ...
        <input
          ...
          type="text"
          placeholder={inCollectionOverview ? studioText('搜索收藏夹名称...') : studioText('搜索提示词、参数...')}
