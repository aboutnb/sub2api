...
export default function DetailModal() {
  ...
  const taskProviderName = taskProvider ? getApiProviderLabel(settings, taskProvider) : studioText('未知')
  const taskProfileName = task.apiProfileName || studioText('未知')
  const taskModel = task.apiModel || studioText('未知')
  ...
  const outputCompressionText = task.params.output_compression == null ? studioText('未设置') : String(task.params.output_compression)
  ...
  const handleDelete = () => {
    setDetailTaskId(null)
    setConfirmDialog({
      title: studioText('删除任务'),
      message: studioText('确定要删除这个任务吗？关联的图片资源也会被清理（如果没有其他任务引用）。'),
  ...
  const handleCopyError = async () => {
    const errorText = task.error || studioText('生成失败')
    try {
      await copyTextToClipboard(errorText)
      showToast(studioText('完整报错已复制'), 'success')
    } catch (err) {
  ...
  const handleCopyPrompt = async () => {
    ...
    try {
      ...
      showToast(studioText('提示词已复制'), 'success')
    } catch (err) {
  ...
  const handleCopyInputImage = async () => {
    ...
    try {
      ...
      showToast(studioText('参考图已复制'), 'success')
    } catch (err) {
      console.error(err)
  ...
  const handleDownloadCurrentOutput = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (result.successCount === 0) {
        showToast(studioText('下载失败'), 'error')
      } else {
        showToast(studioText('下载成功'), 'success')
      }
    } catch (err) {
      console.error(err)
      showToast(studioText('下载失败'), 'error')
  ...
  const handleDownloadCurrentOriginalOutput = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (result.successCount === 0) {
        ...
        showToast(studioText('原图下载成功'), 'success')
      }
    } catch (err) {
      console.error(err)
      showToast(studioText('下载失败'), 'error')
  ...
  const handleDownloadAllOutputs = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (result.successCount === 0) {
        ...
        showToast(result.successCount > 1 ? `下载成功：${result.successCount} 张图片` : studioText('下载成功'), 'success')
      }
    } catch (err) {
      console.error(err)
      showToast(studioText('下载失败'), 'error')
    ...
    >
        ...
        <div className="md:w-1/2 w-full h-64 md:h-auto bg-gray-100 dark:bg-black/20 relative flex items-center justify-center flex-shrink-0 min-h-[16rem]">
              ...
              {currentOutputImageId && (
                <div className="relative group flex">
                  <button
                    ...
                    aria-label={studioText("下载图片")}
                  >
                    <DownloadIcon className="h-4 w-4" />
                  </button>
                  <ViewportTooltip visible={downloadImageTooltip.visible} className="whitespace-nowrap">{studioText("下载图片")}</ViewportTooltip>
                </div>
              )}
              {task.outputImages.length > 1 && (
                <div className="relative group flex">
                  <button
                    ...
                    aria-label={studioText("下载全部")}
                  >
                    <DownloadIcon className="h-4 w-4" />
                    <span className="text-[9px] font-bold leading-none mt-[1px]">ALL</span>
                  </button>
                  <ViewportTooltip visible={downloadAllTooltip.visible} className="whitespace-nowrap">{studioText("下载全部")}</ViewportTooltip>
                </div>
              )}
            </div>
          )}
              ...
              {currentOriginalOutputImageId && (
                <div className="absolute bottom-4 right-4 z-20 flex">
                  <button
                    ...
                    aria-label={studioText("下载原图")}
                  >
                    <DownloadIcon className="h-4 w-4" />
                    <span className="text-[9px] font-bold leading-none mt-[1px] uppercase">orig</span>
                  </button>
                  <ViewportTooltip visible={downloadOriginalImageTooltip.visible} className="whitespace-nowrap">{studioText("下载原图")}</ViewportTooltip>
                </div>
              )}
            </>
          )}
          ...
          {task.status === 'error' && isFalReconnecting && (
            <div className="w-full max-w-md px-4 text-center">
              ...
              <p className="text-sm font-medium text-yellow-500">{studioText("重连中")}</p>
            </div>
          )}
          {task.status === 'error' && !isFalReconnecting && (
            <div className="w-full max-w-md px-4 text-center">
              ...
              <div className="mt-3 flex items-center justify-center gap-2">
                <div className="relative group">
                  <button
                    ...
                    aria-label={studioText("复制完整报错")}
                  >
                    <CopyIcon className="h-4 w-4" />
                  </button>
                  <ViewportTooltip visible={copyErrorTooltip.visible} className="whitespace-nowrap">{studioText("复制完整报错")}</ViewportTooltip>
                </div>
                {task.rawResponsePayload && (
                  <div className="relative group">
                    <button
                      ...
                      aria-label={studioText("查看原始响应")}
                    >
                      <CodeIcon className="h-4 w-4" />
                    </button>
                    <ViewportTooltip visible={viewRawResponseTooltip.visible} className="whitespace-nowrap">{studioText("查看原始响应")}</ViewportTooltip>
                  </div>
                )}
                {task.rawImageUrls && task.rawImageUrls.length > 0 && (
                  <div className="relative group">
                    <button
                      ...
                      onClick={async () => {
                        if (task.rawImageUrls!.length === 1) {
                          ...
                          try {
                            ...
                            showToast(studioText('图片链接已复制'), 'success')
                          } catch (err) {
                      ...
                      aria-label={studioText("复制图片链接")}
                    >
                      <LinkIcon className="h-4 w-4" />
                    </button>
                    <ViewportTooltip visible={copyRawUrlsTooltip.visible} className="whitespace-nowrap">{studioText("复制图片链接")}</ViewportTooltip>
                  </div>
                )}
                {streamPartialImageIds.length > 0 && (
                  <div className="relative group">
                    <button
                      ...
                      aria-label={studioText("下载中间步骤图")}
                    >
                      <DownloadIcon className="h-4 w-4" />
                    </button>
                    <ViewportTooltip visible={downloadPartialImagesTooltip.visible} className="whitespace-nowrap">{studioText("下载中间步骤图")}</ViewportTooltip>
                  </div>
                )}
                <div className="relative group">
                  <button
                    ...
                    aria-label={studioText("重试任务")}
                  >
                    <svg className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                    </svg>
                  </button>
                  <ViewportTooltip visible={retryTooltip.visible} className="whitespace-nowrap">{studioText("重试任务")}</ViewportTooltip>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="md:w-1/2 w-full p-5 overflow-y-auto overscroll-contain flex flex-col">
          ...
          <div data-selectable-text className="flex-1">
            ...
            {showReferenceSection && (
              <div className="mb-4">
                <div className="flex items-center gap-1.5 mb-2">
                  <h3 className="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wider">{studioText("参考图")}</h3>
                  {allInputImageIds.length > 0 && (
                    <button
                      onClick={handleCopyInputImage}
                      className="p-1 rounded text-gray-400 hover:bg-gray-100 dark:text-gray-500 dark:hover:bg-white/[0.06] transition"
                      title={studioText("复制参考图")}
                    >
                      <CopyIcon className="h-4 w-4" />
                    </button>
                  )}
            ...
            {/* 参数 */}
            <h3 className="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-2">{studioText("参数配置")}</h3>
            {showSourceInfo && (
              <div className="mb-2 min-w-0 overflow-hidden rounded-lg bg-gray-50 px-3 py-2 text-xs dark:bg-white/[0.03]">
                <span className="text-gray-400 dark:text-gray-500">{studioText("来源")}</span>
                <br />
            ...
            <div className="grid grid-cols-2 gap-2 text-xs mb-4 min-w-0">
              <div className="bg-gray-50 dark:bg-white/[0.03] rounded-lg px-3 py-2 min-w-0 overflow-hidden">
                <span className="text-gray-400 dark:text-gray-500">{studioText("尺寸")}</span>
                <br />
                <div className="mt-0.5 overflow-x-auto hide-scrollbar whitespace-nowrap mask-edge-r pr-2">
              ...
              <div className="bg-gray-50 dark:bg-white/[0.03] rounded-lg px-3 py-2 min-w-0 overflow-hidden">
                <span className="text-gray-400 dark:text-gray-500">{studioText("质量")}</span>
                <br />
                <div className="mt-0.5 overflow-x-auto hide-scrollbar whitespace-nowrap mask-edge-r pr-2">
              ...
              <div className="bg-gray-50 dark:bg-white/[0.03] rounded-lg px-3 py-2 min-w-0 overflow-hidden">
                <span className="text-gray-400 dark:text-gray-500">{studioText("格式")}</span>
                <br />
                <div className="mt-0.5 overflow-x-auto hide-scrollbar whitespace-nowrap mask-edge-r pr-2">
              ...
              {supportsTransparentOutput && (
                <div className="bg-gray-50 dark:bg-white/[0.03] rounded-lg px-3 py-2 min-w-0 overflow-hidden">
                  <span className="text-gray-400 dark:text-gray-500">{studioText("透明背景")}</span>
                  <br />
                  <div className="mt-0.5 overflow-x-auto hide-scrollbar whitespace-nowrap mask-edge-r pr-2">
              ...
              {task.params.output_format !== 'png' && (
                <div className="bg-gray-50 dark:bg-white/[0.03] rounded-lg px-3 py-2 min-w-0 overflow-hidden">
                  <span className="text-gray-400 dark:text-gray-500">{studioText("压缩率")}</span>
                  <br />
                  <div className="mt-0.5 overflow-x-auto hide-scrollbar whitespace-nowrap mask-edge-r pr-2">
              ...
              <div className="bg-gray-50 dark:bg-white/[0.03] rounded-lg px-3 py-2 min-w-0 overflow-hidden">
                <span className="text-gray-400 dark:text-gray-500">{studioText("审核")}</span>
                <br />
                <div className="mt-0.5 overflow-x-auto hide-scrollbar whitespace-nowrap mask-edge-r pr-2">
              ...
              {!isAgentTask && (
                <div className="bg-gray-50 dark:bg-white/[0.03] rounded-lg px-3 py-2 min-w-0 overflow-hidden">
                  <span className="text-gray-400 dark:text-gray-500">{studioText("数量")}</span>
                  <br />
                  <div className="mt-0.5 overflow-x-auto hide-scrollbar whitespace-nowrap mask-edge-r pr-2">
          ...
          {/* 操作按钮 */}
          <div className="grid grid-cols-4 sm:flex sm:flex-wrap gap-2 pt-4 border-t border-gray-100 dark:border-white/[0.08]">
            <button
              onClick={handleReuse}
            ...
            >
              <EditIcon className="w-4 h-4 flex-shrink-0" />{studioText("编辑输出")}</button>
            <button
            ...
            >
              <TrashIcon className="w-4 h-4 flex-shrink-0" />{studioText("删除任务")}</button>
            <button
              onClick={handleToggleFavorite}
              className={`col-span-1 sm:flex-none sm:w-11 w-full min-h-10 flex items-center justify-center rounded-xl transition ${
                task.isFavorite
              ...
              title={task.isFavorite ? studioText('编辑收藏夹') : studioText('收藏任务')}
            >
          ...
          <div ref={rawUrlsModalRef} className="flex w-full max-w-2xl max-h-[90vh] flex-col overflow-hidden rounded-2xl bg-white shadow-xl dark:bg-[#1c1c1e]" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4 dark:border-white/[0.08] shrink-0">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  onClick={async () => {
                    try {
                      ...
                      showToast(studioText('复制成功'), 'success')
                    } catch (err) {
                      showToast(getClipboardFailureMessage(studioText('复制失败'), err), 'error')
                    }
                  }}
                ...
                >
                  <CopyIcon className="w-3.5 h-3.5" />{studioText("全部复制")}</button>
                <button
                  type="button"
                ...
                {rawImageUrls.map((url, i) => (
                  <div key={i} className="group flex items-center gap-3 p-3 sm:p-4 rounded-xl bg-white dark:bg-[#1c1c1e] border border-gray-100 dark:border-white/[0.06] shadow-sm hover:shadow-md transition-all">
                    ...
                    <button
                      ...
                      title={studioText("复制链接")}
                    >
                      <CopyIcon className="w-4 h-4 sm:w-3.5 sm:h-3.5" />
                      <span className="hidden sm:inline">{studioText("复制")}</span>
                    </button>
                  </div>
                ))}
              </div>
      ...
      {showRawResponseModal && task?.rawResponsePayload && (
        ...
        >
          ...
          >
            <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4 dark:border-white/[0.08] shrink-0">
              <h3 className="text-base font-semibold text-gray-900 dark:text-white">{studioText("原始响应数据")}</h3>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={async () => {
