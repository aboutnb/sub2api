        ...
        <div className="w-40 min-w-[10rem] h-full bg-gray-100 dark:bg-black/20 relative flex items-center justify-center overflow-hidden flex-shrink-0">
          {task.status === 'running' && streamPreviewSrc && (
            <>
              ...
              />
              {streamPreviewLoaded && (
                <span className="absolute top-1.5 right-1.5 flex items-center gap-1 rounded bg-blue-500 px-1.5 py-0.5 text-[10px] font-medium text-white backdrop-blur-sm sm:text-xs">{studioText("预览")}</span>
              )}
            </>
          )}
          ...
          {task.status === 'error' && isFalReconnecting && (
            <div className="flex flex-col items-center gap-1 px-2">
              ...
              <span className="text-xs text-yellow-500 text-center leading-tight">{studioText("重连中")}</span>
            </div>
          )}
          {task.status === 'error' && !isFalReconnecting && (
            <div className="flex flex-col items-center gap-1 px-2">
              ...
              <span className={`text-xs text-center leading-tight ${isInterrupted ? 'text-yellow-500' : 'text-red-400'}`}>
                {isInterrupted ? studioText('已停止') : studioText('失败')}
              </span>
            </div>
          )}
        ...
        <div className="flex-1 p-3 flex flex-col min-w-0">
          ...
          <div className="mt-auto flex flex-col gap-1.5">
            ...
            >
              ...
              {showTransparentOutput && (
                <span className="flex items-center gap-1 px-1.5 py-0.5 rounded bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-xs flex-shrink-0">
                  <TransparentBgIcon className="w-3 h-3 flex-shrink-0" />{studioText("透明背景")}</span>
              )}
            ...
            >
              ...
              <TaskActionButton
                tooltip={task.isFavorite ? studioText('编辑收藏夹') : studioText('收藏任务')}
