...
function fillWhiteMask(canvas: HTMLCanvasElement) {
  ...
  if (!ctx) throw new Error(studioText('当前浏览器不支持 Canvas'))
...
function drawMaskImageToCanvas(maskImage: HTMLImageElement, maskCanvas: HTMLCanvasElement) {
  ...
  if (Math.abs(maskAspect - canvasAspect) > 0.001) {
    throw new Error(studioText('遮罩尺寸与当前图片不一致'))
  }

  const maskCtx = maskCanvas.getContext('2d', { willReadFrequently: true })
  if (!maskCtx) throw new Error(studioText('当前浏览器不支持 Canvas'))
...
export default function MaskEditorModal() {
  ...
  const handleRemoveMask = () => {
    setConfirmDialog({
      title: studioText('移除遮罩'),
      message: studioText('确定要撤销对这张图片的所有涂抹并移除遮罩吗？'),
      tone: 'danger',
      action: () => {
        clearMaskDraft()
        setMaskEditorImageId(null)
        showToast(studioText('已移除遮罩'), 'success')
      },
    })
  }
  ...
  useEffect(() => {
    ...
    async function loadCanvases() {
      try {
        ...
        if (cancelled) return
        if (!dataUrl) {
          showToast(studioText('图片已不存在，无法编辑遮罩'), 'error')
          setMaskEditorImageId(null)
          return
        }
        ...
        if (!imageCtx) throw new Error(studioText('当前浏览器不支持 Canvas'))
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMaskEditorImageId(null)
      showToast(studioText('遮罩已保存'), 'success')
    } catch (err) {
      if (
        saveTokenRef.current !== token ||
        activeSessionIdRef.current !== savingSessionId ||
    ...
    <>
      ...
      <div className="flex-none flex items-center justify-between px-4 py-3 sm:px-6 sm:py-3.5 border-b border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-950 z-20">
        <div className="flex items-center gap-3">
          <button onClick={close} disabled={isSaving} className="p-2 sm:p-2.5 -ml-2 text-gray-500 hover:bg-gray-100 rounded-lg sm:rounded-xl dark:text-gray-400 dark:hover:bg-gray-800 transition" title={studioText("取消")}>
            <svg className="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/></svg>
          </button>
          <div className="relative flex items-center gap-1.5 sm:gap-2">
            <h2 className="text-sm sm:text-base font-semibold text-gray-800 dark:text-gray-200" id="mask-editor-title">{studioText("编辑遮罩")}</h2>
            <button
              type="button"
            ...
            {showMaskInfo && (
              <div className="absolute left-0 top-full mt-2 w-64 sm:w-72 rounded-xl border border-gray-200/80 bg-white px-3.5 py-2.5 text-xs sm:text-sm leading-5 sm:leading-6 text-gray-600 shadow-lg dark:border-white/[0.08] dark:bg-gray-900 dark:text-gray-300">
                ...
                <p>{studioText("根据官方文档说明，此功能仅基于提示词，无法完全控制模型编辑区域。")}</p>
                <p className="mt-2">{studioText("建议附加类似“只编辑遮罩区域”的提示词以提升模型指令遵循程度。")}</p>
              </div>
            )}
          </div>
        </div>
        <div className="flex items-center gap-2 sm:gap-3">
          {maskDraft?.targetImageId === imageId && (
            <button onClick={handleRemoveMask} className="flex h-8 sm:h-[38px] items-center gap-1.5 px-3.5 sm:px-4 text-xs sm:text-sm font-medium rounded-xl bg-gray-100 dark:bg-white/[0.08] text-gray-700 dark:text-gray-300 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-500/20 dark:hover:text-red-400 transition">{studioText("移除遮罩")}</button>
          )}
          <button onClick={handleSave} disabled={!isReady || isSaving} className="flex h-8 sm:h-[38px] items-center gap-1.5 px-4 sm:px-5 text-xs sm:text-sm font-medium rounded-xl text-white bg-blue-500 hover:bg-blue-600 active:bg-blue-700 shadow-sm transition disabled:opacity-50">
            {isSaving ? studioText('保存中...') : studioText('保存')}
          </button>
        </div>
      </div>
      ...
      <div ref={stageRef} className="flex-1 relative flex items-center justify-center overflow-hidden bg-gray-100/50 dark:bg-black/50 p-0 pb-[76px] sm:p-6 sm:pb-[100px]" style={{ containerType: 'size' }}>
        {isLoading && (
          <div className="absolute inset-0 z-20 flex items-center justify-center bg-white/50 text-sm text-gray-500 backdrop-blur-sm dark:bg-gray-900/50 dark:text-gray-300">{studioText("正在载入图片...")}</div>
        )}
        <div
          ref={baseFrameRef}
        ...
        <div className="absolute bottom-4 sm:bottom-8 left-1/2 -translate-x-1/2 flex items-center justify-center z-20 pointer-events-none w-full px-2 sm:px-4">
          <div className="flex items-center gap-2 sm:gap-4 px-2 sm:px-3 py-1.5 sm:py-2 bg-white/95 dark:bg-[#0f0f0f]/95 backdrop-blur-md border border-gray-200/80 dark:border-white/5 rounded-2xl sm:rounded-[1.25rem] shadow-2xl pointer-events-auto">
            ...
            <div className="flex items-center gap-0.5 sm:gap-2 sm:ml-1">
              <button onClick={handleUndo} disabled={!canUndo} className="p-2 sm:p-2.5 text-gray-500 hover:bg-gray-100 rounded-lg sm:rounded-xl disabled:opacity-30 dark:text-[#8a8a8e] dark:hover:bg-white/10 dark:hover:text-gray-200 transition-all" title={studioText("撤销")}>
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M3 7v6h6" />
                  <path d="M21 17a9 9 0 0 0-9-9 9 9 0 0 0-6 2.3L3 13" />
                </svg>
              </button>
              <button onClick={handleRedo} disabled={!canRedo} className="p-2 sm:p-2.5 text-gray-500 hover:bg-gray-100 rounded-lg sm:rounded-xl disabled:opacity-30 dark:text-[#8a8a8e] dark:hover:bg-white/10 dark:hover:text-gray-200 transition-all" title={studioText("重做")}>
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M21 7v6h-6" />
              ...
              <button onClick={resetViewTransform} disabled={!isReady || isSaving || !isZoomed} className="p-2 sm:p-2.5 text-gray-500 hover:bg-gray-100 rounded-lg sm:rounded-xl disabled:opacity-30 dark:text-[#8a8a8e] dark:hover:bg-white/10 dark:hover:text-gray-200 transition-all" title={studioText("重置视图")}>
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M4 14h6v6"/>
                  <path d="M20 10h-6V4"/>
                  <path d="M14 10l7-7"/>
              ...
              <button onClick={handleClear} disabled={!isReady || isSaving} className="p-2 sm:p-2.5 text-gray-500 hover:bg-gray-100 rounded-lg sm:rounded-xl disabled:opacity-30 dark:text-[#8a8a8e] dark:hover:bg-white/10 dark:hover:text-gray-200 transition-all" title={studioText("清空遮罩")}>
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M3 6h18"/>
