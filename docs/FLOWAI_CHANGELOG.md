...
| 当前 HEAD（含治理提交） | `7784e928eabf87e412944d25c221f45abd9833dd` |
| 最后已审并合入的上游基线 | `upstream/main` = `3b7753a8e`（0.1.181） |
| 当前抓取但尚未合入的上游 | `7b693ae42`（0.1.183，117 个提交） |
| 应用版本 | `0.1.181` |
| 相对上游的非合并提交 | 85 个功能/修复提交（另有 1 个治理提交） |
...
文档提交按上面的受限规则豁免，但仍会被路径检查；脚本会逐个检查 hash 是否存在于标记区，
新增代码提交未登记时，CI/发布门禁失败。
