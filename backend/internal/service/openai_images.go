...
func (s *OpenAIGatewayService) ParseOpenAIImagesRequest(c *gin.Context, body []byte) (*OpenAIImagesRequest, error) {
	...
	if err := validateCompatibleImagesModel(req.Model); err != nil && !isImageStudioRequest(c.Request.Context()) {
		return nil, err
	}
...
func (s *OpenAIGatewayService) forwardOpenAIImagesAPIKey(
	...
	if err := validateCompatibleImagesModel(requestModel); err != nil && !isImageStudioRequest(ctx) {
		return nil, err
	}
