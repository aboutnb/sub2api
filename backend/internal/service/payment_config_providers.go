...
var (
	...
	easyPayCustomMethodUpstreamTypePattern = regexp.MustCompile(`^[a-z0-9_-]+(?:\.[a-z0-9_-]+)?$`)
)
...
func validateEasyPayCustomMethods(config map[string]string, supportedTypes string) error {
	...
	for _, method := range methods {
		...
		if !easyPayCustomMethodUpstreamTypePattern.MatchString(method.UpstreamType) {
			return infraerrors.BadRequest("VALIDATION_ERROR", "customMethods upstreamType may only contain lowercase letters, digits, underscores, hyphens, and at most one period")
		}
