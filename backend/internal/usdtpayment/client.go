...
// CreateCashierOrder uses BEpusdt's original MD5-signed API. The user chooses
// the chain in the embedded checkout page, so no network is sent here.
func (c *Client) CreateCashierOrder(ctx context.Context, request map[string]any) (*cashierCreateData, error) {
	...
	if err := c.legacyCall(ctx, http.MethodPost, "/api/v1/order/create-order", request, &envelope); err != nil {
		return nil, err
	}
	...
	if envelope.Data.PaymentURL == "" && c.publicBaseURL != "" {
		envelope.Data.PaymentURL = c.publicBaseURL + "/pay/checkout/" + url.PathEscape(envelope.Data.TradeID)
	}
	return &envelope.Data, nil
}
...
func (c *Client) CashierInfo(ctx context.Context, tradeID string) (*cashierInfoData, error) {
	...
	if err := c.legacyCall(ctx, http.MethodPost, "/api/v1/pay/info", map[string]any{"trade_id": tradeID}, &envelope); err != nil {
		return nil, err
	}
	if envelope.StatusCode != 200 {
...
func (c *Client) legacyCall(ctx context.Context, method, path string, input map[string]any, output any) error {
	request := make(map[string]any, len(input)+1)
	for key, value := range input {
		request[key] = value
	}
	request["signature"] = epusdtSign(request, c.legacyToken)
